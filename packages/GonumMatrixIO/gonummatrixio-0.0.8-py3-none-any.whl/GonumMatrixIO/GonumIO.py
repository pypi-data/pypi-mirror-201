...
def saveMatrix(matrix: _np.ndarray, filepath):
    """
    ...
    meta = _np.array([18373144242814977,matrix.shape[0],matrix.shape[1],0,0]).view("float")
    matrixData = matrix.flatten()
    binaryData = _np.concatenate([meta, matrixData])
    try:
...
def loadMatrix(filepath: str):
    ...
    :returns: (_np.ndarray) The numpy array with data from the matrix saved at filepath
    ...
    try:
        metadata = _np.fromfile(filepath, dtype="int", count=5)
    except FileNotFoundError:
        raise

    try:
        matrix = _np.fromfile(filepath, offset=40)
    except FileNotFoundError:
        raise
    except ValueError:
        raise
...
def saveVector(vec: _np.ndarray, filepath):
    """
    ...
    # My apologies for the magic number. See https://pkg.go.dev/gonum.org/v1/gonum@v0.12.0/mat#Dense.MarshalBinary for details
    meta = _np.array([18373144242814977,vec.shape[0],vec.shape[1],0,0]).view("float")
    vecData = vec.flatten()
    binaryData = _np.concatenate([meta, vecData])
    try:
        binaryData.tofile(filepath)
    except FileNotFoundError:
        raise
...
def loadVector(filepath):
    ...
    :returns: (_np.ndarray) The numpy array with data from the vector saved at filepath
    :raises FileNotFoundError: If filepath does not point to a valid file
    ...
    try:
        vec = _np.fromfile(filepath, dtype=_np.float64, offset=40)
    except FileNotFoundError:
        raise
    except ValueError:
        raise
