...
class PackageType(ModelSimple):
    ...
    def __init__(self, *args, **kwargs):
        """PackageType - a model defined in OpenAPI

        Note that value can be passed either in args or in kwargs, but not in both.

        Args:
            args[0] (str): Types of [packaging](#section/Description/Packaging-process):   * `SLOT` - space in the transport box for a stack of pallets, the height limits the stack of pallets   * `PALLET` - a pallet, the products are stacked on top of a pallet, with the help of a cardboard partition it can be divided into 2 - 6 parts, in each of which the same products are placed   * `MIXBOX` - mix-box, the products are folded inside the box ., must be one of ["SLOT", "PALLET", "MIXBOX", ]  
