...
def createDemoWidget():
    ...
    if os.path.exists(demoZipFile):
        ...
        if len(widgetid) == 0:
            utils.mecordPrint("create demoWidget fail! mecord server is not avalid")
            return
...
def createWidget(root):
    if PathIsEmpty(root) == False:
        utils.mecordPrint("current folder is not empty, create widget fail!")
        return
        
    widgetid = xy_pb.CreateWidgetUUID()
    if len(widgetid) == 0:
        utils.mecordPrint("create widget fail! mecord server is not avalid")
        return
    ...
    utils.mecordPrint("create widget success")

def CheckWidgetDataInPath(path):
    data = GetWidgetConfig(path)
    if "widget_id" not in data:
        utils.mecordPrint("folder is not widget")
        return False

    if "widget_id" in data:
        widget_id = data["widget_id"]
        if len(widget_id) == 0:
            utils.mecordPrint("widget_id is empty!")
            return False
...
def addWidgetToEnv(root):
    ...
    if store.insertWidgets(widget_id, mainPythonPath):
        utils.mecordPrint(f"add {widget_id.ljust(len(widget_id)+4)} {mainPythonPath}")
...
def remove(args):
    ...
    if os.path.exists(args):
        ...
        if "widget_id" not in data:
            utils.mecordPrint(f"path {args} is not widget folder!")
            return
        widget_id = data["widget_id"]
    if xy_pb.DeleteWidget(widget_id):
        utils.mecordPrint(f"widget:{widget_id} is removed from mecord server")
...
def publishWidget(root):
    ...
    if hasPermission == False:
        inputed = False
        while inputed == False:
            utils.mecordPrint(f"you do not have permission to access widget:[{widget_id}]")
            word = input("do you want to override it? Y/n:")
            if "Y" in word:
                changeOwner = True
            inputed = True
    if hasPermission == False and changeOwner == False:
        utils.mecordPrint("publish abandon")
        return
    ...
    if changeOwner:
        ...
        if len(newWidgetid) == 0:
            utils.mecordPrint("override fail! mecord server is not avalid")
            return
...
def checkUploadComplete(checkid, dist):
    ...
    if rst == 1: #success
        utils.mecordPrint("publish widget success")
        ...
        utils.mecordPrint("publish fail")
        return
