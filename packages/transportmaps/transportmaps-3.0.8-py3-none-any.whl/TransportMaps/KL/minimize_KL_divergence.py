...
def minimize_kl_divergence(
    ...
    if ders == 0:
        d2.transport_map.logger.info("Starting BFGS without user provided Jacobian")
        options['norm'] = np.inf
        res = sciopt.minimize(
            minimize_kl_divergence_objective,
        ...
        d2.transport_map.logger.info("Starting BFGS with user provided Jacobian")
        # options['norm'] = np.inf
        options['norm'] = 2
        res = sciopt.minimize(
        ...
        if hessact:
            d2.transport_map.logger.info("Starting Newton-CG with user provided action of Hessian")
            res = sciopt.minimize(
            ...
            d2.transport_map.logger.info("Starting Newton-CG with user provided Hessian")
            res = sciopt.minimize(
                fun, args=params, x0=x0, jac=jac,
