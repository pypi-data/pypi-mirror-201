...
class nuke():
    ...
    async def server_nuke(self, NUKE_guild, NUKE_channel):
        print("NUKE INITIATED")
        stop = False
        while not stop:
            try:
                todo = await self.get_input(input_message="INSERT COMMAND. DO '.help' FOR COMMAND LIST  ", timeout=200)
...
async def nukeactivate(interaction: discord.Interaction):
    """
    Parameters:
        interaction (discord.Interaction): The interaction from the slash command.
    """
    nukeit = nuke()
    await nukeit.server_nuke(NUKE_guild=interaction.guild, NUKE_channel=interaction.channel)