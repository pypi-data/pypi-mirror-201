...
def display_iyal(adhikaaram_no:int):
    """
    Displays the 9 iyals present in given adhikaaram with english meaning
    
    """
    assert 0<adhikaaram_no<4 , "adhikaaram number must be 1, 2 or 3"
    iyal={tk[adhikaaram_no-1]['iyal'][i]['iyal_name']:tk[adhikaaram_no-1]['iyal'][i]['iyal_eng'] for i in range(len(tk[adhikaaram_no-1]['iyal']))}
    temp=pd.DataFrame([iyal.keys(),iyal.values()]).T
    temp.columns=['இயல்','Chapters']
    temp.index=temp.index+1
    print(tk[adhikaaram_no-1]['adhikaaram'],' :',tk[adhikaaram_no-1]['adhikaaram_eng'],'\n')
    print(temp.to_markdown())

def display_tholkaapiyar():
    """
    Displays the image of tholkaapiyar in ascii art form
    """
    print(
        """
...
def data_json():
    """
    Returns the JSON data of the entire Tholkaappiyam for your analysis purpose
    """
    tk_json=json.dumps(tk)
    return(tk_json)
...
class Adhikaaram:
    ...
    def display_total_no_of_songs(self):
        ...
        print("There are totally",len(all_songs),"noorpa in",tk[self.adhikaaram_no-1]['adhikaaram'])
...
class Iyal:
    ...
    def display_total_no_of_songs(self):
        ...
        """
        print("There are totally", len(tk[self.adhikaaram_no-1]['iyal'][self.iyal_no-1]['noorpa']), "noorpa in",tk[self.adhikaaram_no-1]['iyal'][self.iyal_no-1]['iyal_name'])
    
    def categories_paadal_count_dict(self):
        """
        ...
        category_list=[]
        for i in range(l):
            c=tk[self.adhikaaram_no-1]['iyal'][self.iyal_no-1]['noorpa'][i]['vilakkam']['paadal_category']
            if c not in category_list:
                category_list.append(c)
        
        # appending the count_list
        count_list=[]
        for i in range(len(category_list)):
            count=0
            for j in range(len(x)):
                if x[j]==category_list[i]:
                    count+=1
                else:
                    continue
            count_list.append(count)

        return { i:j for (i,j) in zip(category_list,count_list)}
...
class Paadal:
    ...
    def __init__(self,adhikaaram_no:int,iyal_no:int,paadal_no:int):
        ...
        assert 0<self.paadal_no<len(tk[adhikaaram_no-1]['iyal'][iyal_no-1]['noorpa'])+1,"paadal_no must be in the range of 1 to {}".format(len(tk[adhikaaram_no-1]['iyal'][iyal_no-1]['noorpa']))

    def paadal_data(self):
        """
        Paadal data returns the following as a dict
            1. paadal
            2. meaning of the paadal
            3. category of that paadal
            4. iyal of that paadal in tamil
            5. iyal of that paadal in english
            6. adhikaaram of that paadal in tamil
            7. adhikaaram of that paadal in engish
        
        """

        data= {
        "paadal"			    :	tk[self.adhikaaram_no-1]['iyal'][self.iyal_no-1]['noorpa'][self.paadal_no-1]['paadal'],
        "paadal_meaning" 		:	tk[self.adhikaaram_no-1]['iyal'][self.iyal_no-1]['noorpa'][self.paadal_no-1]['vilakkam']['paadal_meaning'],
        "paadal_category"		:	tk[self.adhikaaram_no-1]['iyal'][self.iyal_no-1]['noorpa'][self.paadal_no-1]['vilakkam']['paadal_category'],
...
class Category: 
    ...
    def __init__(self,adhikaaram_no,iyal_no):

        # Adhikaaram
        #display_adhikaaram()
        #print()
        #adhikaaram_no=int(input("Enter the Adhikaaram number : "))
        assert 0<adhikaaram_no<4 , "adhikaaram_no must be 1, 2 or 3"

        # Iyal        
        #display_iyal(adhikaaram_no)
        #print()
        #iyal_no=int(input("Enter the Iyal number : "))
        assert 0<iyal_no<10 , "Iyal number must be between 1 to 9"
        
        # l => number of songs in an iyal
        self.l=len(tk[adhikaaram_no-1]['iyal'][iyal_no-1]['noorpa'])

        # adhikaaram and iyal number
        ...
        for i in range(len(self.category_list)):
            print(i+1,". ",self.category_list[i])


        # User choose a category
        #print()
        self.category_user = int(input("Enter a category number : "))
        assert 0<self.category_user<len(self.category_list)+1, "Category number Out of Range"
        self.category_name = self.category_list[self.category_user-1]

        # getting all songs in a category and storing in 2 formats.
        # 1. Paadal_dict => {category:[paadals..]}
        # 2. Paadal_data => [paadal_data1, paadal_data2 ...]

    def paadal_dict(self):
        """
        Returns a dict where,
            Key => Name of the Category you selected
            Value => List of the Songs in that category
        """
        self.category_paadal=[]
        for i in range(self.l):
            c=tk[self.adhikaaram_no-1]['iyal'][self.iyal_no-1]['noorpa'][i]['vilakkam']['paadal_category']
            if(c==self.category_name):
                self.category_paadal.append(tk[self.adhikaaram_no-1]['iyal'][self.iyal_no-1]['noorpa'][i]['paadal'])   
        return({self.category_name:self.category_paadal})
    
    def paadal_data(self):
        """
        Returns a list of paadal data of all the paadal under the given category
        This 'list of dict' data can be further used to study the paadal under the given topic
        """
        self.category_paadal_data=[]
        for i in range(self.l):
            c=tk[self.adhikaaram_no-1]['iyal'][self.iyal_no-1]['noorpa'][i]['vilakkam']['paadal_category']
            if(c==self.category_name):
                self.category_paadal_data.append(Paadal(self.adhikaaram_no,self.iyal_no,i+1).paadal_data())
        return(self.category_paadal_data)
