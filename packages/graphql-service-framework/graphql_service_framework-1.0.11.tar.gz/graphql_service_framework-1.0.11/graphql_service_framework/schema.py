...
from typing import TypeVar, Optional, Type, Dict, Callable
...
from graphql_service_framework.middleware import add_mesh_middleware
...
class Schema(GraphQLRootTypeDelegate, metaclass=APIVersion):
    ...
    def create_app(
        ...
        if not config.get("service_name"):
            config["service_name"] = to_snake_case(self.__class__.__name__)

        if not config.get("api_version"):
            config["api_version"] = self.__class__.api_version

        if not config.get("http_health_path"):
            config["http_health_path"] = f"{relative_path}/health"

        health_path = config.get("http_health_path")
        ...
        app = graphql_http_server.app(main=main)

        app = add_mesh_middleware(app, config)
