...
def _upload_and_rewrite_working_dir_in_create_production_job(create_production_job):
    if create_production_job.config is not None:
        ...
        if isinstance(create_production_job.config, dict):
            logger.warn(
                "Please note that the config field should be a CreateProductionJobConfig object "
                "and the dict type will no longer be supported in the future. "
                "Please update to use CreateProductionJobConfig so that your code will continue to work."
            )
...
class AnyscaleSDK(DefaultApi):  # type: ignore
    ...
    def fetch_job_logs(self, job_id: str) -> str:
        ...
        get_job_logs_fns: List[Callable[..., str]] = [
            lambda: asyncio.run(_get_logs_from_running_job(self, job_id)),
