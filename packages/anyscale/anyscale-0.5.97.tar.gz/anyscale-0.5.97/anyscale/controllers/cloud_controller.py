...
from anyscale.utils.imports.gcp import (
    try_import_gcp_client_factory,
    try_import_gcp_discovery,
    try_import_gcp_logger,
    try_import_gcp_utils,
    try_import_gcp_verify_lib,
)


WAIT_FOR_ACTIVE_CLOUD_RETRIES = 60
...
class CloudController(BaseController):
    ...
    def setup_aws_cross_account_role(self, region: str, name: str) -> None:
        ...
        for _ in range(5):
            role_name = f"{ANYSCALE_IAM_ROLE_NAME}-{secrets.token_hex(4)}"

            role: Any = _get_role(role_name, region)
            if role is None:
                break
        else:
            raise RuntimeError(
        ...
        try:
            ...
            self.log.info(f"Create failed, cleaning up IAM role: {role_name}")
            try:
    ...
    def verify_gcp_cloud_resources(
        ...
        if not factory:
            credentials, credentials_project = get_application_default_credentials(
                self.log
            )
        ...
        verify_gcp_access_service_account_result = verify_lib.verify_gcp_access_service_account(
            factory, resources, project_id, gcp_logger
        )
    ...
    def register_gcp_cloud(  # noqa: PLR0913
        ...
        try:
            ...
            filestore_config = get_gcp_filestore_config(
                factory,
                project_id,
                vpc_name,
                filestore_location,
            ...
            with self.log.spinner("Verifying cloud resources..."):
                if not self.verify_gcp_cloud_resources(
                    create_cloud_resource_gcp, project_id, region, factory
                ):
                    raise ClickException(
                        "Cloud registration failed! Please make sure all the resources provided meet the requirements and try again."
                    )
