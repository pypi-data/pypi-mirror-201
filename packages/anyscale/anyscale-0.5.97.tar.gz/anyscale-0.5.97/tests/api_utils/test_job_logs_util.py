...
@pytest.mark.asyncio()
async def test_get_logs_from_running_job():
    ...
    with patch.object(
        ...
        assert await _get_logs_from_running_job(mock_base_api, job_run_id) == mock_logs
...
@pytest.mark.asyncio()
async def test_get_logs_from_running_job_throws_exception_anyscale_api():
    ...
    with pytest.raises(UnsupportedLogRetrievalMethodError, match=error_message):
        await _get_logs_from_running_job(mock_base_api, job_run_id)
...
@pytest.mark.asyncio()
async def test_get_logs_from_running_job_throws_exception_ray_dashboard_api():
    ...
    for exception_type in [ClientConnectionError, asyncio.TimeoutError]:
        with patch.object(
            ...
            UnsupportedLogRetrievalMethodError,
            match="only supported if the Anyscale SDK client can reach the cluster",
        ):
            await _get_logs_from_running_job(mock_base_api, job_run_id)
    ...
    with patch.object(
        ...
        LogNotFoundOnActiveClusterError,
        match='Job "job_123" is not a part of the current active session of the cluster.',
    ):
        await _get_logs_from_running_job(mock_base_api, job_run_id)
...
async def test_get_logs_from_running_production_job():
    ...
    mock_get_job_run_id = Mock(return_value=job_run_id)
    mock_logs = "a\nb"
    mock_get_logs_from_running_job = CoroutineMock(return_value=mock_logs)
    to_patch = {
        _get_job_run_id.__name__: mock_get_job_run_id,
        _get_logs_from_running_job.__name__: mock_get_logs_from_running_job,
    }
    with patch.multiple(job_logs_util, **to_patch):
        assert (
    ...
    mock_get_logs_from_running_job.assert_awaited_once_with(
