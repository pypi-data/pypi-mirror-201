...
def test_generate_config_from_entrypoint(mock_auth_api_client):
    mock_logger = Mock()
    service_controller = ServiceController(log=mock_logger)
    entrypoint = ["python", "test.py"]
    name = "test_name"
    ...
    test_compute_config_id = "mock_compute_config_id"
    test_build_id = "mock_build_id"

    mock_get_default_cluster_compute = Mock(
        return_value=Mock(id=test_compute_config_id)
    )
    mock_validate_successful_build = Mock()
    mock_get_default_cluster_env_build = Mock(return_value=Mock(id=test_build_id))
    with patch.multiple(
        "anyscale.controllers.job_controller",
        ...
        job_config = service_controller.generate_config_from_entrypoint(
...
def test_generate_config_from_file(
    ...
    """
    This test checks that the config file for Service v1 and v2
    ...
    config_dict = {
        ...
        "entrypoint": "python test.py" if runtime_env else None,
        "healthcheck_url": "test.com" if runtime_env else None,
        "runtime_env": runtime_env,
...
def test_generate_config_from_file_with_extra_field(mock_auth_api_client):
    ...
    with patch(
        ...
        service_controller.generate_config_from_file("test_service_config_file",)


@pytest.mark.parametrize(
    ("user_provided_working_dir", "expected_working_dir_arg"),
    [
        (".", "."),
        ("./subdir/subdir2", "./subdir/subdir2"),
        ("/root_dir/subdir1", "/root_dir/subdir1"),
    ],
)
def test_generate_config_from_file_inside_workspace_working_dir_override(
    mock_auth_api_client,
    user_provided_working_dir: Optional[str],
    expected_working_dir_arg: str,
):
    """
    This test checks that if the service v2 is launched from the workspace without
    a working dir being specified, the method would back up the files into S3
    and update the working_dir of the service to point to the S3 location.
    """
    service_controller = ServiceController()

    name = "test_service_name"
    description = "mock_description"
    # Simulates that the user doesn't specify either working_dir or upload_path
    ray_serve_config = {
        "runtime_env": {"pip": ["requests"], "working_dir": user_provided_working_dir}
    }
    build_id = "test_build_id"
    compute_config_id = "test_compute_config_id"
    project_id = "test_project_id"
    ...
    org_id = "test_org_id"
    service_controller.api_client.get_decorated_cluster_api_v2_decorated_sessions_cluster_id_get.return_value = DecoratedsessionResponse(
        result=Mock(cloud=Mock(id=cloud_id))
    )

    s3_bucket_name = "test_s3_bucket_name"
    service_controller.api_client.get_cloud_with_cloud_resource_api_v2_clouds_with_cloud_resource_router_cloud_id_get.return_value = Mock(
        result=Mock(
            provider=CloudProviders.AWS, cloud_resource=Mock(aws_s3_id=s3_bucket_name),
        )
    )
    service_controller.api_client.get_user_info_api_v2_userinfo_get.return_value = Mock(
        result=Mock(organizations=[Mock(id=org_id)])
    )

    workspace_id = "Test_workspace_id"
    expected_working_dir = "s3://bucket"
    ...
    with patch.dict(
        ...
        expected_service_config = ServiceConfig(
            name=name,
            description=description,
            ray_serve_config={"runtime_env": rewritten_runtime_env},
            project_id=project_id,
            build_id=build_id,
            compute_config_id=compute_config_id,
            version="test_abc",
    ...
    assert runtime_env_arg["working_dir"] == expected_working_dir_arg
...
@pytest.mark.parametrize(
    ("working_dir", "upload_path"),
    [
        ("https://github.com/anyscale/docs_examples/archive/refs/heads/main.zip", None),
        ("s3://already-a-uri/path", None),
    ],
)
def test_generate_config_from_file_inside_workspace_no_working_dir_override(
    mock_auth_api_client, working_dir: str, upload_path: Optional[str]
):
    """
...
@pytest.mark.parametrize("override_canary_percent", [None, 0, 40])
