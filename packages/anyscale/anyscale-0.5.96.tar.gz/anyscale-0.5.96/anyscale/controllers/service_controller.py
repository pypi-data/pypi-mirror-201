...
from anyscale.client.openapi_client.models.service_event_current_state import (
    ServiceEventCurrentState,
)
from anyscale.client.openapi_client.models.service_goal_states import ServiceGoalStates
...
class ServiceController(BaseController):
    ...
    def generate_config_from_file(
        ...
        *,
        ...
        self._override_configs_in_workspaces(service_config)

        if service_config.runtime_env:
            # Service v1 specific override since Service v2's runtime_env
            # is under service_config.ray_serve_config["runtime_env"]
    ...
    def _override_configs_in_workspaces(self, service_config: ServiceConfig) -> None:
        """
        We override some service configs if launched in workspaces.

        Please note that this method mutates service_config in-place.
        """
        if (
            is_anyscale_workspace()
            and "ANYSCALE_SESSION_ID" in os.environ
            and service_config.ray_serve_config
        ):
            existing_runtime_env = service_config.ray_serve_config.get(
                "runtime_env", {}
            )

            # If deploying service v2 from workspaces and no upload_path is specified,
            ...
            if not is_dir_remote_uri(working_dir) and upload_path is None:
    ...
    def deploy_from_config(self, service_config: ServiceConfig):
        ...
        current_state = self._format_service_state(service.state.current_state)
    ...
    def rollout(
        ...
        current_state = self._format_service_state(service.goal_state)
        self.log.info(
        ...
        if service_id:
            if _is_v2_service_id(service_id):
                ...
                services.append(
                    [
                        ...
                        self._format_service_state(service_v2_result.current_state),
            ...
            services = [
                [
                    service.name,
                    service.id,
                    self._format_service_state(service.current_state),
    ...
    def _format_service_state(
        self, state: Union[ServiceEventCurrentState, HaJobStates, ServiceGoalStates]
    ):
        if state == "RUNNING":
            return "ACTIVE"
        return state
