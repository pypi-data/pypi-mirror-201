...
from geodesic.stac import page_through_results
...
class DatasetExplorerWidget(ipywidgets.VBox):
    """Query and Warp data from all the datasets that this user has access to.

    The Dataset Explorer is intended to be a simple way to browse some of the datasets in Entanglement using
    a user interface.

    For privacy purposes, a user can choose to only show a subset of projects

    Public Attributes:
        warp_results: a `numpy.ndarray` of the result of running a warp request
        query_results: a `geodesic.FeatureCollection`
        dataset: the selected dataset
        selected_project: the selected project
        warp_request: the currently active warp request
        query_request: the currently active query request

    Args:
        figsize: the size of the figure in the raster pane. It's difficult to make this dynamic, so it's set to a
                sensible default.
        show_projects_matching: a list of project names or regex patterns to show in the projects dropdown. If `None`,
                will show all.
    """

    def __init__(self, figsize=(8, 5), show_projects_matching=None):
        self.geometry = None
        self.warp_results = None
        ...
        self._setup_query_vbox()
        self._setup_warp_vbox()
    ...
    def _setup_figure(self, figsize):
        ...
        self.figure.suptitle("Warp Output", fontsize=16)
    ...
    def _setup_query_vbox(self):
        self.query_submit_button = ipywidgets.Button(
            description="submit", layout=ipywidgets.Layout(width="60px", margin="12px")
        )
        self.query_submit_button.on_click(self._on_query_submit)
        ...
        self.query_vbox = ipywidgets.VBox(
            [
                self.limit,
                self.page_size,
                self.collections,
                self.color,
                self.query_submit_button,
            ]
        )

    def _setup_warp_vbox(self):
        self.warp_submit_button = ipywidgets.Button(
            description="submit", layout=ipywidgets.Layout(width="60px", margin="12px")
        )
        self.warp_submit_button.on_click(self._on_warp_submit)
        ...
        self.resampling = ipywidgets.Dropdown(
            options=geodesic.entanglement.dataset._valid_resampling,
            value="nearest",
        ...
        self.warp_vbox = ipywidgets.VBox(
            [
                ...
                self.warp_submit_button,
            ],
        )

    def _setup_dataset_operations_accordion(self):
        self.dataset_operations_accordian = ipywidgets.Accordion(
            [self.query_vbox, self.warp_vbox],
            layout=ipywidgets.Layout(margin="10px", width="95%"),
        )
        self.dataset_operations_accordian.set_title(0, "query")
        self.dataset_operations_accordian.set_title(1, "warp")
    ...
    def _on_query_submit(self, event):
        limit = self.limit.value
        req = self.query_request
        fc = self.dataset.query(**req)

        fc = page_through_results(fc, limit=limit)
    ...
    def _on_warp_submit(self, event):
        warp_request = self.warp_request
        if warp_request is None:
            print("invalid warp request options")
            return

        bands = []
        for asset_bands in warp_request["asset_bands"]:
            asset = asset_bands["asset"]
            bands.extend([f"{asset}:{band}" for band in asset_bands["bands"]])
        self.rendering_bar.set_bands_list(bands)

        self.warp_results = self.dataset.warp(**warp_request)
        self._redraw_imshow(None)
        self.rhs.selected_index = 1

    def _redraw_imshow(self, _):
        if self.warp_results is None:
            return
        ...
        if renderer_props["renderer"] == "colormap":
            ...
            self.ax.imshow(self.warp_results[band_idx], **renderer_props["imshow_kwargs"])
        else:
            rgb = self.warp_results[renderer_props["bands"]].astype(np.float32)
            # Normalize
