...
from grocropclient.constants import FrequencyList, OtherItemsList, SourceList, MetricList, CropList, GRO_YIELD_MODELS, AreaWeightingSeriesList, ItemNameCategoryList, CROP_BUDGET_REGIONS, NATIONAL_AND_STATE_CROP_BUDGET_REGIONS, US_States, US_STATES_SET, US_COUNTIES_SET, US_COUNTIES_FIPS_TO_REGION_ID, US_COUNTIES_REGION_ID_TO_FIPS
...
def is_valid_area_weighting_region(region_id: int) -> bool:
    ...
    if region_id == US_States.UNITED_STATES or region_id in US_STATES_SET or region_id in US_COUNTIES_SET:
        return True
    return False
    ...
    def get_crop_budget_yield_model(self, item_name: str, region_name: str, source_name: str, latest_date_only=False) -> pd.DataFrame:
        ...
        if crop == ItemNameCategoryList.CORN.value:
            if region_name in NATIONAL_AND_STATE_CROP_BUDGET_REGIONS: 
                return self.get_yield_model_df(region_name, CropList.CORN.value, 284, latest_date_only)
            else:
                yield_dict = self._client.get_area_weighted_series(AreaWeightingSeriesList.CORN_YIELD.value, ['NASS Corn (US only)'], region_id, latest_date_only=latest_date_only)
        elif crop == ItemNameCategoryList.SOYBEANS.value:
            if region_name in NATIONAL_AND_STATE_CROP_BUDGET_REGIONS:
                return self.get_yield_model_df(region_name, CropList.SOYBEANS.value, 287, latest_date_only)
            else:
                yield_dict = self._client.get_area_weighted_series(AreaWeightingSeriesList.SOYBEANS_YIELD.value, ['NASS Soybeans (US only)'], region_id, latest_date_only=latest_date_only)
 
        return pd.DataFrame({'Yield': yield_dict.values()}, yield_dict.keys())
    

    def get_crop_budget_price(self, item_name: str, region_name: str, source_name: str, latest_date_only=False) -> pd.DataFrame:
        """
        ...
        crop = is_corn_or_soybeans(item_name)
        if crop == ItemNameCategoryList.CORN.value:
            price_dict = self._client.get_area_weighted_series(AreaWeightingSeriesList.CORN_PRICE.value, ['AVERAGE'], region_id,latest_date_only=latest_date_only)
        elif crop == ItemNameCategoryList.SOYBEANS.value:
            price_dict = self._client.get_area_weighted_series(AreaWeightingSeriesList.SOYBEANS_PRICE.value, ['AVERAGE'], region_id, latest_date_only=latest_date_only)
...
######################################
    ...
    def get_yield_model_data(self, yield_model_id: str, start_date: str = None, end_date: str = None):
        ...
        if yield_model_id not in GRO_YIELD_MODELS:
            ...
            params = { "show_revisions": True, "show_metadata": True,
                       "source_id": SourceList.GRO_YIELD_MODEL.value, "frequency_id": 9,
    ...
    def get_yield_model_df(self, region_name: str, item_id: int, unit_id: int, latest_date_only: bool) -> pd.DataFrame:
        """
        ...
        dates = [data_point['reporting_date'].split('T')[0] for data_point in data_points]
    ...
    def get_dtn_fertilizer_prices(self, fertilizer_id: int, region_id: int, start_date: str = None, end_date: str = None):
    ...
    def get_Gro_Drought_Index(self, region_id: int, frequency: int = FrequencyList.DAILY.value, start_date: str = None, end_date: str = None):
    ...
    def get_Gro_Observed_Flood(self, region_id: int, start_date: str = None, end_date: str = None):
        ...
        params = { "metric_id": MetricList.OBSERVED_FLOOD, "item_id": OtherItemsList.WATER_AREAS,
                   "frequency_id": FrequencyList.DAILY.value, "source_id": SourceList.GRO_DERIVED_GEOSPATIAL,
                   "region_id": region_id }
        if start_date:
            params["start_date"] = start_date
        if end_date:
    ...
    def get_MODIS_NDVI(self, region_id: int, frequency: int = FrequencyList.EIGHT_DAY.value, start_date: str = None, end_date: str = None):
        params = { "metric_id": MetricList.VEGETATION, "item_id": OtherItemsList.VEGETATION_NDVI,
                   "frequency_id": frequency, "source_id": SourceList.GIMMS_MODIS,
                   "region_id": region_id }
        if start_date:
            params["start_date"] = start_date
        if end_date:
    ...
    def get_MODIS_LST(self, region_id: int, frequency: int = FrequencyList.DAILY.value, start_date: str = None, end_date: str = None):
        params = { "metric_id": MetricList.TEMPERATURE, "item_id": OtherItemsList.LAND_TEMPERATURE,
                   "frequency_id": frequency, "source_id": SourceList.MODIS_TERRA,
                   "region_id": region_id }
        if start_date:
            params["start_date"] = start_date
        if end_date:
    ...
    def get_ESA_SMOS(self, region_id: int, frequency: int = FrequencyList.DAILY.value, start_date: str = None, end_date: str = None):
        params = { "metric_id": MetricList.AVAILABILITY_IN_SOIL, "item_id": OtherItemsList.SOIL_MOISTURE,
                   "frequency_id": frequency, "source_id": SourceList.ESA_SMOS,
                   "region_id": region_id }
        if start_date:
            params["start_date"] = start_date
        if end_date:
    ...
    def get_GFS_Tmax(self, region_id: int, start_date: str = None, end_date: str = None):
        params = { "metric_id": MetricList.TEMPERATURE, "item_id": OtherItemsList.TEMPERATURE_MAX,
                   "frequency_id": FrequencyList.DAILY.value, "source_id": SourceList.GFS_00Z,
    ...
    def get_GFS_Tmin(self, region_id: int, start_date: str = None, end_date: str = None):
    ...
    def get_GFS_Precip(self, region_id: int, start_date: str = None, end_date: str = None):
    ...
    def get_area_weighted_series(self, series_name, weight_names, region_id, method='sum', latest_date_only=False) -> dict:
        """
        ...
        for r in region_id:
            if not is_valid_area_weighting_region(r):
                raise Exception(f"{r} is not the United States, a US state, or a US county")

        # All regions have been checked, do the weighted average
        return self._client.get_area_weighted_series(series_name = series_name,
    ...
    def convert_fips_codes_to_region_ids(self, fips_codes: List[str]) -> List[int]:
        ...
        for f in fips_codes:
            assert re.match("^[0-9]+$", f), f"FIPS codes must be passed as strings containing only the numeric FIPS code, but {f} was passed"
    ...
    def convert_region_ids_to_fips_codes(self, region_ids: List[int]) -> List[str]:
        ...
        for r in region_ids:
            assert type(r)==int, f"region_ids must be passed as integers, but {r} was passed with type {type(r)}"
