...
class RumorView(object):
    ...
    def show_columns(
        ...
        #TODO: remove nodes far from target
    ...
    def show_relations(self, c1, c2, min_lift=1.5, show_lift=True, show_df=True, **nx_options):
        plot_df = self.lift_df if show_lift else self.cprob_df
        nx_args = _overwrite_dict(NX_OPTIONS_DEFAULT, nx_options)
        ...
        for c1_node in c1_nodes:
            for c2_node in c2_nodes:
                if self.lift_df.loc[c2_node, c1_node] > min_lift:
                    ...
                    g.add_edge(
                        c1_node, c2_node, title=title, value=plot_df.loc[c2_node, c1_node]
                    )
