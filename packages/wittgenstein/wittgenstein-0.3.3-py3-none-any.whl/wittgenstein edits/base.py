...
from wittgenstein.utils import rnd, weighted_avg_freqs, min_max_smooth
        ...
        for i, (p, cr) in enumerate(zip(predictions, covering_rules)):
            if not p:
                ...
                if valid_class_freqs:
                    ...
                    probas[i, :] = 0 # change this to a prior?
                    invalid_example_idx.append(i)
                    for r in cr:
                        invalid_rules[r].add(i)
                    #warning_str = f"rule {cr}: Some examples lacked any rule with sufficient sample size to predict_proba: {i}\n Consider running recalibrate_proba with smaller param min_samples, or set require_min_samples=False"
                    #_warn(warning_str, RuntimeWarning, filename="base", funcname="predict_proba")
        """
