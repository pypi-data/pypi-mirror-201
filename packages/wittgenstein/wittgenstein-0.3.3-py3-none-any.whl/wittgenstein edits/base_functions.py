...
def recalibrate_proba(
    ...
    for i, rule in enumerate(ruleset.rules):
        n_samples = len(df)
        p = num_pos(df, class_feat=class_feat, pos_class=pos_class)
        tp = num_pos(rule.covers(df), class_feat=class_feat, pos_class=pos_class)
        fp = p - tp
        tpr = tp / p if p else 0
        fpr = fp / p if p else 0
        rule.class_freqs = (tpr, fpr)
        rule.class_ns = (tp, fp)
        print('rule', rule)
        ...
        if (
            ...
            or p < 1
            or tp < required_correct_samples
        ):
            insufficient_rules.append(rule)

            #print(rule)
            #print(df.dtypes)
            #print(rule_class_ns[i], rule_class_freqs[i])
            #assert(False)

    # Collect class frequencies for negative predictions
    uncovered = df.drop(ruleset.covers(df).index)
    n = num_neg(df, class_feat=class_feat, pos_class=pos_class)
    ...
    print('rule_ct', len(ruleset.rules))
    ...
    if insufficient_rules:  # WARN if any rules lack enough samples
        ...
        warning_str = f"param min_samples={min_samples}; insufficient number of samples or fewer than {required_correct_samples} true positives for rules {[f'{r} class counts: {sum(r.class_ns)}' for r in pretty_insufficient_rules]}"
        _warn(
            warning_str,
            RuntimeWarning,
        ...
        sample_failure = True
    if n < min_samples:  # WARN if negative class lacks enough samples
    ...
    #for rule, freqs, ns in zip(ruleset.rules, rule_class_freqs, rule_class_ns):
    ...
    #    fn >= min_samples and fn >= required_correct_samples
    #):
    #    ruleset.uncovered_class_freqs = (tn, fn)
