...
def recalibrate_proba(
    ruleset, Xy_df, class_feat, pos_class, min_samples=10, require_min_samples=True, verbosity=0,
):
    ...
    if insufficient_rules:  # WARN if any rules lack enough samples
        pretty_insufficient_rules = "\n".join([f'{r} class counts {sum(r.class_ns)}' for r in insufficient_rules])
