...
Module capable of detecting inconsistencies in the inputs data. The main 
functions xlsx_trap() and sarasa() placed at end
...
def miss_column(data_df):
    ...
    AGREGAR DOCUMENTACION
    '''    
    if not all(col in list(data_df.columns) for col in ['nuclei','sp2',
                                             'exp_data','exchange',
...
def miss_data(data_df, isom ):
    ...
    #check labels
    data_df = data_df.drop(['nuclei','sp2','exp_data','exchange'],axis=1)
    try : 
        np.array(data_df, dtype= int)
    except: 
        print ('ACA5')
        error_found('Not interger number in "label" column')
...
def xlsx_trap(inputs, sheet):
    ...
    data = pd.read_excel(inputs['xlsx'], sheet_name=sheet ,
                         engine= eng, index_col=0)
...
def check_NormalTermination():
    'Identifica calculos en Error y mueve el .log y el. gjc a la carpeta "Termino Mal"'
    removed_files = False
