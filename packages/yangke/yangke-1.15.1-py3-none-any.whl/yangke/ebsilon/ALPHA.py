...
from yangke.common.QtImporter import QModelIndex, QMimeData
from PyQt5.QtGui import QDrag
from PyQt5.QtWidgets import QComboBox, QApplication, QLineEdit
...
class MainWindow(YkWindow, QMainWindow):
    ...
    def keyPressEvent(self, a0: QKeyEvent) -> None:
        if a0.key() == Qt.Key_F9:
            self.calculate()
        elif a0.modifiers() == Qt.ShiftModifier | Qt.ControlModifier and a0.key() == Qt.Key_AsciiTilde:
    ...
    def update_scene_nav_view(self):
        """
        场景导航面板

        :return:
        """
        scene_list_model = QStandardItemModel()  # 场景组件列表模型
        for scene_id, 设备组件 in self.content_widget.scene.items_scene_id.items():
            设备组件: YkGraphicsItem = 设备组件
            if scene_id < 0:
                continue
            ...
            scene_list_model.appendRow(item)
        # self.ui.lv_com.setItemDelegate(YkStyledItemDelegate())
        self.ui.lv_scene.setSpacing(4)

        self.ui.lv_scene.setModel(scene_list_model)
        ...
        try:
            for scene_id, obj in items.items():
                ...
                objs_pickable = {"EBS_ID": obj.EBS_ID, "EBS_NAME": obj.EBS_NAME, "id": obj.id, "scene_id": obj.scene_id,
                                 ...
                                 "ports": obj.ports, "bak_info": bak_info,
                                 }
    ...
    def display_info(self, scene_id, tab=None, save_data=True):
        ...
        # 更新场景组件导航面板上的组件列表
        self.update_scene_nav_view()
        idx = list(self.content_widget.scene.items_scene_id.keys()).index(scene_id) - 1  # 因为场景中有一个组件不显示
