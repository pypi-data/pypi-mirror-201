...
def start_server_app(deal=None, app_name='yangke.web.flaskserver', instance_path=__file__,
                     ...
                     run_immediate=True, deal_request=False):
    """
    ...
    start_server_app(deal, deal_request=True)  # 启动服务器，且给deal函数传递request对象
    #==============================================================================================

    :param use_action: 是否使用Action参数指定操作类型，不使用则完全由用户定义所有url响应
    :param deal: 用户指定的url处理方法，deal的参数是且必须是http/https请求的参数
    ...
    :param deal_request: 是否直接将request对象传递给deal函数，默认为False，deal函数接收到的是提取的参数字典，如果为True，则传递给deal函数的是request的原始对象
    :return:
    """
