...
from PyQt5.QtWidgets import QGraphicsSceneDragDropEvent, QGraphicsTextItem

from yangke.common.config import logger
from yangke.common.qt import YkWindow, YkFoldableWidget, YkItem, distance
from yangke.ebsilon.constant.constant import 管线图层id, Pressure, Temperature, Enthalpy, MassFlow, Power, Unit
...
class YkGraphicsScene(QGraphicsScene):
    ...
    def mousePressEvent(self, event: QGraphicsSceneMouseEvent) -> None:
        ...
        if event.button() == Qt.LeftButton:
            if isinstance(item_, SceneGridItem) or isinstance(item_, CoordinateItem):  # 点击的位置没有其他组件
                if self.state == "常规":
                    ...
                    if idx != 0:  # 如果组件列表中选中任何组件，则添加组件到场景中
                        ...
                        # 需要单独处理管线点击事件，因为有些管线被上层管线覆盖，Qt默认是选不中下层管线的，需要处理

                        # super().mousePressEvent(event)  # 如果点击到管线上，会首先触发最上层管线的mousePressEvent事件
                        # res: YkGraphicsItem = self.flag_item.selected_item
                        # if isinstance(res, PipeItem):
                        # 说明点击到了管线的boundingBox内，但没有点击到线段上，则会返回该管线的实例
                        # pipe: PipeItem = self.pipe_item_at(event.scenePos(), res)
                        pipe: PipeItem = self.pipe_item_at(event.scenePos())
                        if pipe is not None:
                            pipe.mousePressEvent(event)
                        else:
                            self.clearFocus()
                            self.clearSelection()
            ...
            elif not isinstance(item_, YkGraphicsItem):
                pass
                ...
                # 前两种情况
                if connect_port_id != 0:  # 如果确实点击到了图元上的端口，则可能是1即将绘制管线，2连接正在绘制的管线这两种情况
                    port: Port = item_.ports[connect_port_id]
                    if self.state == "常规":  # 如果当前场景状态并不是绘制管道，则以点击的端口开始绘制管道
                        if port.type1 in ["Inlet", "Outlet", "Signal Inlet", "Signal Outlet"]:  # 只响应特定类型的端口
                            if not port.is_connect:
                                ...
                                self.last_line.connect_items.append(ConnectItem(item_.scene_id, connect_port_id, 0))
                        ...
                        elif port.type1 == "Signal Inlet" or port.type1 == "Signal Outlet":
                            if not port.is_connect:
                                self.views()[0].viewport().setCursor(Qt.CrossCursor)  # 将场景视图的鼠标设置为十字星
                                self.state = "绘制管道"  # 设置场景状态为正在画线
                                self.last_line = PipeItem(fluid_type="Signal", color=Qt.black)
                ...
                else:  # 只是点击到图元上，但未点击到端口上，一般来说，这是要3拖动组件，4显示组件信息，这两种情况交给图元的鼠标点击事件处理
                    # 无须任何操作
                    ...
        elif event.button() == Qt.RightButton:
    ...
    def mouseMoveEvent(self, event: QGraphicsSceneMouseEvent) -> None:
        ...
        mw.setText(f"{int(event.scenePos().x())},{int(event.scenePos().y())}")
...
class PipeItem(YkGraphicsItem):
    ...
    def get_connect_point_ids(self, need_port_type=False):
        ...
        :param need_port_type: 是否按端口类型返回列表，如果为True，则返回三个列表，分别对应Inlet、Outlet和Set Value三类端口
        :return:
        """
        if need_port_type:
            input_ids = []
    ...
    def mouseMoveEvent(self, event: QGraphicsSceneMouseEvent) -> None:
        ...
        try:
            if not hasattr(self, "moving_point"):  # 如果没有moving_point属性，则说明不是移动管线操作
                ...
            elif self.moving_point >= 0:  # 说明在移动管线上的某个点
