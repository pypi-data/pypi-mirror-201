...
def get_size_of_window(hwnd, client=False):
    """
    获取窗口大小
...
def capture_pic(hwnd, x1=0, y1=0, x2=0, y2=0, save_to: str = "memory", show=False):
    ...
    :param x2: 截图区域的右下角x坐标
    :param y2: 截图区域的右下角y坐标
    :param save_to: 保存截图到文件时，通过该参数指定文件名，不指定则为内存中保存的图片，否则，返回图片的ndarray数组
    :return:
    """
    import win32ui
    left, top, right, bot = win32gui.GetWindowRect(hwnd)
...
def find_pic(template: str, screen: str = "temp.png", threshold: float = 0.8):
    ...
    :param screen: 大图片，即在该图片中查找
    :param template: 小图片，即在大图片中查找该图片的相对位置
    :param threshold: 阈值，越大则要求越相似
    :return: exists, (x,y)
    """
...
def post_key(hwnd, key, shift=None, special_key=False, last_time=0.01):
    """
    向指定窗口发送按键事件，记事本已测试成功，逆水寒测试成功。但部分按键的功能与实际按键功能不一致，原因未知。
    ...
    post_key(hwnd, 'A', [win32con.VK_CONTROL], False)  # 全选记事本内容，相当于发送Ctrl+A按键事件
    post_key(hwnd, 'S', [win32con.VK_MENU], False)  # 打开记事本设置面板，相当于发送Alt+S按键事件
    post_key(hwnd, ord('A'), None, False)  # 在编辑区输入字母a，相当于在编辑区按“A"键
    post_key(hwnd, 'escape', None, False)  # 按Esc键
    ...
    if win32gui.IsWindow(hwnd):
        ThreadId = _user32.GetWindowThreadProcessId(hwnd, None)  # win32process.GetWindowThreadProcessId给出了错误的值
        ...
        _user32.AttachThreadInput(win32api.GetCurrentThreadId(), ThreadId, True)  # 附加python线程至目标窗口
        ...
        if shift is not None and len(shift) > 0:
            ...
            time.sleep(0.01)
        _user32.AttachThreadInput(win32api.GetCurrentThreadId(), ThreadId, False)  # 从目标线程上分离python线程
