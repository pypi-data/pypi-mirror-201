...
def start_crawl(start_paper_index=1,end_paper_index=None,mode='structure',save_path='data',log=True,wait_time=120,browser_type='Chrome'):
    ...
    if not os.path.exists(save_path):
        os.mkdir(save_path)
        logger.info('已在当前目录创建data文件夹')
    else:

        if os.listdir(save_path) and start_paper_index==1:
            cover = input(f'{save_path}文件夹中已存在文件, 覆盖请输入y, 不覆盖请输入n, 程序将退出, 请输入: ')
            if cover == 'y':
