...
class KubernetesJobOperator(KubernetesJobOperatorDefaultsBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.image = image
        logging.INFO("测试env: " + envs.__str__())
        self.envs = envs
        ...
        # 添加callable
        assert python_callable is None or isinstance(python_callable, callable), ValueError(
            "python_callable must be a callable or None")
    ...
    def execute(self, context: Context) -> Any:
        logging.info("测试context1: " + context.__str__())
        context_merge(context, self.op_kwargs, templates_dict=self.templates_dict)
        logging.info("测试context2: " + context.__str__())
        return_value = self.python_callable(context)
