...
class SubstrateListener(object):
    ...
    def __sync_writer_ssh_pub_keys(self):
        ...
        for writer_ssh_key in writer_ssh_keys_from_db:
            logging.info(
                f"from db          : writer_ssh_key: {writer_ssh_key}")
        # get the writer_ssh_keys from the file system
        writer_ssh_keys_from_file_system = self.__writer_ssh_keys_from_file_system()
        for writer_ssh_key in writer_ssh_keys_from_file_system:
            logging.info(
                f"from file system : writer_ssh_key: {writer_ssh_key}")
