...
class TinyB(object):
    ...
    async def _req(self, endpoint: str, data=None, files=None, method: str = 'GET', retries: int = 1, **kwargs):  # noqa: C901
        ...
        try:
            with requests.Session() as session:
                ...
                session.mount('https://', requests.adapters.HTTPAdapter(max_retries=retry))  # type: ignore
                session.mount('http://', requests.adapters.HTTPAdapter(max_retries=retry))  # type: ignore
                if method == 'POST':
                    if files:
                        response = await sync_to_async(session.post, thread_sensitive=False)(url, files=files, verify=verify_ssl, **kwargs)  # type: ignore
                    else:
                        response = await sync_to_async(session.post, thread_sensitive=False)(url, data=data, verify=verify_ssl, **kwargs)  # type: ignore
                elif method == 'PUT':
                    response = await sync_to_async(session.put, thread_sensitive=False)(url, data=data, verify=verify_ssl, **kwargs)  # type: ignore
                elif method == 'DELETE':
                    response = await sync_to_async(session.delete, thread_sensitive=False)(url, data=data, verify=verify_ssl, **kwargs)  # type: ignore
                else:
                    response = await sync_to_async(session.get, thread_sensitive=False)(url, verify=verify_ssl, **kwargs)  # type: ignore
    ...
    async def connections(self, connector: Optional[str] = None):
        ...
        bigquery_connection = await self.bigquery_connection()
    ...
    async def datasource_append_data(self, datasource_name: str, file: Path, mode: str = 'append', status_callback=None, sql_condition: Optional[str] = None, format: str = 'csv', replace_options: Optional[Set[str]] = None):
        params = {
    ...
    async def query(self, sql: str, pipeline: str = None):
        params = {}
        if pipeline:
            params = {
                'pipeline': pipeline
    ...
    async def merge_workspace_branch(self, branch_id: str):
        return await self._req(f"/v0/branches/{branch_id}/deployment", method='POST', data=b'')
    ...
    async def branch_regression_tests(self, branch_id: str, pipes: Optional[List[str]], test_type: str, only_response_times: bool,
                                      failfast: bool, ignore_order: bool, validate_processed_bytes: bool,
                                      limit: Optional[int] = None, samples_by_params: Optional[int] = None, matches: Optional[List[str]] = None,
                                      params: Optional[List[Dict[str, Any]]] = None):
        regression_commands: List[Dict[str, Any]] = []
        test: Dict[str, Any] = {test_type: {'config': {'assert_result_ignore_order': ignore_order,
                                                       'failfast': failfast,
                                                       'assert_time_increase_percentage': 25 if only_response_times else -1,
                                                       'assert_bytes_read_increase_percentage': 25 if validate_processed_bytes else -1}}}
        if limit is not None:
            test[test_type].update({'limit': limit})
        if samples_by_params is not None:
            test[test_type].update({'samples_by_params': samples_by_params})
        if matches is not None:
            test[test_type].update({'matches': matches})
        ...
        if not pipes:
            pipes = ["*"]
        for pipe in pipes:
            regression_commands.append({'pipe': "*" if pipe is None else pipe,
                                        'tests': [test]})
        data = json.dumps(regression_commands)
        url = f"/v0/branches/{branch_id}/regression"
        return await self._req(url, method='POST', data=data, headers={'Content-Type': 'application/json'})

    async def branch_regression_tests_file(self, branch_id: str, regression_commands: List[Dict[str, Any]]):
        data = json.dumps(regression_commands)
        url = f"/v0/branches/{branch_id}/regression"
        return await self._req(url, method='POST', data=data, headers={'Content-Type': 'application/json'})
    ...
    async def connection_create_kafka(
        ...
        return await self._req("/v0/connectors", method='POST', headers={'Content-Type': 'application/json'}, data=json.dumps(connection_params))

    async def kafka_list_topics(self, connection_id: str, timeout=5):
        resp = await self._req(f"/v0/connectors/{connection_id}/preview?preview_activity=false",
    ...
    async def datasource_query_copy(self, datasource_name: str, sql_query: str):
        return await self._req(f'/v0/sql_copy?copy_to={datasource_name}',
                               data=sql_query,
                               method='POST')
