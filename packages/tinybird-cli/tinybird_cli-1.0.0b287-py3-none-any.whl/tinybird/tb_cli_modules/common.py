...
async def print_branch_regression_tests_summary(client, job_id, host, response=None):
    ...
    for step in response['progress']:
        run = step['run']
        click.echo(FeedbackManager.success_test_endpoint_no_token(host=host, pipe=run['pipe_name']))

        click.echo(''.join(run['output']))

        if not run.get('metrics_summary') or not run.get('metrics_timing'):
            click.echo("No requests to run regression")
        else:
            column_names = ['Timing Metric (s)', 'Main', 'Branch']

            click.echo('\n==== Test Metrics ====\n')
            click.echo(format_pretty_table([[run['metrics_summary']['run'],
                                             run['metrics_summary']['passed'],
                                             run['metrics_summary']['failed'],
                                             run['metrics_summary']['percentage_passed'],
                                             run['metrics_summary']['percentage_failed']]],
                                           column_names=['Test Run', 'Test Passed', 'Test Failed', '% Test Passed',
                                                         '% Test Failed']))

            click.echo('\n==== Response Time Metrics ====\n')
            click.echo(format_pretty_table(
                [[metric, run['metrics_timing'][metric][0], run['metrics_timing'][metric][1]] for
        ...
        if not run['was_successfull']:
            ...
            for failure in run['failed']:
                try:
                    ...
                    pass
    if failed:
        raise click.ClickException("Failed regression tests")


async def merge_workspace_branch(ctx: Context, branch_id: str, current_main_workspace: Any, wait: Optional[bool], verbose: Optional[bool]):
    """
    ...
    try:
        config, _, _ = await get_config_and_hosts(ctx)
        response = await client.merge_workspace_branch(branch_id)  # type: ignore
        ...
        if wait:
            ...
            await wait_job(client, job_id, job_url, 'Branch Deployment', wait_observer=partial(merge_branch_status.print_merge_branch_summary))
    except Exception as e:
        click.echo(FeedbackManager.error_exception(error=str(e)))
