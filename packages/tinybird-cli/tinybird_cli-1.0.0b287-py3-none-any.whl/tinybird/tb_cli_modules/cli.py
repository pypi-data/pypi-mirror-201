...
from tinybird.datafile import AlreadyExistsException, Datafile, ParseException, build_graph, folder_push, folder_pull, format_datasource, format_pipe, get_name_tag_version, get_project_filenames, parse_datasource, parse_pipe, wait_job, get_resource_versions, diff_command
import tinybird.context as context
from tinybird.config import FeatureFlags, get_config, VERSION, CURRENT_VERSION, SUPPORTED_CONNECTORS

from tinybird.tb_cli_modules.common import CatchAuthExceptions, coro, create_tb_client, folder_init, \
    getenv_bool, load_connector_config, get_config_and_hosts, \
    echo_safe_humanfriendly_tables_format_smart_table, get_current_main_workspace
...
@click.option('--only-changes', is_flag=True, default=False, help="Push only changes from branch to main", hidden=True)
...
@click.option('--check-requests-from-branch', is_flag=True, default=False, help="When set, the checker will get branch requests", hidden=True)
...
async def push(
    ...
    check_requests_from_branch: bool,
    folder: str,
    ...
    await folder_push(
        ...
        tests_check_requests_from_branch=check_requests_from_branch,
        folder=folder,
...
async def diff(
    ...
    if not main:
        changed = await diff_command(filename, fmt, client, no_color, with_print=not only_resources_changed)
    else:
        ...
        if not ws_client:
            click.echo(FeedbackManager.error_workspace(workspace=origin))
            return
        changed = await diff_command(filename, fmt, ws_client, no_color, with_print=not only_resources_changed)
    ...
    if only_resources_changed:
        ...
        for resource, status in dict(sorted(changed.items(), key=lambda item: str(item[1]))).items():
            if status is None:
                continue
            status = 'changed' if status not in ['remote', 'local'] else status
