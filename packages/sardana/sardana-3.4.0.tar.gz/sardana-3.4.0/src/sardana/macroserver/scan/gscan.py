...
class CScan(GScan):
    ...
    def get_velocity_range(self, motor):
    ...
    def _go_through_waypoints(self):
        ...
        for i, waypoint in waypoints:
            ...
            if state in (Alarm, Fault):
                msg = "Moving to start position ended with {} state ".format(
                    state.name.capitalize()
                )
                msg += "on position(s):"
                for name, pos in zip(motion.names, _positions):
                    msg += " {}={}".format(name, pos)
            if state == Alarm:
