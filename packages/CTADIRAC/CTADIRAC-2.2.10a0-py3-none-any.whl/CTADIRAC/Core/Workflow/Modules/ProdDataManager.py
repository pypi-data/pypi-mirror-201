...
class ProdDataManager:
    ...
    def createMDStructure(self, metadata, basepath, program_category, output_type):
        ...
        # if not res["OK"]:
        # return res
        # Set MD for the sub directory - data_level, configuration_id, merged
        # metadataSub = {}
        if output_type == "Data" or "Log":
            for metadataKey in [
                "outputType",
                "data_level",
                "configuration_id",
                ...
                if metadataKey not in metadataSubDir:
                    metadataSubDir[metadataKey] = md[metadataKey]
            res = self.fcc.setMetadata(path, metadataSubDir)
        elif output_type == "Model":
            for metadataKey in ["outputType", "data_level", "configuration_id"]:
                if metadataKey not in metadataSubDir:
                    metadataSubDir[metadataKey] = md[metadataKey]
            res = self.fcc.setMetadata(path, metadataSubDir)
