...
class Label(Boite):
    ...
    def __init__(self, parent, rect: Union[Tuple[int, int, int, int],Tuple[int, int, int],Tuple[int, int], pygame.Rect], texte: str, angle=0, **kwargs):
        if len(rect) == 2:
            rect = (rect[0], rect[1], 0, 0)
        elif len(rect) == 3:
            rect = (rect[0], rect[1], rect[2], 0)
...
class Dialog(Boite):
    ...
    def repack(self):
        """réordonne les objets d'une boite de dialoque
    ...
    def scan_mouse(self):
        """gère les évènements souris de la boit de dialogue ou de l'ihm """
        x, y = processing.mouseXY()
        click = processing.mouse_click()
