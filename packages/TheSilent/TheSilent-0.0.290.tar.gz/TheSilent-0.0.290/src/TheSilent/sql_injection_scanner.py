    ...
    for j in my_result:
        for c in mal_sql:
            ...
            try:
                ...
                if tor:
                    result = web_session.get(new_url, verify = False, headers = {"User-Agent":return_user_agent()}, proxies = tor_proxy, timeout = (5, 30))

                else:
                    result = web_session.get(new_url, verify = False, headers = {"User-Agent":return_user_agent()}, timeout = (5, 30))
                ...
                if result.status_code == 403:
                    ...
                    if tor:
                        result = web_session.get(new_url, verify = False, headers = {"User-Agent":return_user_agent()}, proxies = tor_proxy, timeout = (5, 30)).text
                ...
                print(red + "ERROR! Connection Error! We may be being IP banned! Waiting one minute before retrying!")
                time.sleep(60)
            ...
            try:
                ...
                if tor:
                    result = web_session.get(j, verify = False, headers = {"User-Agent":return_user_agent()}, cookies = mal_cookie, proxies = tor_proxy, timeout = (5, 30))

                else:
                    result = web_session.get(j, verify = False, headers = {"User-Agent":return_user_agent()}, cookies = mal_cookie, timeout = (5, 30))
                ...
                for i in form_input:
                    for ii in mal_sql:
                        ...
                        if name[0] in activity:
                            ...
                            if action[0] != "":
                                if j.endswith("/") and action[0] != links + "/" or j.endswith("/") and action[0] != links:
                                    my_link = links + action[0]

                                elif action[0] == j + "/" or action[0] == j:
                                    my_link = j
                            ...
                            if tor == True:
                                if method[0].lower() == "get":
                                    get = web_session.get(my_link, params = mal_dict, verify = False, headers = {"User-Agent":return_user_agent()}, proxies = tor_proxy, timeout = (5, 30))
                                
                                if method[0].lower() == "post":
                                    post = web_session.post(my_link, data = mal_dict, verify = False, headers = {"User-Agent":return_user_agent()}, proxies = tor_proxy, timeout = (5, 30))
                                
                            if tor == False:
                                if method[0].lower() == "get":
                                    get = web_session.get(my_link, params = mal_dict, verify = False, headers = {"User-Agent":return_user_agent()}, timeout = (5, 30))
                                
                                if method[0].lower() == "post":
                                    post = web_session.post(my_link, data = mal_dict, verify = False, headers = {"User-Agent":return_user_agent()}, timeout = (5, 30))
