    ...
    for links in my_result:
        ...
        try:
            for mal_script in mal_payloads:
                ...
                if tor:
                    result = web_session.get(my_url, verify = False, headers = {"User-Agent":return_user_agent()}, proxies = tor_proxy, timeout = (5, 30))
        ...
        try:
            for mal_script in mal_payloads:
                ...
                if tor:
                    result = web_session.get(links, verify = False, headers = {"User-Agent":return_user_agent()}, cookies = mal_cookie, proxies = tor_proxy, timeout = (5, 30))

                else:
                    result = web_session.get(links, verify = False, headers = {"User-Agent":return_user_agent()}, cookies = mal_cookie, timeout = (5, 30))
