...
def ip_change2():
    ...
    try:
        old_ip = requests.get("http://ip.wkwk.kr/", timeout=2).text
    except:
        for _ in range(30):
            try:
                old_ip = requests.get("http://ip.wkwk.kr/", timeout=2).text
    ...
    for cnt in range(90):
        ...
        try:
            cur_ip = requests.get("http://ip.wkwk.kr/", timeout=2).text
        except:
            pass
        else:
            if old_ip == cur_ip:
...
def get_time_now(__format: str, __delta=False):
    ...
    __format_list = ['%Y','%y','%m','%B','%b','%d','%j','%H','%l','%p','%M','%S','%f','%w','%A','%a','%W','%U','%c','%_x','%X']
...
def err_logging(input_data, program_title=None, path='./error.txt', msg_flag = True):
    '''
    ...
    try:
        ip_check = requests.get('http://ip.wkwk.kr/')
        ip_address = ip_check.text
        ip_check.close()
    except:
        ip_address = 'None("http://ip.wkwk.kr/"와 연결이 되지 않아 ip를 확인 할 수 없습니다.)'
