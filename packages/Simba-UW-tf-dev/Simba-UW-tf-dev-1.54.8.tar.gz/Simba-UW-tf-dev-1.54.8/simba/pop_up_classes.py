...
from simba.enums import (ReadConfig,
                         ...
                         Paths,
                         Dtypes)
#from simba.SimBA import SimbaProjectPopUp
import pandas as pd
import subprocess
import urllib
...
class FeatureSubsetExtractorPopUp(PopUpMixin):
    def __init__(self, config_path: str):
        super().__init__(title='EXTRACT FEATURE SUBSETS')
        self.feature_subset_options = ['Two-point body-part distances (mm)',
                                       'Within-animal three-point body-part angles (degrees)',
                                       'Within-animal three-point convex hulls (mm2)',
                                       'Within-animal four-point convex hulls (mm2)',
                                       'Entire animal convex hulls (mm2)',
                                       'Frame-by-frame body-part movements (mm)']
        self.config_path = config_path
        self.settings_frm = LabelFrame(self.main_frm, text='SETTINGS', font=("Helvetica", 12, 'bold'), pady=5, padx=5)
...
class ThirdPartyAnnotatorAppenderPopUp(PopUpMixin):
    def __init__(self, config_path: str):
        ...
        log_frm.grid(row=4, column=0, sticky=NW)
