...
class TestSltu(unittest.TestCase):
    ...
    def test_sltu_1(self):
        '''
            Tests the sltu instruction sets the destination register to 1 if the value
            stored in source register is less than target register, 0 otherwise without sign
            extending
        '''
        reg = Registers()
        reg.set_register("$t0", 5)
        reg.set_register("$t1", -3)
        instructions.slt("$t2", "$t0", "$t1", reg, signed=False)
