...
from local_llama_index.embeddings.base import BaseEmbedding
...
def get_embedding(
    ...
    """
    text = text.replace("\n", " ")
    return localopenai.Embedding.create(input=[text], engine=engine)["data"][0]["embedding"]
...
async def aget_embedding(text: str, engine: Optional[str] = None) -> List[float]:
    ...
    return (await localopenai.Embedding.acreate(input=[text], engine=engine))["data"][0][
        "embedding"
    ]
...
def get_embeddings(
    ...
    data = localopenai.Embedding.create(input=list_of_text, engine=engine).data
...
async def aget_embeddings(
    ...
    data = (await localopenai.Embedding.acreate(input=list_of_text, engine=engine)).data
    data = sorted(data, key=lambda x: x["index"])  # maintain the same order as input.
    return [d["embedding"] for d in data]
