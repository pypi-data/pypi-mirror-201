...
from local_llama_index.constants import MAX_CHUNK_SIZE, NUM_OUTPUTS
from local_llama_index.prompts.base import Prompt
from local_llama_index.utils import (ErrorToRetry, globals_helper,
    ...
    def _predict(self, prompt: Prompt, **prompt_args: Any) -> str:
        ...
        if self.retry_on_throttling:
            llm_prediction = retry_on_exceptions_with_backoff(
                ...
                [
                    ErrorToRetry(localopenai.error.RateLimitError),
                    ErrorToRetry(localopenai.error.ServiceUnavailableError),
                    ErrorToRetry(localopenai.error.TryAgain),
                    ErrorToRetry(
                        localopenai.error.APIConnectionError, lambda e: e.should_retry
                    ),
                ],
            )
        else:
