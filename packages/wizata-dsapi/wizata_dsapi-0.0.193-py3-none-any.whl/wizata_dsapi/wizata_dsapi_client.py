...
class WizataDSAPIClient:
    ...
    def get(self, obj=None, script_name=None):
        """
    ...
    def upsert(self, obj):
        ...
        work with Script or directly your function using the technical name of the function
    ...
    def query(self,
              ...
              interval: int,
              agg_method: str = "mean") -> pandas.DataFrame:
        """
    ...
    def execute(self,
        ...
        if ml_model is not None:
            if isinstance(ml_model, uuid.UUID):
                execution.ml_model = MLModel(ml_model)
            elif isinstance(ml_model, str):
                execution.ml_model = MLModel(uuid.UUID(ml_model))
        ...
        if experiment is not None:
            if isinstance(experiment, uuid.UUID):
                ...
                execution.experiment_id = uuid.UUID(experiment)
        ...
        if isinstance(execution, Execution):
            ...
            if response.status_code == 200:
                ...
                if "anomaliesList" in obj.keys() and isinstance(obj["anomaliesList"], str):
                    execution.anomalies = json.loads(execution["anomaliesList"])
                return execution
            else:
                raise self.__raise_error(response)
        else:
    ...
    def register_model(self,
                       ...
                       has_target_feat: bool = False) -> tuple[MLModel, pandas.DataFrame]:
        """
