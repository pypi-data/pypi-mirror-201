...
def node2dict(node: neo4j.graph.Node):
    return {"id": node.id, "labels": list(node.labels), "properties": node._properties}


def relationship2dict(relationship: neo4j.graph.Relationship):
...
def parse_record(record: neo4j.data.Record, flag=True):
    ...
    if isinstance(record, neo4j.data.Record):
        for rd in record:
            if isinstance(rd, neo4j.graph.Node):
                _record["node"].append(node2dict(rd))
            elif issubclass(rd.__class__, neo4j.graph.Node):
                _record["node"].append(node2dict(rd))
            elif isinstance(rd, neo4j.graph.Relationship):
                _record["relationship"].append(relationship2dict(rd))
            elif issubclass(rd.__class__, neo4j.graph.Relationship):
                _record["relationship"].append(relationship2dict(rd))
            elif isinstance(rd, list) or isinstance(rd, tuple):
                for x in rd:
                    if isinstance(x, neo4j.graph.Node):
                        _record["node"].append(node2dict(x))
                    elif issubclass(x.__class__, neo4j.graph.Node):
                        _record["node"].append(node2dict(x))
                    elif isinstance(x, neo4j.graph.Relationship):
                        _record["relationship"].append(relationship2dict(x))
                    elif issubclass(x.__class__, neo4j.graph.Relationship):
                        _record["relationship"].append(relationship2dict(x))
                    elif isinstance(x, neo4j.graph.Path):
