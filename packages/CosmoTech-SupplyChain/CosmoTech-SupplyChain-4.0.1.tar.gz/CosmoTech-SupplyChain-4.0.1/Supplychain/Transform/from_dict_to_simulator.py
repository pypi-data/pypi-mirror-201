...
class FromDictToSimulatorConverter(Timer):
    ...
    def __stock_to_operation(self):
        ...
        retainers = set()
        if self.configuration['IntermediaryStockDispatchPolicy'] == "AllowRetention":
            retainers = set(
                stock['id']
                for stock in stock_csv
        ...
        def compute_prop_sto(source):
            return [
                1 / (number_of_edges_by_stock[i["source"]] + int(source in retainers))
            ] * self.configuration["SimulatedCycles"]

        for i in input_csv:
            prop_sto = compute_prop_sto(i["source"])
            sto_item = {
