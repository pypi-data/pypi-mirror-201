...
class Browser:
    ...
    def _getPageDimensions(self):
        ...
        self.windowInnerHeight = dimensions['windowInner']['height']
        ...
        # crop image heights from top and bottom 
        # can differ for first image, versus last image
        self._cropFirst = self._cropMiddle = self._cropLast = { 'top': 0, 'bottom': 0 }
        ...
        if isExpectedWidth and isExpectedHeight:
            ...
            if self._deviceInfo['osName'] == 'android':
                ...
                cropBottomHeight = testImageHeight - expectedHeight
                self._cropFirst = self._cropMiddle = self._cropLast = { 'top': 0, 'bottom': cropBottomHeight }
                log.info(
                    f'Size Test: Android device has {cropBottomHeight}px extra pixels to crop on each image at the bottom')
                ...
                cropTopHeight = testImageHeight - expectedHeight
                self._cropFirst = self._cropMiddle = self._cropLast = { 'top': cropTopHeight, 'bottom': 0 }
                log.info(
                    f'Size Test: iOS device has {cropTopHeight}px extra pixels to crop on each image (top and/or bottom)')
            else:
                raise Exception(f'Size Test: Unknown platform to handle when javascript values do not match image size')

        # special case for safari to avoid rounded corners and top 1px border in seleneium screenshot
        if self._deviceInfo['browserName'] == 'safari' and self._deviceInfo['browserVersion'] == 16:
            bottomCornersHeight = 15 # rounded corners
            topBorderHeight = 1 # top border
            
            # adjust viewport height by removing calculation for top and bottom height that will be cropped
            self.viewportHeight -= bottomCornersHeight + topBorderHeight

            # only crop if more than one page
            if (self._getNumberOfPagesToScroll() > 1):
                # define crops
                cropTopHeight = topBorderHeight * self.devicePixelRatio
                cropBottomHeight = bottomCornersHeight * self.devicePixelRatio
                self._cropFirst = { 'top': 0, 'bottom': cropBottomHeight } # only crop bottom height on first image
                self._cropMiddle = { 'top': cropTopHeight, 'bottom': cropBottomHeight }
                self._cropLast = { 'top': cropTopHeight, 'bottom': 0 }  # only crop top height on last image
    ...
    def takeFullpageScreenshot(self, path: str, lazyload=None):
        ...
        if totalPages == 1:
            ...
            # only look at first when single page (mobiles will have both values set)
            if self._cropFirst['top']:
                file = ImageTools.cropTop(file, self._cropFirst['top'])

            if self._cropFirst['bottom']:
                file = ImageTools.cropBottom(file, self._cropFirst['bottom'])
            ...
            while not done:
                ...
                if pageIndex == 0: # on first page
                    if self._cropFirst['top']:
                        file = ImageTools.cropTop(file, self._cropFirst['top'])
                    if self._cropFirst['bottom']:
                        file = ImageTools.cropBottom(file, self._cropFirst['bottom'])
                elif pageIndex == totalPages - 1: # on last page
                    if self._cropLast['top']:
                        file = ImageTools.cropTop(file, self._cropLast['top'])
                    if self._cropLast['bottom']:
                        file = ImageTools.cropBottom(file, self._cropLast['bottom'])
                else: # on middle page but not the last
                    if self._cropMiddle['top']:
                        file = ImageTools.cropTop(file, self._cropMiddle['top'])
                    if self._cropMiddle['bottom']:
                        file = ImageTools.cropBottom(file, self._cropMiddle['bottom'])

                # crop remaining image as necessary when on last page
                if self._scrollMethod == 'CSS_TRANSLATE':
