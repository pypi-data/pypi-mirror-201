...
class Node:
    ...
    features: typing.List[
        typing.Union[typing.Tuple[np.array, np.array], np.array, bytes]
...
class BinaryWriter:
    ...
    def add(self, data: typing.Iterator[typing.Union[Node, Edge]]):
        """
        Write binary for a node and all of its edges.

        args:
            data: Union[Node,Edge] Data for a node first, then all of its edges in order of type then dst.
                All of the node's outgoing edges do not need to go to the same writer.
        """
        ...
        for src, dst, typ, weight, created_at, removed_at, features in data:
            ...
            if dst == -1:
                ...
                assert (
                    removed_at is None or node_removed_at == -1 or removed_at <= node_removed_at
