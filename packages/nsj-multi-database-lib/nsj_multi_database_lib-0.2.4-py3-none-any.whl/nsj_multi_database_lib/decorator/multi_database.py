...
from nsj_multi_database_lib.settings import get_logger, get_crypt_key
        ...
        def wrapper(*args, **kwargs):
            try:
                with InjectorFactory() as factory:
                    ...
                    # Recuperando o tenant da query string
                    ...
                    if database["user"] is not None and database["password"] is not None:
                        ...
                        # Decypt user
                        user = database["user"].encode()
                        user = fernet.decrypt(user).decode()

                        # Decypt user
                        password = database["password"].encode()
                        password = fernet.decrypt(user).decode()
