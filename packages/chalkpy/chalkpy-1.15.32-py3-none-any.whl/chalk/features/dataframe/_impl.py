...
class DataFrameImpl(metaclass=DataFrameMeta):
    ...
    def read_delta(
        ...
        if new_columns is not None:
            data = data.rename({c: new_columns for c in data.columns})
        return cls(data)
    ...
    def read_avro(
        ...
        data = None
        if (
            (type(path) == "string" and path.startswith("s3"))
            or (type(path) == "PosixPath")
            and (str(path).startswith("s3"))
        ):
            try:
                import fsspec
            except ImportError:
