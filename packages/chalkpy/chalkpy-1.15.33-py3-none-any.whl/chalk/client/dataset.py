...
from chalk import BranchId, EnvironmentId
from chalk.client import ChalkClient, Dataset, DatasetRevision
...
class DatasetRevisionImpl(DatasetRevision):
    ...
    def download_data(
        ...
        for url in urls:
            executor.submit(_download_data, url, path).result()
...
class DatasetImpl(Dataset):
    ...
    def recompute(self, branch: BranchId, features: Optional[List[Union[Feature, Any]]] = None):
        new_revision = self._client._recompute_dataset(
        ...
        self.revisions.append(dataset_revision_from_response(new_revision, self._client))
        return self
