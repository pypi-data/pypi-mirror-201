...
class Resolver(ResolverProtocol[P, T], abc.ABC):
    ...
    @classmethod
    def _deploy_on_notebook_change(cls, resolver: Resolver):
        """
        If this code is being run in a Jupyter notebook, cloudpickle this
        resolver's dependencies and send it to the API server.
        """
        if not notebook.is_notebook():
            return
        pickled_resolver = cloudpickle.dumps(resolver)
        client = ChalkAPIClientImpl._latest_client
        # TODO ignore errors for now until server endpoint is created
        try:
            resp = client._send_updated_resolver(resolver.environment, pickled_resolver)
            if resp.errors:
                for e in resp.errors:
                    _logger.debug(f"{e}")
                raise RuntimeError("Failed to update resolvers.")
        except Exception:
            pass

    hook: ResolverHook = _deploy_on_notebook_change
...
def _validate_parse_function(
    ...
    return_annotation = get_type_hints(parse_fn).get("return")
    ...
    if not issubclass(parse_input, BaseModel):
        raise TypeError(f"Parse function '{parse_fqn}' return annotation must be of type pydantic.BaseModel")
