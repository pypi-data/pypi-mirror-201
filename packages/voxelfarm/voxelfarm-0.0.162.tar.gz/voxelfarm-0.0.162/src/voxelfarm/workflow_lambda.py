...
class workflow_lambda_host:
    ...
    def set_workflow_definition(self, workflow_definition):
        ...
        if 'success' in result and result['success']:
            if "complete" in result and result["complete"]:
                ...
                if (workflow_request.state != state.initialize) and ('activated' not in workflow_request.properties):
    ...
    def create_product_version(self, project_id, product_id, inputs = None, files = None):
        ...
        if project_entity:
            ...
            last_version = int(product_entity['version_last']) if 'version_last' in product_entity else 0
            version_number = last_version + 1
            self.lambda_framework.log(f'Updating version {version_number} in product')
            self.vf_api.update_entity(
                project=project_id,
                id=product_folder_id,
                fields={
                    'version_last' : str(version_number)
                }
            )
