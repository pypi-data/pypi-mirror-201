...
class Dataset:
    ...
    def start_time(self) -> datetime:
        ...
        start_datetime: datetime = self.__dataframe[timestamp_col_name].min() - timedelta(
            seconds=60,
        )  # subtracting a second, so to include all the data
    ...
    def end_time(self) -> datetime:
        ...
        end_datetime equals max(timestamp) + 1 second, so that it can be
        used as part of a right-open interval.
        """
        timestamp_col_name: str = cast(str, self.schema.timestamp_column_name)
        end_datetime: datetime = self.__dataframe[timestamp_col_name].max() + timedelta(
            seconds=60,
        )  # adding a second, so it can be used as part of a right open interval
        return end_datetime
