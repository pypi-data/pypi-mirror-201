...
class PendingTransactionData(BaseModel):
    ...
    history: conlist(PendingTransactionHistory) = Field(..., description="An array representing any previous states of the hold, if it has been modified (For example, increasing or decreasing the hold amount).")
    ...
    def operation_validate_enum(cls, v):
        if v not in ('hold_new', 'hold_increase', 'hold_decrease', 'hold_replace', 'hold_decline', 'hold_cancel', 'hold_post', 'hold_expire'):
            raise ValueError("must be one of enum values ('hold_new', 'hold_increase', 'hold_decrease', 'hold_replace', 'hold_decline', 'hold_cancel', 'hold_post', 'hold_expire')")
        return v

    @validator('status')
    def status_validate_enum(cls, v):
        if v not in ('PENDING', 'REPLACED', 'DECLINED', 'RELEASED', 'PARTCLEARED', 'EXPIRED', 'CLEARED', 'INTERNAL_ERROR'):
            raise ValueError("must be one of enum values ('PENDING', 'REPLACED', 'DECLINED', 'RELEASED', 'PARTCLEARED', 'EXPIRED', 'CLEARED', 'INTERNAL_ERROR')")
        return v
