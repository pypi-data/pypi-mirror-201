...
class VirtualCardResponse(BaseModel):
    ...
    creation_time: datetime = Field(..., description="The timestamp representing when the card issuance request was made")
    ...
    id: StrictStr = Field(..., description="Card ID")
    ...
    type: StrictStr = Field(..., description="Indicates the type of card to be issued")
    ...
    bin: Optional[StrictStr] = Field(None, description="The bin number")
    card_brand: CardBrand = ...
    __properties = ["form", "account_id", "card_product_id", "creation_time", "customer_id", "emboss_name", "expiration_month", "expiration_time", "expiration_year", "id", "is_pin_set", "last_four", "last_modified_time", "metadata", "reissue_reason", "reissued_from_id", "reissued_to_id", "type", "card_status", "memo", "status_reason", "bin", "card_brand"]

    @validator('form')
    def form_validate_enum(cls, v):
        if v not in ('VIRTUAL'):
            raise ValueError("must be one of enum values ('VIRTUAL')")
        return v
    ...
    def reissue_reason_validate_enum(cls, v):
        ...
        if v not in ('EXPIRATION', 'LOST', 'STOLEN', 'DAMAGED', 'NAME_CHANGE', 'VIRTUAL_TO_PHYSICAL', 'PRODUCT_CHANGE', 'APPEARANCE'):
            raise ValueError("must be one of enum values ('EXPIRATION', 'LOST', 'STOLEN', 'DAMAGED', 'NAME_CHANGE', 'VIRTUAL_TO_PHYSICAL', 'PRODUCT_CHANGE', 'APPEARANCE')")
        return v

    @validator('type')
    def type_validate_enum(cls, v):
        if v not in ('DEBIT'):
            raise ValueError("must be one of enum values ('DEBIT')")
        return v
    ...
    @classmethod
    def from_json(cls, json_str: str) -> VirtualCardResponse:
        """Create an instance of VirtualCardResponse from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> VirtualCardResponse:
        """Create an instance of VirtualCardResponse from a dict"""
        if obj is None:
            return None

        if type(obj) is not dict:
            return VirtualCardResponse.parse_obj(obj)

        _obj = VirtualCardResponse.parse_obj({
            "form": obj.get("form"),
