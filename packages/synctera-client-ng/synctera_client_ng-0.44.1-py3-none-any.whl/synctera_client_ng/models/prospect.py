...
class Prospect(BaseModel):
    ...
    status: StrictStr = Field(..., description="Customer's status")
    ...
    note: Optional[StrictStr] = Field(None, description="A note is required when updating a Customers's ban_status between SUSPENDED and ALLOWED.")
    ...
    related_customers: Optional[conlist(Relationship1)] = Field(None, description="Customer's relationships with other accounts eg. guardian")
    ...
    __properties = ["dob", "first_name", "last_name", "status", "ban_status", "creation_time", "email", "id", "kyc_exempt", "kyc_last_run", "kyc_status", "last_updated_time", "legal_address", "metadata", "middle_name", "note", "phone_number", "related_customers", "shipping_address", "ssn", "ssn_source", "tenant"]

    @validator('status')
    def status_validate_enum(cls, v):
        if v not in ('ACTIVE', 'ESCHEAT', 'DECEASED', 'DENIED', 'DORMANT', 'FROZEN', 'INACTIVE', 'PROSPECT', 'SANCTION'):
            raise ValueError("must be one of enum values ('ACTIVE', 'ESCHEAT', 'DECEASED', 'DENIED', 'DORMANT', 'FROZEN', 'INACTIVE', 'PROSPECT', 'SANCTION')")
        return v
    ...
    @classmethod
    def from_json(cls, json_str: str) -> Prospect:
        """Create an instance of Prospect from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> Prospect:
        """Create an instance of Prospect from a dict"""
        if obj is None:
            return None

        if type(obj) is not dict:
            return Prospect.parse_obj(obj)

        _obj = Prospect.parse_obj({
            "dob": obj.get("dob"),
