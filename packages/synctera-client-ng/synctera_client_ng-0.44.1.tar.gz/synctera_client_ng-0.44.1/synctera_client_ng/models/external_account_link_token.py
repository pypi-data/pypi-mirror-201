...
class ExternalAccountLinkToken(BaseModel):
    ...
    def country_codes_validate_enum(cls, v):
        for i in v:
            if i not in ('US'):
                raise ValueError("each list item must be one of ('US')")
        return v

    @validator('language')
    def language_validate_enum(cls, v):
        if v not in ('EN', 'FR', 'ES', 'NL', 'DE'):
            raise ValueError("must be one of enum values ('EN', 'FR', 'ES', 'NL', 'DE')")
        return v
    ...
    def sdk_type_validate_enum(cls, v):
        ...
        if v not in ('WEB', 'IOS', 'ANDROID'):
            raise ValueError("must be one of enum values ('WEB', 'IOS', 'ANDROID')")
        return v

    @validator('type')
    def type_validate_enum(cls, v):
        if v not in ('DEPOSITORY', 'CREDIT', 'INVESTMENT', 'MICRO_DEPOSIT'):
            raise ValueError("must be one of enum values ('DEPOSITORY', 'CREDIT', 'INVESTMENT', 'MICRO_DEPOSIT')")
        return v
