...
class DepositGet(BaseModel):
    ...
    __properties = ["account_id", "back_image_id", "business_id", "check_amount", "deposit_currency", "front_image_id", "metadata", "person_id", "creation_time", "date_captured", "date_processed", "deposit_amount", "id", "last_updated_time", "ocr_account_number", "ocr_check_number", "ocr_routing_number", "status", "transaction_id", "vendor_info"]

    @validator('status')
    def status_validate_enum(cls, v):
        if v not in ('SUBMITTED', 'PENDING', 'FAILED', 'REJECTED'):
            raise ValueError("must be one of enum values ('SUBMITTED', 'PENDING', 'FAILED', 'REJECTED')")
        return v
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DepositGet:
        """Create an instance of DepositGet from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> DepositGet:
        """Create an instance of DepositGet from a dict"""
        if obj is None:
            return None

        if type(obj) is not dict:
            return DepositGet.parse_obj(obj)

        _obj = DepositGet.parse_obj({
