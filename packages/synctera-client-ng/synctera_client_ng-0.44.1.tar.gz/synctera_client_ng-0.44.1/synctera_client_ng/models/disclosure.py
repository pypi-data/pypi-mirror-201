...
class Disclosure(BaseModel):
    ...
    type: DisclosureType = ...
    version: constr(strict=True) = Field(..., description="Version of the disclosure document.")
    ...
    def event_type_validate_enum(cls, v):
        if v not in ('DISPLAYED', 'VIEWED', 'ACKNOWLEDGED'):
            raise ValueError("must be one of enum values ('DISPLAYED', 'VIEWED', 'ACKNOWLEDGED')")
        return v
    ...
    @classmethod
    def from_json(cls, json_str: str) -> Disclosure:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> Disclosure:
        """Create an instance of Disclosure from a dict"""
        if obj is None:
            return None

        if type(obj) is not dict:
            return Disclosure.parse_obj(obj)

        _obj = Disclosure.parse_obj({
