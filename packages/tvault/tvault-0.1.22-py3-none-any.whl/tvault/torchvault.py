...
class TorchVault:
    ...
    def log_scheduler(self, scheduler):
        ...
        model_log[str(model_idx)]["scheduler"] = scheduler.__str__()
    ...
    def log_optimizer(self, optimizer):
        ...
        model_log[str(model_idx)]["optimizer"] = optimizer.__str__()
        self.write_model_log("", model_log)
    ...
    def add_tag(self, sha="", tag="", idx=None):
        ...
        for model_idx in target_idxs:
            ...
            model_log[str(model_idx)]["tag"] = tag
    ...
    def add_result(self, sha="", result=0, idx=None):
        ...
        for model_idx in target_idxs:
            ...
            model_log[str(model_idx)]["result"] = result
        print(f"write log: {model_log}")
        self.write_model_log(sha, model_log)
    ...
    def log_model(self, model):
        ...
        model_log[str(model_idx)]["model"] = model.__str__()
        model_log[str(model_idx)]["src"] = dict(filter_target_class)
        model_log[str(model_idx)]["external_func"] = dict(filter_target_funcs)
