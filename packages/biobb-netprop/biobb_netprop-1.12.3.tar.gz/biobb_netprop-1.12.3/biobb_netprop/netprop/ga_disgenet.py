...
class GADisgenet(BiobbObject):
    """
    | biobb_netprop Gene Attribute Disgenet
    ...
    def launch(self) -> int:
        ...
        if self.properties.get('retrieve_by'):
            ...
            response = ga_va_session("gene", self.properties.get('retrieve_by'), self.properties, self.out_log, self.global_log)
        else:
...
def ga_disgenet(input_file_path: str, output_file_path: str, properties: dict = None, **kwargs) -> int:
    """Create :class:`GADisgenet <disgenet.ga_disgenet.GADisgenet>` class and
    execute the :meth:`launch() <disgenet.ga_disgenet.GADisgenet.launch>` method."""
