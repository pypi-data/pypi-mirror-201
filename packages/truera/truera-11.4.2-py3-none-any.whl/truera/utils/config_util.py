...
def get_config_value(config, key1, key2, default=None, dynaconf_settings=None):
    if dynaconf_settings is not None:
        for key, value in dynaconf_settings.items():
...
def get_config_value_arbitrary_depth(
    config, keys, default, dynaconf_settings=None
):
    if dynaconf_settings is not None:
        for key, value in dynaconf_settings.items():
            # dynamic configs takes precedence over static ones
...
def read_tenant_specific_config(tenant_id, logger=None):
    # load all tenant-specific configs from file source dynamically, where tenant_id is always the first-level key
    # TODO: specify fresh_vars individually for configs that need to always be read from the source instead of from the cache
    ...
    if logger:
        logger.debug(
            "Loaded dynaconf configurations: {}".format(dynaconf_settings)
        )

    # reload fresh config for tenant id from source file
    tenant_config = dynaconf_settings.get(tenant_id, fresh=True)
    return None if tenant_config is None else tenant_config
