...
class UserAnalyticsServiceClient():
    ...
    def poll_queue_for_events(self):
        while self.is_consumer_active:
            sendEventRequest = self.event_queue.get()
            try:
                self.communicator.send_analytics_event(sendEventRequest)
            except:
                if self.logger is not None:
    ...
    def create_send_event_request(
        ...
        *,
        ...
        for key in event_properties:
            sendEventRequest.event_properties[key] = str(event_properties[key])
        sendEventRequest.event_properties["is_success"] = str(is_success)
    ...
    def track_event(
        ...
        *,
        ...
        try:
            ...
            self.event_queue.put(sendEventRequest)
        except:
            if self.logger is not None:
