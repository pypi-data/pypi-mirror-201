...
class NLPAttribution(RNNAttribution):
    ...
    def _get_attrs(
        *,
        ...
        if model_args.partition_at == ModelPartition.INTERMEDIATE_OUTPUT:
            ...
            token_attrs = input_attrs.sum(axis=-1)
            if not mem_streamer.memmap_is_init():
                mem_streamer.init_memmap_fp(
                    QoIType.CLASS_WISE.get_attr_artifact_save_name(),
    ...
    def calculate_attribution(
        *,
        ...
        """Core function that calculates attributions.
        ...
        Args:
            ...
            model_run_wrapper (Wrappers.ModelRunWrapper): functions needed to run the model
            output_path (Path): the output storage location of artifacts
            sample_size (int): the number of samples to run for.
            model_args (convert_struct): the attribution config - legacy support for dictionaries. # TODO: have this has attr config only
            backend (Backend): The backend of tf or pytorch.
            baseline_type (BaselineType, optional): Only changed for experimental baselines. Defaults to BaselineType.DYNAMIC.
            baseline (Optional[TensorLike], optional): The baseline record. Defaults to None.
            resolution (int, optional): The number of interpolations for Integrated gradients. Defaults to DEFAULT_IG_RESOLUTION.
            gradient_path_sample_size (int, optional): The number of records for feature interactions. Defaults to 0.
            filter_func (FilterFunc, optional): a function to filter out records. Defaults to None.
            pre_run (bool, optional): A flag to generate pre-requirements for attributions. Defaults to False.
        """
        ...
        if backend == Backend.PYTORCH:
            ...
            if model_args.use_training_mode:
                logger.warn(
                    "configuration attribute `use_training_mode` is set to `True`. Only use this if you ran into torch errors. Otherwise, torch uses this parameter which can cause inconsistenty on gradients or batch norms."
                )
                model.train()
            else:
                model.eval()
        ...
        # Renable this to figure out the correct run configuration.
        # tl_model.print_layer_names()
