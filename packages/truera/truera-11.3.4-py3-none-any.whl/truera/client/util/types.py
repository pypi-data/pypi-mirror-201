...
class Parameter(AutoNone, LogicalType):
    ...
    def __new__(cls, name: str, typ: Annotation, doc: str = ""):
        # __new__ needed for type subtypes.

        typ = eval_type(typ, globals=caller_globals())

        display_name = Parameter._display_name(name, typ, doc)

        return super().__new__(cls, display_name, (), {})

    @staticmethod
    def _display_name(name: str, typ: type, doc: str, display_name: str = None):
    ...
    def __init__(self, name: str, typ: type, doc: str = ""):
        self.name = name
        self.doc = doc
        self.typ = eval_type(typ, globals=caller_globals())

        display_name = Parameter._display_name(name, self.typ, doc)
