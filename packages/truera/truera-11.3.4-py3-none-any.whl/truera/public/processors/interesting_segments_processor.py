...
class InterestingSegmentsProcessor:
    ...
    def pointwise_metrics_aggregator(
        ...
        if interesting_segment_type in POINTWISE_METRIC_MAP:
            return SimpleAggregator(
                interesting_segment_type, minimum_size, size_exponent
            )
        elif interesting_segment_type in THRESHOLDED_METRIC_MAP:
            ...
            return SegmentSizeAggregator(interesting_segment_type, None, None)
    ...
    def _create_numerical_base_expression(
        ...
        if np.all(np.isnan([lo, hi])):
            ...
            operator = FilterExpressionOperator.FEO_OR if np.any(
                np.isnan([lo, hi])
            ) else FilterExpressionOperator.FEO_AND
...
class SimpleAggregator(AggregatorBaseClass):
    ...
    def __init__(
        ...
        super().__init__(segment_type, minimum_size, size_exponent)
    ...
    def __call__(
        ...
        if any([curr < self.minimum_size for curr in szs]):
            return -np.inf
        if len(sums) == 1:
            return self.sign * (sums[0][0] /
                                szs[0]) * szs[0]**self.size_exponent
        if len(sums) == 2:
            return self.sign * ((sums[0][0] / szs[0]) - (sums[1][0] / szs[1])
                               ) * min(szs)**self.size_exponent
        raise TruEraInvalidArgumentError(
...
class NumeratorAndDenominatorAggregator(AggregatorBaseClass):
    ...
    def __call__(
        ...
        if len(sums) == 1:
            return self.sign * (sums[0][0] /
                                sums[0][1]) * szs[0]**self.size_exponent
        if len(sums) == 2:
            return self.sign * (
...
class SegmentSizeAggregator(AggregatorBaseClass):
    ...
    def __call__(
        ...
        if len(sums) == 2:
            return np.abs(sums[0][0] - sums[1][0])
        raise TruEraInvalidArgumentError(
