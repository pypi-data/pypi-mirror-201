...
logging_conf = os.path.join(os.path.dirname(os.path.abspath(__file__)), "logging.yaml")
...
def render_subroutines(data, cwd=None, name=None):
...
def abml_file_mode(file: str, parameters: dict, env: Environment, cwd=None, name=None, config={}):
    ...
    render_subroutines(data, cwd=cwd, name=name)
...
def run(files=None):
    ...
    parameters_yaml = filter(re.compile("_parameters.abml").match, os.listdir("."))
    parameters = {}
...
def load_grid(env, parameters, filename="_grids.abml"):
    grids = {}
    for grid_file in filter(re.compile(filename).match, os.listdir(".")):
        template = env.get_template(grid_file)
        grids.update(yaml.load(template.render(**parameters), Loader=Loader))
    return grids


def load_parameters(filename="_parameters.abml"):
    parameters = {}
    for param_file in filter(re.compile(filename).match, os.listdir(".")):
        with open(param_file, mode="r", encoding="utf-8") as f:
            parameters.update(yaml.load(f, Loader=Loader))
    return parameters


def load_filenames(files, ext=".abml"):
    if files is None or files == ():
        pattern = re.compile(f"^(?!_).*{ext}$")
        files = list(filter(pattern.match, os.listdir(".")))
    return files


def load_config(filename="_config.abml"):
    config_file = filter(re.compile(filename).match, os.listdir("."))
    for config in config_file:
        with open(os.path.basename(config), mode="r", encoding="utf-8") as f:
...
def run_grid(files=None):
    ...
    grids = load_grid(env, parameters)
    files = load_filenames(files)

    for grid_name, grid in grids.items():
        parameters_list = []
        for set_ in generate_param_grid(grid):
        ...
        for parameters_ in parameters_list:
            ...
            for file in files:
                file = os.path.basename(file)
                processed = abml_file_mode(file, parameters_, env, cwd=cwd, name=filename)
        df.index = indices
