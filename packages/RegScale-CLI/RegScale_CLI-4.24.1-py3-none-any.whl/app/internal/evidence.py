...
from app.utils.app_utils import check_file_path, create_progress_object, error_and_exit
...
def start():
    ...
    # Fitz's library requires this static directory in the PWD.
    ...
    with progress:
        ...
        global directories
        directories = set_directory_variables(task=task0)
        ...
        # call function to define variable for use outside of function
        global docs
        docs = parse_required_docs(evidence_folder=directories[0], task=task1)
        ...
        times = get_doc_timestamps(
            evidence_folder=directories[2], directory=directories[1], task=task2
        )
        ...
        # call function to define variable for use outside of function
        global texts
        texts = set_required_texts(evidence_folder=directories[0], task=task3)
        ...
        # call function to define variable for use outside of function
        global folders
        folders = find_required_files_in_folder(
            evidence_folder=directories[2], task=task4
        )
        ...
        # call function to define variable for use outside of function
        global SIGRESULTS
        SIGRESULTS = signature_assessment_results(
            directory=folders, r_docs=docs[0], task=task5
        )
        ...
        # call function to define variable for use outside of function
        global DOCRESULTS
        DOCRESULTS = document_assessment_results(
            directory=folders, documents=docs[1], task=task6
        )
        ...
        # call function to define variable for use outside of function
        global file_texts
        file_texts = parse_required_text_from_files(
            evidence_folder=directories[2], task=task7
        )
        ...
        search_results = text_string_search(
            f_texts=file_texts, req_texts=texts, task=task8
        )
        ...
        # call function to define variable for use outside of function
        global TEXTRESULTS
        TEXTRESULTS = text_assessment_results(
            searches=search_results, r_texts=texts, task=task9
        )
        ...
        # call function to define variable for use outside of function
        global data
        data = gather_test_project_data(evidence_folder=directories[0], task=task10)
        ...
        global TIMERESULTS
        TIMERESULTS = assess_doc_timestamps(
            timestamps=times, documents=docs[0], task=task11
        )
        ...
        # call function to define variable to use outside of function
        global results
        results = build_assessment_dataframe(assessments=report, task=task13)

        task14 = progress.add_task("[white]Calculating assessment score...", total=1)

        # call function to define variable for use outside of function
        global score_data
        score_data = build_score_data(assessments=results, task=task14)
        ...
        # call function to define variable for use outside of function
        global html_output
        html_output = build_html_table(assessments=report, task=task15)
        ...
        create_child_assessments(project_data=data, output=html_output, task=task16)
...
def parse_required_docs(evidence_folder: str, task) -> Tuple[list[dict], set[str]]:
    """
...
def get_doc_timestamps(evidence_folder: str, directory: str, task) -> list[dict]:
    """
...
def set_required_texts(evidence_folder: str, task) -> set[str]:
    """
...
def find_required_files_in_folder(evidence_folder: str, task) -> list[dict]:
    """
...
def document_assessment_results(
    directory: list[dict], documents: set[str], task
) -> list[dict]:
    """
...
def parse_required_text_from_files(evidence_folder: str, task) -> list[dict]:
    """
...
def text_string_search(f_texts: list[dict], req_texts: set[str], task) -> list[dict]:
    """
...
def assess_doc_timestamps(
    timestamps: list[dict], documents: list[dict], task
) -> list[dict]:
    """
...
def build_html_table(assessments: list[dict], task) -> list[dict]:
    """
...
def create_child_assessments(
    project_data: list[dict], output: list[dict], task
) -> None:
    """
