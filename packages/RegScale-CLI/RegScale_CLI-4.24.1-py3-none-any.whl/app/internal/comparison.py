...
from app.utils.regscale_utils import (
    create_regscale_assessment,
    upload_file_to_regscale,
    verify_provided_module,
)
from models.app_models.click import NotRequiredIf, regscale_id, regscale_module
    ...
    if exists(old_file) and exists(new_file):
        with job_progress:
            ...
            if old_file_type == ".csv" and new_file_type == ".csv":
                ...
                error_and_exit(
                    f"{old_file_type} files are not a supported file type provided for comparison."
                )
            ...
            title = (
                f"{file_type} Comparison for {regscale_module.title()}-{regscale_id}"
            )
        ...
        error_and_exit(
            "Please check the file paths of the provided files and try again."
        )
...
def comparison(
    ...
    :raises: General error if unaccepted file type provided
...
def create_filtered_html_table(
    data: pd.DataFrame, column: str, value: any, pop_flag: bool = True
) -> str:
    """
