...
from models.regscale_models.user import User
...
def get_access_token() -> dict:
    ...
    :raises: JSONDecodeError if API request cannot convert to json object
    :raises: KeyError if API request doesn't have "access_token" as a key
    ...
    try:
        ...
        error_and_exit(
            f"Unable to obtain access token! Please verify credentials.\nError: {ex}\n{token}"
        )
    # return the result
    return token
...
def get_group(str_group: str) -> None:
    ...
    if len(groups_data) == 0:
        ...
        logger.info(
            "Fetching the list of members for this AD group - %s", str(group_id)
        )
        try:
        ...
        for us in regscale_new:
            ...
            try:
                ...
                error_and_exit(
                    f"Unable to check role: {us['id']}, {regscale_role['id']}.\nError: {ex}"
                )
            ...
            if str_check == "false":
                ...
                try:
                    ...
                    error_and_exit(
                        f"Unable to assign role: {us['id']}/{regscale_role['id']}.\nError: {ex}"
                    )
        ...
        for us in remove_users:
            ...
            if str_check == "true":
                ...
                try:
                    ...
                    error_and_exit(
                        f"Unable to remove role: {us['id']}/{regscale_role['id']}.\nError: {ex}"
                    )

                # deactivate the user if they were in this role
                url_deactivate = (
                    f'{config["domain"]}/api/accounts/changeUserStatus/{us["id"]}/false'
                )
                try:
