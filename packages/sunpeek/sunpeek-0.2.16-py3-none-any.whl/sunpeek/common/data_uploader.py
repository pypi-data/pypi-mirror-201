...
import sunpeek.common.time_zone
...
class DataUploader_df:
    ...
    plant : Plant
        A configured plant.
    timezone : str
        Optional. Time zone information, if not given in the csv data.
    ...
    def do_upload(self, calculate_virtuals: bool = True):
        ...
        self._post_upload()
        utils.sp_logger.debug(f"[data_uploader] --- Finished after {(time.time() - start_time):.1f} seconds ---")
    ...
    def all_csv_to_single_df(self):
        ...
        """
        utils.sp_logger.debug(f"[data_uploader] Reading csv files to DataFrame.")
        utils.sp_logger.debug(f"[data_uploader] Concatenating {len(self.files)} files.")
        start_time = time.time()
        ...
        for file in self.files:
            ...
            try:
                ...
                if not isinstance(df_all_files.index, pd.DatetimeIndex):
                    raise DataProcessingError('Cannot concatenate DataFrames with mixed timezones since this results in'
                                              ' the DataFrame index not being a DatetimeIndex anymore.')

            finally:
                self.output.response_per_file.append(file_response)

        if df_all_files is None:
            raise DataProcessingError('Reading csv files resulted in a DataFrame with no rows.')
        ...
        index_duplicates = df_all_files.index.duplicated(keep=False)
        self.output.n_duplicates_index = index_duplicates.sum()
        if self.output.n_duplicates_index:
            utils.sp_logger.warning(f"Found {self.output.n_duplicates_index} duplicate index entries in data. "
                                    f"All rows with duplicate index will be removed.")
        df_all_files = df_all_files[~index_duplicates]

        if len(df_all_files) < 2:
            raise DataProcessingError('Reading csv files resulted in a DataFrame with less than 2 rows.')

        utils.sp_logger.debug(
            f"[data_uploader] --- Done parsing {len(self.files)} files in {(time.time() - start_time):.1f} seconds.")
    ...
    def _parse_timestamps(self, bio):
        ...
        ds = sunpeek.common.time_zone.validate_timezone(ds, self.timezone)
...
class DataUploader_db(DataUploader_df):
    ...
    def _pre_upload(self):
        ...
        if table_exists:
            utils.sp_logger.debug(f"[data_uploader] Table {self.table_name} exists in database. Adding columns for any "
                                  f"new sensors for plant {self.plant.name}.")
            self._update_table()
        else:
            utils.sp_logger.debug(f"[data_uploader] Creating table {self.table_name} in database.")
    ...
    def _post_upload(self):
        ...
        utils.sp_logger.debug(f"[data_uploader] Deleting overlapping data from table {self.table_name}.")
        ...
        utils.sp_logger.debug(f"[data_uploader] Writing dataframe to table {self.table_name}...")
        # Timestamps need to solved as DATETIME_COL_NAME col if saved in db
        df = df.rename_axis(
            DATETIME_COL_NAME).reset_index()  # TODO reset_index() not necessary, done twice, see execute_batch
        ...
        if self.output.db_response['measure_data_saved_db_ok']:
            ...
            utils.sp_logger.debug(f"[data_uploader] Data succesfully saved in db table {self.table_name}.")
            hit_db.disconnect_db(self.db_connection)
        else:
            utils.sp_logger.debug(f"[data_uploader] Error writing data to db table {self.table_name}.")
