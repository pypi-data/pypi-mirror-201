    ...
    def gen_code(self,show_notification=False):
        ...
        for filedata in self.entry_data:
            ...
            if "uni-directional" in filedata:
                ...
                commands.append("cond = (isinstance(q,float) or isinstance(q,int))")
                commands.append("if cond: assert not np.isnan(q)")
                # ^ will also pass for np.float64
                commands.append("if cond: assert q < +np.inf")
                commands.append("if cond: assert q > -np.inf")
                ...
                if str(unidir) == "True": # do not do bool(unidir)
                    ...
                    commands.append("if cond: assert q >= 0, 'have to pass positive quantity: unidirectional transaction'")
            ...
            # append insert_link command to code
