...
class Interpolator(ABC):
    ...
    def get_fms(self,f,m,s):        
        #u_f, u_m, u_s = self.get_adjusted_fms(f,m,s)
    ...
    def get_adjusted_fms(self, f, m, s):
        use_f,use_m,use_s = self._F, self._M, self._S
        u_f = f%use_f
        u_m = m%use_m
        u_s = s%use_s
    ...
    def get_adjusted_fmsx(self, f, m, s,F=-1,M=-1,S=-1):                
        use_f,use_m,use_s = self._F, self._M, self._S
        if F+M+S != -3:
            use_f,use_m,use_s = F,M,S        
        u_f,u_m,u_s = f,m,s
    ...
    def get_val_slice(self,unit_coords, deriv = 0):
        ...
        vals = np.zeros(unit_coords.shape)
        ...
        for i in range(int(-1*width/2 + 1), int(width/2 + 1)):
            ...
            for j in range(int(-1*width/2 + 1), int(width/2 + 1)):      
                ...
                for k in range(int(-1*width/2 + 1), int(width/2 + 1)):          
                    ...
                    xp_u, yp_u, zp_u = self.get_adjusted_fms(xp,yp,zp)
                    p = self.get_fms(xp_u, yp_u, zp_u)
                    #print(xp,yp,zp)
                    #vals.append(p)                    
                    arr[count] = p
                    count += 1
    ...
    *******************************************************************************
    ...
    def make_periodic_coeffs(self):
        ...
        if not self.padded:
            ...
            self.create_coeffs_inplace(self._F,self._M,self._S)                    
        else:
            self._coeffs =self.extend_vals_with_buffer(self.buffer)
            #print(self._coeffs)            
            #self.create_coeffs_copy(self._F+2*self.buffer,self._M+2*self.buffer,self._S+2*self.buffer)        
            self.create_coeffs_inplace(self._F+2*self.buffer,self._M+2*self.buffer,self._S+2*self.buffer)        
            #print(self._coeffs)
        ...
        for z in range(buffer):
            for y in range(-1*buffer, self._M+buffer):
                for x in range(-1*buffer, self._F+buffer):
                    xe,ye,ze = self.get_adjusted_fms(x-buffer,y-buffer,z-buffer)
                    p = self.get_fms(xe,ye,ze)
                    padded[x,y,z] = p                            
    ...
    def create_coeffs_inplace(self, thisF, thisM, thisS):
        ...
        for y in range(thisM):        
            for z in range(thisS):                        
                ...
                self.convert_to_interp_coeffs(pole, num_poles, thisF, row)
        #Y-wise
        for x in range(thisF):        
            for z in range(thisS):                        
                ...
                self.convert_to_interp_coeffs(pole, num_poles, thisM, row)
        #Z-wise
        for x in range(thisF):
            for y in range(thisM):                        
    ...
    def create_coeffs_copy(self, thisF, thisM, thisS):
        ...
        for y in range(thisM):        
            for z in range(thisS):        
                row = self.get_row3d(y, z, thisF)
                line = self.convert_to_interp_coeffs(pole, num_poles, thisF, row)
                self.put_row3d(y, z, line, thisF)
        #Y-wise
        for x in range(thisF):        
            for z in range(thisS):        
                row = self.get_col3d(x, z, thisM)
                line = self.convert_to_interp_coeffs(pole, num_poles, thisM, row)
                self.put_col3d(x, z, line, thisM)
        #Z-wise
        for x in range(thisF):
            for y in range(thisM):        
                row = self.get_hole3d(x, y, thisS)
                line = self.convert_to_interp_coeffs(pole, num_poles, thisS, row)
                self.put_hole3d(x, y, line, thisS)
    ...
    def get_row3d(self,y,z,length):        
        row = []
        for x in range(length):
            row.append(self.get_coeff(x, y, z))
        return row
    
    def get_col3d(self,x,z,length):        
        col = []
        for y in range(length):
            col.append(self.get_coeff(x, y, z))
        return col
    
    def get_hole3d(self,x,y,length):        
        bore = []
        for z in range(length):
            bore.append(self.get_coeff(x,y,z))
        return bore
    
    def put_row3d(self,y,z,row,length):        
        for x in range(length):
            self.put_coeff(x, y, z, row[x])
    
    def put_col3d(self,x,z,col,length):        
        for y in range(length):
            self.put_coeff(x, y, z, col[y])
    
    def put_hole3d(self,x,y,bore,length):        
        for z in range(length):
            self.put_coeff(x, y, z, bore[z])
