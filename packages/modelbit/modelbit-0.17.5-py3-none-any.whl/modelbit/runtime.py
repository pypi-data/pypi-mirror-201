...
def addObject(deployment: str, name: str, val: Any):
  dataFiles: Dict[str, str] = {}
  dataFiles[f"data/{name}.pkl"] = uploadRuntimeObject(val, pickle.dumps(val), name)
  return _changeFilesAndDeploy(deployment, dataFiles)


def addFile(deployment: str, local_filepath: str, modelbit_filepath: str):
  from .cli.filter import shouldUploadFile

  dataFiles: Dict[str, str] = {}
  with open(local_filepath, "rb") as f:
    data = f.read()
    dataFiles[modelbit_filepath] = uploadRuntimeObject(None, data, local_filepath) if shouldUploadFile(
        local_filepath, data) else data.decode("utf8")
  return _changeFilesAndDeploy(deployment, dataFiles)
