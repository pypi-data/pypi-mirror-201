"""
https://github.com/rizki4106/django_jwt_middleware/blob/master/src/django_jwt_middleware/source.py
"""
        ...
        def decorator(views_func):
            def container(request, *args, **kwargs):
                ...
                if cookie_name in cookies:
                    ...
                    try:
                        jwt.decode(
                            str(cookies.get(cookie_name)), jwt_key, algorithm
                        )
                        ...
                        response = JsonResponse(
                            data={"status": 403, "message": str(err)}
                        )
                    ...
                    response = JsonResponse(
                        data={
                            "status": 403,
                            "message": "token cookie must be sent in request cookie as  {}".format(
                                cookie_name
                            ),
                        }
                    )
        ...
        def decorator(views_func):
            def container(request, *args, **kwargs):
                ...
                if header_name.lower() in headers:
                    ...
                    try:
                        jwt.decode(
                            str(headers.get(header_name.lower())),
                            jwt_key,
                            algorithm,
                        )
                        return views_func(request, *args, **kwargs)
                    ...
                    response = JsonResponse(
                        data={
                            "status": 403,
                            "message": "token headers must be sent in request header as {}".format(
                                header_name.lower()
                            ),
                        }
                    )
