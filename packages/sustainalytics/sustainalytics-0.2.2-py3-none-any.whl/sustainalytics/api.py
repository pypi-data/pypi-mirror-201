...
class API(object):
        ...
        try:
            ...
            if dtype == 'json':
                temp_data = requests.get('https://api.sustainalytics.com/v2/ReportService',
                                         headers=self.access_headers, params=(('ProductId', productId),),timeout=60).json()
            else:
    ...
    def get_data(self, identifiers, productId=None, packageIds=None, fieldClusterIds=None, dtype='json', fieldIds=None,
                 chunk=10, use_progressbar=True):
        """
        ...
        if productId is None:
            raise ValueError('Please specify productId')
        if isinstance(productId, list) == False:
            raise TypeError('productId must be a list')
        if isinstance(productId, list) == True and len(productId) > 1:
            raise ValueError('Please specify only one productId')

        data_pull_dt = pd.DataFrame()
        data_pull_json = []
        cntr = 0
        chunk_size = chunk
        assert chunk_size <= 10, "Chunk size should be less than or equal to 10."
        assert use_progressbar in [True,False], "Use ProgressBar must be boolean"
        ...
        assert type(identifiers) is list, "Identifiers must be passed as a list of Sustainalytics EntityId, ISINs, CUSIPs etc"
        assert len(identifiers) != 0, "The identifiers list should not be empty"
        ...
        with tqdm(total=len(identiers_group_list),disable=not use_progressbar) as pbar:
            ...
            for i, ids100 in enumerate(identiers_group_list):
                ...
                # ADD THE PRODUCT ID
                if productId is not None and isinstance(productId, list) and len(productId) > 0:
                    productIds_str = ','.join([str(elem) for elem in productId])

                    params = params + (('productId', productIds_str),)
                ...
                try:
                    # Managing Dataframes
                    requests_url = requests.get('https://api.sustainalytics.com/v2/DataService',
