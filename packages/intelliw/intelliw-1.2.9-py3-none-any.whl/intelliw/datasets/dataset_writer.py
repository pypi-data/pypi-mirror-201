...
class DataCommonWriter(AbstractEngineWriter):
    ...
    def _push_to_source(self, df: pd.DataFrame):
        ...
        total = df.shape[0]
        df.insert(0, 'id', self.train_batch+df.index.astype(str))
        split_data = None
        for idx in range(0, total, self.batch_size):
            ts = str(datetime.datetime.now())
            end_idx = min(total, idx+self.batch_size)
            logger.info(
                f"[Output] DataCommonWriter push data index: {idx} to {end_idx}, total: {total}")
            ...
            resp = iuap_request.post_json(
                url=self.output_addr, json=req, timeout=self.request_timeout)
            resp.raise_for_status()
            resp_content = resp.json
            logger.info(
                f"[Output] DataCommonWriter response: {resp_content}")
    ...
    def _push_to_s3(self, df: pd.DataFrame):
        curkey = os.path.join(config.STORAGE_SERVICE_PATH, self.train_batch, "output.csv")
        with tempfile.NamedTemporaryFile() as temp:
            filename = temp.name + '.csv'
            df.to_csv(filename, index=False)
            StorageService(curkey, 'AliOss', "upload").upload(filename)
        return curkey
    ...
    def write(self, data):
        ...
        try:
            ...
            raise OutputRequestException(f"DataCommonWriter push data error:{e}, try to s3: {curkey}")
        return self.write_successed
