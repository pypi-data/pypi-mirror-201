...
EventComposites = Union[
    ...
    composite.EventBertrandComposite,
]
ComponentComposites = Union[
    composite.RiskComponentComposite,
    composite.TextComponentComposite,
]
...
class ConfigProcessor:
    ...
    * a production configuration (self.prod_config) : config which describes
    ...
    def __init__(self, config_filename: Path, rules: str, drafting_datetime: Datetime):
        """__init__
        ...
        if weather["condition"] is not None:
            ...
            new_weather["condition"] = composite.EventComposite(
                field=composite.FieldComposite(
                    file=single_data_config[(file_id, weather["condition"]["field"])][
                        0
                    ]["local"],
                    selection={"slice": {"valid_time": (start_time, stop_time)}},
                    grid_name=single_data_config[
                        (file_id, weather["condition"]["field"])
                    ][0]["geometry"],
                    name=single_data_config[(file_id, weather["condition"]["field"])][
                        0
                    ]["param"],
                ),
