        ...
        if self.grid_name:
            with xr.open_dataset(self.filename) as tmp_ds:
                ...
                if mask_da.dtype == "bool":
                    mask_da = mask_da.where(mask_da > 0)
                for var in opt_das:
    ...
    for var in inter:
        ...
        if lev_inter != set():
            if special_vars[var] == "max":
                dout[var] = xr.ufuncs.fmax(var1, var2.broadcast_like(var1))
...
def change_grid(da: xr.DataArray, grid_out: str) -> xr.DataArray:
    ...
    if da_b.dtype == bool:
        return da_b.astype("int8").interp_like(grid_da).astype("int8").astype("bool")

    return da_b.interp_like(grid_da).astype(da_b.dtype)
