...
class WeatherPeriod(Period):
    """We don't want to describe Periods the same way for Risks and for Weathers
    So the fastest way to do it is to overload the property that loads the templates
    used to describe the periods.
    """
    ...
    def template_retriever(self) -> TemplateRetriever:
        ...
        if not isinstance(self._template_retriever, TemplateRetriever):
            self._template_retriever = read_file(
                TEMPLATES_FILENAMES[Settings().language]["synthesis"]["period"]["short"]
            )
        return self._template_retriever
