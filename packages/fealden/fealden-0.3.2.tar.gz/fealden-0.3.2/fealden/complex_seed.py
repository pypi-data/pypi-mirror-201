...
class Seed:
    ...
    @staticmethod
    def _make_seed_format(dots: str) -> list[str]:
        """Helper function for create_complex_seed, makes a seed_graph format"""
        dot_pattern = ["".join(num) for num, _ in groupby(dots)]
        dot_length = len(dot_pattern)
        out_list: list[str] = []
        ss_count = 1
        ds_count = 2
        for i, patt in enumerate(dot_pattern):
            if patt == "." and not out_list:
                out_list.append(f"{ss_count} 0 {ds_count}")
                # ss_count += 2
            elif patt == "(":
                out_list.append(
                    f"{ds_count} {ss_count} {ss_count+2} {ss_count+2} {ss_count+4}"
                )
                # ds_count += 2
                ss_count += 2
            elif patt == "." and out_list:
                if i == dot_length - 1:
                    out_list.append(f"{ss_count} {ds_count} 0")
                else:
                    out_list.append(f"{ss_count} {ds_count} {ds_count}")
                    ss_count += 2

        return out_list

    @classmethod
    def create_from_dot_bracket(cls, seq: str, dots: str) -> Seed:
        """Create a multi-Node starting seed based on a user-supplied sequence and
        dot-bracket representation.

        This implementation should allow better handling of complex aptamer
        starting sequences.

        Args:
            seq (str): input sequence, e.g. CACGTG
            dots (str): input dot-bracket sequence, e.g.  ...((..))....

        Returns:
            Seed: a Seed class members with pre-populated nodes
        """

        dot_pattern = ["".join(num) for num, _ in groupby(dots)]
        dots_pattern = ["".join(grp) for _, grp in groupby(dots)]
        seed_in = cls._make_seed_format(dots)
        my_nodes = cls.make_graph(cls, seed_in, node.SSNode(None), {}, "", "")
        try:
            del my_nodes["0"]
        except KeyError:
            pass

        dot_lengths = [len(item) for item in dots_pattern]
        seq_chunks = [
            seq[sum(dot_lengths[:i]) : sum(dot_lengths[: i + 1])]
            for i in range(len(dot_lengths))
        ]

        for i, (patt, chunk) in enumerate(zip(dot_pattern, seq_chunks)):
            if patt == ".":
                print(repr(my_nodes[f"{i+1}"]))
                my_nodes[f"{i+1}"].set_seq(chunk)
                print(repr(my_nodes[f"{i+1}"]))
            elif patt == "(":
                my_nodes[f"{i+1}"].set_seq(chunk)
