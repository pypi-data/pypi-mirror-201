...
class SarusJSONEncoder(json.JSONEncoder):
    ...
    def encode_obj(self, obj: Any) -> Any:
        if isinstance(obj, tuple):
            ...
            return {k: self.encode_obj(v) for k, v in obj.items()}
        return obj
...
class SarusJSONDecoder(json.JSONDecoder):
        ...
        if isinstance(obj, dict):
            if '_type' in obj:
                ...
                if obj['_type'] == 'tuple':
                    ...
                    cls = getattr(sys.modules[data['module']], data['name'])
                    return cls
                elif obj['_type'] == 'dtype':
                    return pd.api.types.pandas_dtype(data)
                elif obj['_type'] == 'slice':
                    return slice(*data)
            return {k: self.decode_obj(v) for k, v in obj.items()}
        elif isinstance(obj, list):
    ...
    def decode_bytes(cls, b: bytes, *args: Any, **kwargs: Any) -> Any:
        decoder = cls()
        obj = decoder.decode(b.decode('utf-8'), *args, **kwargs)
        return decoder.decode_obj(obj)
