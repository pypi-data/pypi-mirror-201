...
class tenant_context(ContextDecorator):
    ...
    def get_descendants(self):
        descendants = self.tenant.get_descendants(include_self=True)
        return [str(descendant.pk) for descendant in descendants]

    def __enter__(self):
        self.connection = connections[get_tenant_database_alias()]
        self.previous_tenant = connection.tenant
        self.previous_descendants = connection.descendants
        self.connection.set_tenant(self.tenant)
        self.connection.set_tenant_descendants(self.get_descendants())

    def __exit__(self, *exc):
        self.connection.set_tenant(self.previous_tenant)
        self.connection.set_tenant_descendants(self.previous_descendants)
