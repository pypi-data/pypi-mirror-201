...
from deskaone_sdk.Utils import Crypto, __version_code__
from deskaone_sdk.Internal import Internal
...
class __Auth__(AuthBase):
    ...
    def __call__(self, request):
        ...
        request.headers.update({
            'User-Agent'    : f'DesKaOne-Scrypt-V3@{__version_code__}',
            'SIGNATURE'     : signature,
            'TIMESTAMP'     : timestamp,
        ...
        try:
            ...
            if type(DATA) == dict:
                if DATA.get('COMMAND') == 'INTERNAL':
                    ...
                    RESULT = dict(STATUS = True, MESSAGE = REQUEST.get('MESSAGE'), DATA = Result, IP = REQUEST.get('IP'))
                else: RESULT = REQUEST
            else: RESULT = REQUEST
            return RESULT
