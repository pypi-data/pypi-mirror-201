    ...
    for message in messages:
        ...
        if message_type == 'RECORD':
            ...
            try:
                rec = o['record']
                try:
                    logger.info("Debug Record before %s", rec)
                    ...
                    if kps:
                        logger.info("Debug Key Properties %s", kps[0])
                        if len(kps) > 1:
                    ...
                    if _key:
                        rec['_key'] = _key
                    logger.info("Debug Record is %s", rec)
                except Exception as e:
