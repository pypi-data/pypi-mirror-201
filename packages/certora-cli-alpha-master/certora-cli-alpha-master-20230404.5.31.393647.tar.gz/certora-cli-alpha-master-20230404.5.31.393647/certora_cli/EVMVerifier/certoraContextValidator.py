...
def get_conf_key(arg: Attr.ContextAttribute) -> str:
    dest = arg.value.get_dest()
    return dest if dest is not None else arg.name.lower()
...
class CertoraContextValidator:
    ...
    def validate(self) -> None:
        ...
        for arg in Attr.ContextAttribute:
            conf_key = get_conf_key(arg)
            attr = getattr(self.context, conf_key, None)
            if attr is None:
                continue
            if Util.is_new_api() and arg.value.arg_status == Attr.ArgStatus.DEPRECATED:
            ...
            elif arg.value.arg_type == Attr.AttrArgType.USER_DEFINED:
                self.validate_type_user_defined(arg)
            else:
    ...
    def validate_type_user_defined(self, attr: Attr.ContextAttribute) -> None:
        conf_key, value = self.__get_key_and_value(attr)

        if value is None:
            raise RuntimeError(f"calling validate_type_user_defined with null value {conf_key}")
    ...
    def validate_has_value(self, key: str) -> None:
        if not hasattr(self.context, key):
            raise Util.CertoraUserInputError(f"{key} must be set")

    def validate_optimize_map(self) -> None:
        conf_key, value = self.__get_key_and_value(Attr.ContextAttribute.OPTIMIZE_MAP)
        value = getattr(self.context, conf_key, None)
        if value is None:
            return

        values = value.values()
        for contract, num_runs in value.items():
            Vf.validate_non_negative_integer(num_runs)

        if len(set(values)) == 1:
            validation_logger.warning(f"All contracts are optimized for the same number of runs in --optimize_map."
                                      f" --optimize {list(values)[0]} can be used instead")

    def validate_solc_map(self) -> None:
        if not hasattr(self.context, Attr.ContextAttribute.SOLC_MAP.name):
            return
        attr = self.context.solc_map
        values = attr.value.values()
        for source_file, solc_file in attr.value.items():
            Vf.is_solc_file_valid(solc_file)  # raises an exception if file is bad
        if len(set(values)) == 1:
            validation_logger.warning(f"All Solidity source files will be compiled with the same Solidity compiler"
                                      f" in --solc_map. --solc {list(values)[0]} can be used instead")
...
def sort_deduplicate_list_args(context: CertoraContext) -> None:
    ...
    for arg_name in vars(context):
        ...
        if isinstance(arg_val, list) and len(arg_val) > 0:
            setattr(context, arg_name, __sort_dedup_list_attr(arg_val, arg_name))


def __sort_dedup_list_attr(arg_list: List[str], arg_name: str) -> List[str]:
    """
...
def check_packages_arguments(context: CertoraContext) -> None:
    ...
    if context.packages is not None and len(context.packages) > 0:
        ...
        if not Util.PACKAGE_FILE.exists():
            validation_logger.warning(
                f"Default package file {Util.PACKAGE_FILE} not found, external contract dependencies could be "
                f"unresolved. Ignore if solc invocation was successful")
        elif not os.access(Util.PACKAGE_FILE, os.R_OK):
            validation_logger.warning(f"No read permissions for default package file {Util.PACKAGE_FILE}")
        else:
            try:
...
def set_domain(context: CertoraContext) -> None:
    ...
    if context.staging is not None:
        context.domain = Util.STAGING_DOMAIN
    else:
        context.domain = Util.PRODUCTION_DOMAIN
    ...
    if commit_hash:
        if context.local:
            raise Util.CertoraUserInputError("Cannot run on a specific commit in local runs. "
                                             "Did you mean to use --staging?")
        elif context.domain == Util.PRODUCTION_DOMAIN:
            raise Util.CertoraUserInputError("Cannot run on a specific commit in production. "
                                             "Did you mean to use --staging?")
        elif context.staging:  # A non-empty string
            raise Util.CertoraUserInputError(f"Cannot run on both a specific branch {context.staging} "
