...
DASHBOARD_URL = "https://prover.certora.com"
...
def get_package_and_version() -> Tuple[str, str]:
    ...
    package_dict: Dict[str, Any] = pkg_resources.working_set.by_key  # type: ignore[attr-defined]
    distribution = package_dict.get(PRODUCTION_PACKAGE_NAME, None)
    if distribution is None:
        for python_installed_package in package_dict.keys():
            if python_installed_package.startswith(DEV_PACKAGE_NAME_PREFIX):
                distribution = package_dict[python_installed_package]
                break
...
def find_jar(jar_name: str) -> Path:
    ...
    return Path(__file__).parent.parent.parent / "certora_jars" / jar_name
...
def run_local_spec_check(with_typechecking: bool) -> int:
    ...
    path_to_typechecker = find_jar("Typechecker.jar")
