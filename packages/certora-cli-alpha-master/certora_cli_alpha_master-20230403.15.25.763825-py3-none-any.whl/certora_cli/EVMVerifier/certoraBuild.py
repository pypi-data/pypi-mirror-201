...
class CertoraBuildGenerator:
    ...
    def get_contract_in_sdc(self,
        ...
        if compiler_lang == CompilerLangSol():
            ...
            lang = "Vyper"
            types = []
        funcs = self.collect_funcs(contract_data, source_code_file, contract_name, build_arg_contract_file,
                                   compiler_lang, types)
                ...
                for storage_slot in declarations:
                    if type(storage_slot) is dict and type(storage_slot.get("astId", None)) is int:
                        ...
                        if type(node) is dict and "typeName" in node:
                            ...
                            type_instance = descriptor
                            storage_slot["descriptor"] = type_instance.as_dict()
    ...
    def instrument_imports(self, build_arg_contract_file: str, instr: Dict[str, Dict[int, Instrumentation]]) -> bool:
        instrumentation_logger.debug(f"build files are {self.asts[build_arg_contract_file].keys()}")
        files_to_importers: Dict[str, List[Tuple[str, int]]] = dict()
        instrumentation_logger.debug(f"Rewriting imports for contract {build_arg_contract_file}")
        instrumentation_logger.debug(f"The instrumented files are: {instr.keys()}")
        for f_name, nodes in self.asts[build_arg_contract_file].items():
            instrumentation_logger.debug(f"Visiting file {f_name}")
            for node_id, node in nodes.items():
                ...
                instrumentation_logger.debug(f"Found import node, absolute path reported to be {path}")
                if not isinstance(path, str):
                    instrumentation_logger.debug(f"{path} for import {node} in {f_name} is not a dict")
                    return False
                path = CertoraBuildGenerator.normalize_path(path)
                instrumentation_logger.debug(f"path resolved to be {path}")
                if path not in files_to_importers:
                    files_to_importers[path] = []
                files_to_importers[path].append((f_name, node_id))
            instrumentation_logger.debug(f"Done with {f_name}")
        imports_to_rewrite: List[Tuple[str, int]] = []
        visited: Set[str] = set()
        worklist = list(instr.keys())
        i = 0
        instrumentation_logger.debug(f"Computing import closure for {build_arg_contract_file}")
        while i < len(worklist):
            item = CertoraBuildGenerator.normalize_path(worklist[i])
            instrumentation_logger.debug(f"Visiting file {item} at iteration {i}")
            i = i + 1
            if item in visited:
                instrumentation_logger.debug("Has already been visited, skipping")
                continue
            visited.add(item)
            imp = files_to_importers.get(item, None)
            if imp is None:
                instrumentation_logger.debug(f"no importers for {item}")
                continue
            instrumentation_logger.debug(f"These files are reported to import {item}: {imp}")
            imports_to_rewrite += imp
            for (nxt_file, _) in imp:
                if nxt_file in visited:
                    continue
                worklist.append(nxt_file)
        instrumentation_logger.debug(f"import closure computation complete, we have {imports_to_rewrite}")
        for file_name, import_id in imports_to_rewrite:
            ...
            instrumentation_logger.debug(f"In {file_name}, rewriting node {import_node} to point to {new_path}")
    ...
    def instrument_auto_finders(self, build_arg_contract_file: str, i: int, sdc_pre_finders: List[SDC]) -> \
        ...
        for contract_file, instr_loc in instr.items():
            ...
            if not self.context.debug:
                self.generated_tmp_files.add(new_abs_path)
