...
class SPSolarGainCalibration(CryonirspTaskBase):
                    ...
                    with self.apm_processing_step(f"Reshifting gain spectra for {apm_str}"):
                        ...
                        reshifted_gain = next(
                            self.corrections_remove_spec_geometry(
                                arrays=gain, spec_shift=-spec_shift
                            )
                        )
                        self.intermediate_frame_write_arrays(
    ...
    def do_initial_corrections(self, beam: int, exposure_time: float) -> None:
        ...
        spectral_corrected_solar_array = next(
            self.corrections_remove_spec_geometry(geo_corrected_solar_array, spec_shift)
        )
