...
class CorrectionsMixin:
    ...
    def corrections_correct_geometry(
        ...
        for array in arrays:
            # NB: Added to fix endianness issue with arrays embedded in generator
    ...
    def corrections_correct_bad_pixels(
        ...
        ndarray
            The corrected array
        """
        kernel_size = self.parameters.bad_pixel_correction_median_filter_size
        half_kernel_size = kernel_size // 2

        # Create a masked array from the input and the bad pixel map
        masked_array = np.ma.array(array_to_fix, mask=bad_pixel_map)

        # Create an iterable of (y, x) bad pixel locations
        ...
        for x, y in zip(x_locs, y_locs):
            ...
            # If this is SP, restrict filtering to be along the slit (Y axis)
            # This is to not cause blurring of  the spectral lines
