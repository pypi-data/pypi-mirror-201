...
class GainCalibrationBase(CryonirspTaskBase):
    ...
    def run(self):
        ...
        with self.apm_task_step(
            ...
            for exp_time in self.exposure_times:
                # NB: By using beam = 1 for CI, this method works for both CI and SP
                for beam in range(1, self.constants.num_beams + 1):
                    apm_str = f"{beam = } and {exp_time = }"
                    with self.apm_processing_step(f"Computing gain array for {apm_str}"):
                        logger.info(f"Loading dark array for beam {beam}")
    ...
    def compute_average_gain_array(
        ...
        np.ndarray
        """
        # Get the linearized gain arrays
