...
class SPGeometricCalibration(CryonirspTaskBase):
    ...
    def reference_array(self) -> np.ndarray:
        """Reference array used for determining the offset of beam 2 relative to beam 1."""
        if self._reference_array is None:
            raise ValueError("Reference array has not been set. This should never happen.")
        return self._reference_array

    @reference_array.setter
    def reference_array(self, array: np.ndarray) -> None:
        """Set the reference array used for determining the offset of beam 2 relative to beam 1."""
        self._reference_array = array
    ...
    def basic_corrected_data(self, beam: int) -> np.ndarray:
        ...
        array_generator = self.intermediate_frame_load_intermediate_arrays(tags=tags)
        # If there are multiple arrays, then average them, otherwise return the single array
    ...
    def offset_corrected_array_generator(self, beam: int) -> Generator[np.ndarray, None, None]:
        """
        All arrays for a single beam that have had their beam offset corrected.
        ...
        Generator[np.ndarray, None, None]
            Generator of beam offset corrected arrays
    ...
    def compute_beam_angle(self, beam: int) -> float:
        """
        Compute the rotational offset of the specified beam.

        Parameters
        ----------
        beam
            The number of the beam whose image rotation angle we are calculating

        Returns
        -------
        The beam rotation angle
        ...
        2. Compute a gradient array by shifting the array along the spatial axis (along the slit) and
            calculating a normalized finite difference with the original array. The amount to shift is
            nominally 4 pixels and is accessed from the parameter db.
        3. Compute the location of two lines parallel to the spatial axis that split the image into three
            equal-spectral_width spectral sections. These imaginary lines will become the center of two spatial strips
            over which we will perform additional calculations.
        4. Compute the bounding corners of the spatial strips
        5. Extract the spatial strips as arrays and compute the median values along their spectral axis.
        6. Compute the relative shift of the right strip to the left strip (this is the shift along the spatial axis)
        7. Compute the angular rotation of the beam relative to the array axes from the shift
            and the separation of the strips along the spectral axis
        8. Return the angular rotation

        """
        # Load the lamp gain array
        beam_array = next(
            self.intermediate_frame_load_intermediate_arrays(
                tags=[CryonirspTag.task_lamp_gain(), CryonirspTag.beam(beam)]
            )
        )

        rv = self.parameters.geo_gradient_displacement  # gradient displacement in pixels
        bgrad = (np.roll(beam_array, rv, axis=0) - np.roll(beam_array, -rv, axis=0)) / (
            np.roll(beam_array, rv, axis=0) + np.roll(beam_array, -rv, axis=0)
        )

        # Locate portions of the beam over which to extract two long narrow strips along the spatial axis
        # spatial_length is the length in pixels of the spatial strips (along the slit)
        # We use one half of the slit length (the size of the array spatial axis) as the spatial length of the strips
        divisor = self.parameters.geo_strip_spatial_length_divisor
        spatial_length = math.ceil(beam_array.shape[0] / divisor)
        if spatial_length % 2:
            spatial_length += 1
        # half_spatial_length is the extent above and below the center of the spatial axis
        # (i.e., the strips are centered about the middle of the slit length)
        half_spatial_length = spatial_length // 2
        # spectral_width is the width of each strip along the spectral axis
        # A nominal value of 20 in the divisor gives us ~900/20 = 45
        divisor = self.parameters.geo_strip_spectral_width_divisor
        spectral_width = math.ceil(beam_array.shape[1] / divisor)
        if spectral_width % 2:
            spectral_width += 1
        half_spectral_width = spectral_width // 2
        # spectral_offset is the distance along the spectral axis from the center of the image
        # to the center of each strip
        divisor = self.parameters.geo_strip_spectral_offset_divisor
        spectral_offset = math.ceil(beam_array.shape[1] / divisor)
        logger.info(f"{spatial_length = }, {half_spectral_width = }, {spectral_offset = }")
        # (r0, r1) define the min and max spatial extent of the strips
        r0, r1 = np.array([-half_spatial_length, half_spatial_length]) + bgrad.shape[0] // 2
        # (c0l, c1l) define the min and max spectral extent of the left strip
        c0l, c1l = (
            np.array([-half_spectral_width, half_spectral_width])
            + bgrad.shape[1] // 2
            - spectral_offset
        )
        # (c0r, c1r) define the min and max spectral extent of the right strip
        c0r, c1r = (
            np.array([-half_spectral_width, half_spectral_width])
            + bgrad.shape[1] // 2
            + spectral_offset
        )
        # extract the strips and then compute the median along the rows of each
        xl = np.nanmedian(bgrad[r0:r1, c0l:c1l], axis=1)
        xr = np.nanmedian(bgrad[r0:r1, c0r:c1r], axis=1)
        # xl and xr are two 1d signals that represent the deterministic intensity variations
        # along the slit, due to physical imperfections in the slit, grating and sensor
        # The relative shift between xl and xr is then used to compute the angle of the slit
        # with respect to the array axes

        # determine the shift using a cross correlation
        # phase_cross_correlation(reference_image, moving_image, upsample_factor=n)
        # the method returns the amount by which the moving_image (xr) must be shifted
        # to align the two signals
        upsample_factor = self.parameters.geo_upsample_factor
        shifts, error, phasediff = skir.phase_cross_correlation(
            xl, xr, upsample_factor=upsample_factor
        )
        # use additive inverse of shifts to convert from correction to measurement
        shift = -shifts[0]
        logger.info(f"Measured shift of beam {beam} = {shift}")
        # tilt_angle represents the clockwise rotation of the beam
        tilt_angle = np.arctan(shift / (2 * spectral_offset))
        logger.info(f"Measured angle for beam {beam} = {np.rad2deg(tilt_angle):0.3f} deg")
    ...
    def compute_spectral_shifts(self, beam: int) -> np.ndarray:
        ...
        num_spec = avg_beam_array.shape[0]
