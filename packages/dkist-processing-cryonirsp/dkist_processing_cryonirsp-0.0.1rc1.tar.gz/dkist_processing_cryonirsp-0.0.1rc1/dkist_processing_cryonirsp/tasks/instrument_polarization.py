...
class InstrumentPolarizationCalibrationBase(CryonirspTaskBase, ABC):
    ...
    def run(self) -> None:
        ...
        # These calls calculate and save dark/gain arrays for all required exposure times
        self.perform_dark_calibration(target_exp_times)
        self.perform_gain_calibration(target_exp_times)
        ...
        for modstate in range(1, self.constants.num_modstates + 1):
            for exp_time in self.constants.polcal_exposure_times:
                ...
                try:
                    ...
                    raise ValueError(f"No matching dark array found for {exp_time = } s")
                ...
                try:
                    ...
                    raise ValueError(f"No matching gain array found for {exp_time = } s")
    ...
    def reduce_single_step(
        ...
        with self.apm_processing_step(f"Extract macro pixels from {apm_str}"):
            ...
            output_shape = (
                ...
                self.parameters.polcal_num_spectral_bins,  # correct axis ordering here?
            )
    ...
    def reshape_demod_matrices(self, demod_matrices: np.ndarray) -> np.ndarray:
        ...
        target_shape = (
            self.single_beam_shape + demod_shape
        )  # can we keep this single_beam_shape in here
    ...
    def perform_dark_calibration(self, target_exp_times):
            ...
            for exp_time in target_exp_times:
                for beam in range(1, self.constants.num_beams + 1):
                    ...
                    with self.apm_processing_step(
                        ...
                        # NB: I am saving the file here for debug purposes
    ...
    def perform_gain_calibration(self, exp_times):
        ...
        with self.apm_task_step(f"Generate gains for {len(exp_times)} exposure times"):
            for exp_time in exp_times:
                for beam in range(1, self.constants.num_beams + 1):
                    ...
                    # Get the linearity corrected gain arrays
                    logger.info(
...
class SPInstrumentPolarizationCalibration(InstrumentPolarizationCalibrationBase):
    ...
    def record_polcal_quality_metrics(
        ...
        self.quality_store_polcal_results(
            ...
            bins_1=self.parameters.polcal_num_spatial_bins,  # order correct?
