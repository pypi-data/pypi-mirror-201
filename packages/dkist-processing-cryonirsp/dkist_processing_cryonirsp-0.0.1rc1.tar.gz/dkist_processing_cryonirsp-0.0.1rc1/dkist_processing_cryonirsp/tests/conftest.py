...
class CryonirspHeaders(Spec122Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.add_constant_key("WAVELNTH", 1082.00)
        # num modstates
...
@pytest.fixture()
def cryonirsp_dataset() -> fits.Header:
    """
...
@pytest.fixture()
def calibrated_cryonirsp_dataset(cryonirsp_dataset) -> fits.Header:
    """
    Same as cryonirsp_dataset but with a DATE-END key.

    Because now that's added during ScienceCal
    """
    cryonirsp_dataset["DATE-END"] = (
        Time(cryonirsp_dataset["DATE-BEG"], format="isot", precision=6)
        + TimeDelta(float(cryonirsp_dataset["TEXPOSUR"]) / 1000, format="sec")
    ).to_value("isot")

    return cryonirsp_dataset
...
def _create_parameter_files(param_path: Path) -> None:
    thresh = np.ones((10, 10), dtype=np.double) * 100.0
    polyfit = np.array([0, 0, 0, 1], dtype=np.double)
    np.save(os.path.join(param_path, "ci_thresholds.npy"), thresh)
    np.save(os.path.join(param_path, "sp_thresholds.npy"), thresh)
    np.save(os.path.join(param_path, "ci_polyfit.npy"), polyfit)
    np.save(os.path.join(param_path, "sp_polyfit.npy"), polyfit)


T = TypeVar("CryonirspTestingParameters")
...
def cryonirsp_testing_parameters_factory(
    ...
    class CryonirspTestingParameters:
        ...
        cryonirsp_geo_gradient_displacement: int = 4
        cryonirsp_geo_strip_spatial_length_divisor: int = 2
        cryonirsp_geo_strip_spectral_width_divisor: int = 20
        cryonirsp_geo_strip_spectral_offset_divisor: int = 4
        cryonirsp_solar_spectral_avg_window: WavelengthParameter = WavelengthParameter(
            values=(50, 50, 60, 110)
        )
        cryonirsp_solar_zone_prominence: WavelengthParameter = WavelengthParameter(
            values=(0.5, 0.5, 0.5, 0.5)
        )
        cryonirsp_solar_zone_width: WavelengthParameter = WavelengthParameter(values=(3, 3, 3, 3))
        cryonirsp_solar_zone_bg_order: WavelengthParameter = WavelengthParameter(
            values=(5, 5, 5, 5)
        )
        cryonirsp_solar_zone_normalization_percentile: WavelengthParameter = WavelengthParameter(
            values=(90, 90, 90, 90)
        )
        cryonirsp_solar_zone_rel_height: float = 0.91
        ...
        cryonirsp_bad_pixel_map_median_filter_size: int = 20
        cryonirsp_bad_pixel_map_threshold_factor: float = 5.0
        cryonirsp_bad_pixel_correction_median_filter_size: int = 8
        cryonirsp_ci_thresholds: FileParameter = FileParameter(
            param_path=str(absolute_path / "ci_thresholds.npy")
        )
        cryonirsp_ci_polyfit: FileParameter = FileParameter(
            param_path=str(absolute_path / "ci_polyfit.npy")
        )
        cryonirsp_sp_thresholds: FileParameter = FileParameter(
            param_path=str(absolute_path / "sp_thresholds.npy")
        )
        cryonirsp_sp_polyfit: FileParameter = FileParameter(
            param_path=str(absolute_path / "sp_polyfit.npy")
        )
