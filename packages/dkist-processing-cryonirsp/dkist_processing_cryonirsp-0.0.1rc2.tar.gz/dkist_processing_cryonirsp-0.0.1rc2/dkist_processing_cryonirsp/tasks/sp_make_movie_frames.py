...
class SPMakeCryonirspMovieFrames(CryonirspTaskBase, FitsDataMixin):
    """
    ...
    def run(self):
        ...
        For each stokes state.
            For each map_scan
              - A full scan is used to create a single movie frame
            ...
            with self.apm_processing_step(f"Making movie frame for {map_scan = } "):
                ...
                for stokes_state in stokes_states:
                    ...
                    if len(stokes_paths) > 0:
                        # Loop over the scan steps in a single map_scan
                        for scan_step in range(1, self.constants.num_scan_steps + 1):
                            ...
                            if self.constants.num_scan_steps == 1:
                                logger.info(
                                    "Only a single scan step found. Making a spectral movie."
                                )
