...
def test_sp_science_calibration_task(sp_science_calibration_task, mocker):
    ...
    for file in files:
        ...
        assert hdul[1].header["CNNUMSCN"] == 2
        assert hdul[1].header["CNCURSCN"] == scan_step

        # Check that WCS keys were updated
        if offset[0] > 0:
            assert hdul[1].header["CRPIX2"] == og_header["CRPIX2"]
        if offset[1] > 0:
            assert hdul[1].header["CRPIX1"] == og_header["CRPIX1"]
