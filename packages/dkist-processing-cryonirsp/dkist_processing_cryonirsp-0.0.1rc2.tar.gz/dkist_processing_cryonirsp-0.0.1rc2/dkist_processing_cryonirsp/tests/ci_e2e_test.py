...
from dkist_processing_cryonirsp.tasks.beam_boundaries import BeamBoundariesCalibration
from dkist_processing_cryonirsp.tasks.ci_make_movie_frames import CIMakeCryonirspMovieFrames
...
def translate_122_to_214_task(suffix: str):
    class Translate122To214L0(WorkflowTaskBase):
        def run(self) -> None:
            raw_dir = Path(self.scratch.scratch_base_path) / f"CRYO{self.recipe_run_id:03n}"
            if not os.path.exists(self.scratch.workflow_base_path):
                os.makedirs(self.scratch.workflow_base_path)

            if not raw_dir.exists():
                raise FileNotFoundError(
                    f"Expected to find a raw CRYO{{run_id:03n}} folder in {self.scratch.scratch_base_path}"
                )
...
def main(
    ...
    with ManualProcessing(
        ...
        manual_processing_run.run_task(task=BeamBoundariesCalibration)
