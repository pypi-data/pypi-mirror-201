...
class gPaIRS(QMainWindow):
    def keyPressEvent(self, event):
        if event.key()==Qt.Key.Key_W and event.modifiers().value==Qt.ControlModifier.value:
            self.raise_()
            self.activateWindow()
            w=self.focusWidget()
            if w: w.clearFocus()
    ...
    """ #myToolButtonIcon MIN and PIV
    def keyPressEvent(self, event):
        if event.key() == QtCore.Qt.Key_Control:
            super().keyPressEvent(event) #to preserve classical behaviour before adding the b
            self.w_Tree.keyPressEvent(event)

    def keyReleaseEvent(self, event):
        if event.key() == QtCore.Qt.Key_Control:
            super().keyReleaseEvent(event) #to preserve classical behaviour before adding the b
            self.w_Tree.keyReleaseEvent(event)
    """ 
    ...
    def __init__(self,flagDebug=False,app=None):
        ...
        #self.w_Import.signals.update_par.connect(self.updateCurrentItem)
        #------------------------------------- Callbacks 
        self.setupCallbacks()
        ...
        if os.path.exists(lastcfgname):
            ...
            if not Flag:
                ...
                try:
                    ...
                    self.warningDialog(WarningMessage,flagScreenCenter=True)
...
#********************************************* Selection of items
    ...
    def updateGuiFromTree(self,tree,item,column):
        ...
        '''
        # TBD TA cancellare assolutamente
                tree,_=self.w_Tree.pickTree(self.w_Tree.TREpar.indTree)
...
#********************* File
    def new_uicfg(self):
        if self.cfgname!=lastcfgname:
            ...
            FlagYes=self.questionDialog(Question)
        if FlagYes:
            self.save_uicfg()
        self.reconfigure()
    ...
    def saveas_uicfg(self,filename='',Title="Select location and name of the project file to save"):
        ...
        if filename:
            ...
            try:
                ...
                self.warningDialog(f'Error while saving the configuration file {filename}!\nPlease, retry.')
        return
    ...
    def import_uicfg(self,filename,WarningMessage):
        ...
        try:
            ...
            if Flag:
                ...
                self.warningDialog(WarningMessage+WarningMessage2)
        except Exception as inst:
...
#********************* Help
    ...
    def about(self):
        ...
        mailString=f'<a href="mailto:{mail}"><span style=" text-decoration: underline; color:#0000ff;">{mail}</span></a></p></body></html>'
...
#********************************** Launching
    ...
    def updateIndProc(self):
        ...
        data.resetLog()
        self.w_Log.moveToBottom()
        data.flagRun=data.VIS.Tre.flagRun=-1  #come ultimo o comunque dopo resetProc
    ...
    def pause_proc(self,data:dataTreePar,errMessage=''):
        ...
        if data.FlagFinished or errMessage!='': #save and continue 
            if not self.FlagRun: # bug noto se uno mette in pausa mentre sta finendo di salvare (al 100%) da errore, basta ripremere play oppure eliminare il check
                pri.Process.red('****************************           -------    pause_MINproc Error ****************************   -------')
            else:
                self.nextProc()
        else:
            if data.hasIndexOf(self.w_Tree.TABpar):
    ...
    def setButtonLayout(self):
        ...
        for k,tn in enumerate(self.optabnames):
            ...
            if flag:
                ...
                s='<sup><span style=" font-size:11pt;">🔒</span></sup>'
        ...
        if len(args):
            ...
            if self.GPApar.FlagUndocked:
                ...
                if all([not getattr(self.GPApar,'Flag'+tn) for tn in self.optabnames]):
                    s=max([self.main_splitter.sizes()[0],self.minW_ManTabs])
...
def launchPaIRS(flagDebug=False,flagInputDebug=False):
    ...
    gui.show()
    if splash: QTimer.singleShot(time_showSplashOnTop,splash.hide)
    print('\nWelcome to PaIRS!\nEnjoy it!')
    app.exec()
    return [app,gui]
...
def setAppGuiPalette(self:gPaIRS,palette):
