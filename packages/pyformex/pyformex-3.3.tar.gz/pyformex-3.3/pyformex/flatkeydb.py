...
class FlatDB(dict):
    ...
    def record_error_handler(self, record):
        ...
        This method can safely be overriden in subclasses.
        """
        raise ValueError(f"FlatDB: invalid record : {record}")
    ...
    def key_error_handler(self, key):
        ...
        This method can safely be overriden in subclasses.
        """
        raise ValueError(f"FlatDB: duplicate key : '{key}'")
        ...
        if w == self.endrec:
            ...
            if self.record is None:
                if self.beginrec == '':
                    ...
                    self.error_msg = f"Unrecognized line '{line}'"
                    return 1
    ...
    def parse(self, lines, ignore=False, filename=None):
        ...
        for line in lines:
            ...
            if self.parseLine(line) != 0 and not ignore:
                raise RuntimeError(
                    f"FlatDB: error while reading line {linenr}"
                    f" of database (File: {filename})\n{self.error_msg}")
    ...
    def match(self, key, value):
        ...
        """
        return [
            i for i in self.keys() if key in self[i] and self[i][key] == value
        ]
...
if __name__ == '__main__':
    ...
    import os
    matdb = os.path.dirname(__file__) + '/data/materials.db'
    if not os.path.exists(matdb):
        raise ValueError(f"File does not exist: {matdb}")
    ...
    print(js)
    with open('materials.json', 'w') as fil:
        fil.write(js)
    ...
    mat.readFile(matdb, ignore=True)
    print(mat)
