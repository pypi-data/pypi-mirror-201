...
def createFrameModel():
    ...
    cnt, ind, xbin = histogram2(faces.reshape(-1), np.arange(faces.max()+1))
...
def create_menu(before='help'):
    ...
    return menu.Menu('Hesperia', items=MenuData, parent=pf.GUI.menu, before=before)
...
def show_menu():
    ...
    if not pf.GUI.menu.item('Hesperia'):
        create_menu()
        showInfo("The Hesperia menu is now loaded")
