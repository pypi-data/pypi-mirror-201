...
def sliceIt():
    ...
    with busyCursor():
        ...
        t0= np.array([h[ibr][sbr][0].coords[0] for ibr in [0, 1, 2] for sbr in [0, 1]]).mean(axis=0)
        t1= np.array([h[ibr][sbr][0].coords[-1] for ibr in [0, 1, 2] for sbr in [0, 1]]).mean(axis=0)
        for ibr in [0, 1, 2]:
            for sbr in [0, 1]:
...
def seeding3zones(nseeds=[10, 10], zonesizes=[0.3, 0.3]):
    ...
    seed0, seed1= [np.arange(nseeds[i]+1.)/float(nseeds[i])*zonesizes[i] for i in range(2)]
...
def drawLongitudinalSeeding(H, at):
    for i, j in zip([0, 2, 4], range(3)):  # semibranch,at
        ...
        SA = [draw(
            divPolyLine(l0, at[j][i]), color=c, marksize=7, flat=True, alpha=1)
              for i,c in ((0, 'green'), (1, 'yellow'), (2, 'red'))]
    zoomAll()
    return SA
...
def meshBranch(HC, OC, nlong, ncirc, ntr, nbl):
    ...
    nq = Coords(np.column_stack([nq, ones([len(nq)])]))
...
def inputMeshingParameters():
    ...
    """
    dialog = Dialog(
        caption='BifMesh meshing parameters', store="bifmesh_meshing_params",
        items = [
            _I('n_longit', 2, min=1, max=16, tooltip=
               "Number of hex elements in longitudinal direction of a block"),
            _I('n_circum', 3, min=1, max=16, tooltip=
               "Number of hex elements over the circumference of a 1/4 section"),
            _I('s_radial', '[0.0, 0.6, 1.0]', tooltip=
               "Number of hex elements radially from inner pattern to boundary"
               "layer. It can be an integer or a list of seeds in the range 0.0"
               "to 1.0"),
            _I('s_boundary', '[0.0,0.4, 0.8,1.0]', tooltip=
               "Number of hex elements radially in the boundary layer. It can "
               "be an integer or a list of seeds in the range 0.0 to 1.0"),
            ]
        )
...
def sweepingMesher():
    ...
    """
    # Get the domain
    res = askItems(caption='Bifmesh domain selection', items=[
...
def writeMeshFile():
    ...
    if fn:
        ...
        with busyCursor():
            if ftype == 'pgf':
                writeGeometry(fn, [cfdmesh], sep=' ', mode='w', shortlines=False)
            if ftype == 'inp':
                from pyformex.plugins.fe_abq import exportMesh
                exportMesh(fn, cfdmesh, 'C3D8R')
            if ftype == 'neu':
                from pyformex.filewrite import writeNEU
                writeNEU(fn, cfdmesh)
...
def inputDrawOptions():
    ...
    try:
        ...
        pass
    res = askItems(list(_draw_options.items()))
    if res:
...
def inputSlicingParameters():
    ...
    try:
        ...
        pass
    res = askItems(list(_slice_data.items()))
    if res:
...
def nextStep(msg):
    global stepwise
    if stepwise:
        ans = ask(msg, ['Quit', 'Continue', 'Step'])
        if ans == 'Continue':
            stepwise = False
        return ans != 'Quit'
    else:
...
def example():
    ...
    examplefile = getcfg('datadir') / 'bifurcation.off.gz'
    print(examplefile)
...
def create_menu(before='help'):
    ...
    return menu.Menu('Bifmesh', items=MenuData, parent=pf.GUI.menu, before=before)
