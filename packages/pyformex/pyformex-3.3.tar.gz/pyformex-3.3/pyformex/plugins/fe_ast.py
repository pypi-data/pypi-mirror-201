...
def astInputNames(job):
    """Returns corresponding Code Aster input filename.

    job should be a jobname with or without directory part,
    but with suffix .mail or .comm.

    The jobname is the basename without the suffix and the directory part.
    The filename is the abspath of the job with extension.
    """
    job = Path(job)
    suffix = job.suffix
    if suffix not in ['.mail', '.comm']:
        raise ValueError("Suffix should be .mail or .comm")
    jobname = job.stem
    filename = job.resolve()
...
""" % (jobname, datetime.now(), pf.scriptName, header)))
        ...
        for p in self.prop.getProp('e'):
            if p.set is not None:
                ...
                set = np.where(self.eprop == p.prop)[0]
            else:
                # default is all elements
                set = np.arange(telems)
            ...
            if 'eltype' in p:
                ...
                elnrs = np.array([]).astype(int)
                els = np.array([]).astype(int)
                for i in elems:
                    nels = len(i)
                    if nels > 0:
