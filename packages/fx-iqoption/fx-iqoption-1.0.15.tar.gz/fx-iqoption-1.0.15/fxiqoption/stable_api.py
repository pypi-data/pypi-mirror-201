# python
from fxiqoption.api import IQOptionAPI
import fxiqoption.constants as OP_code
import fxiqoption.country_id as Country
import threading
import time
import logging
import operator
...
from fxiqoption.expiration import get_expiration_time, get_remaning_time
...
class IQ_Option:
    ...
    def get_all_open_time(self):
        ...
        for option in binary_list:
            for actives_id in binary_data[option]["actives"]:
                ...
                OPEN_TIME[option][name]["id"] = actives_id
                OPEN_TIME[option][name]["min"] = active["minimal_bet"]
                OPEN_TIME[option][name]["max"] = active["maximal_bet"]
                OPEN_TIME[option][name]["profit"] = active["option"]['profit']['commission']
                OPEN_TIME[option][name]["group"] = active["group_id"]
    ...
    def get_candles_by_code(self, ACTIVE_CODE, interval, count, endtime):
        self.api.candles.candles_data = None
        try:
            self.api.getcandles(ACTIVE_CODE, interval, count, endtime)
            while self.check_connect:
                if self.api.candles.candles_data != None:
                    break
        except Exception as e:
    ...
    # thank thiagottjv
    # https://github.com/Lu-Yi-Hsun/fxiqoption/issues/65#issuecomment-513998357
