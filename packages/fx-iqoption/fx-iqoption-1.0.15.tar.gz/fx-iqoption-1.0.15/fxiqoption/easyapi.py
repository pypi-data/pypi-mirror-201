...
class IQOption:
    ...
    def isOpen(self):
        ...
        markets = pd.DataFrame(isOpen)
        keys = list(OP_code.ACTIVES.keys())
        mask = markets['Asset'].isin(keys)
    ...
    def history_by_code(self, symbol_code, timeframe,candles):
        candles = candles + 1
        timestamp = self.iq.get_server_timestamp()
        timeframe = self.timeframe_to_seconds(timeframe)
        
        x = self.iq.get_candles_by_code(symbol_code, int(timeframe), candles, timestamp)
