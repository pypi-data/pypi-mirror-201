...
def fetch_price_stock(ticker,start,end,info_types=['Close','Volume'],adjust=-2*365):
    """
...
def compare_msecurity_cross(dflist,measure,start,end,graph=True):
    ...
    if graph:
        # 绘制多条线
        title_txt="比较多种证券产品的业绩指标走势"
        y_label=ectranslate(measure)
        
        import datetime
        today = datetime.date.today().strftime("%Y-%m-%d")
        x_label="数据来源：多种来源混合，统计日期："+today    
...
def rar_ratio_rolling_df(df,ratio_name='sharpe',RF=True,window=240,scope='China'):
    """
