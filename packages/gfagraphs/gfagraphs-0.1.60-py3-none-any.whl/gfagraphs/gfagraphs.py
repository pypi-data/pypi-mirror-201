...
class Graph():
    ...
    def rename_node(self, old_name: str, new_name: str) -> Segment | None:
    ...
    def merge_segments(self, *segs: str, merge_name: str | None = None) -> str:
        ...
        segments_positions: list = [self.get_segment_position(s) for s in sorted(
            [seg for seg in segs], key=lambda x: int(sub('\D', '', str(x))))]
        left_most: Segment = self.segments[segments_positions[0]]
        if merge_name:
            self.rename_node(left_most.datas['name'], merge_name)
        ...
        for ipath in self.get_path_list():
            for idx, _ in enumerate(ipath.datas['path'][:-len(names_to_be_deleted)]):
                if all(ipath.datas['path'][idx+x] == names_to_be_deleted[x] for x in range(len(names_to_be_deleted))):
                    ipath.datas['path'] = ipath.datas['path'][:idx] + \
                        ipath.datas['path'][:idx+len(names_to_be_deleted)]
