...
class Graph():
    ...
    def merge_segments(self, *segs: str, merge_name: str | None = None) -> str:
        ...
        if merge_name:
            left_most.datas['name'] = merge_name
        ...
        for ipath in self.get_path_list():
            for elt in ipath.datas['path']:
                if elt[0] in names_to_be_deleted:
                    ipath.datas['path'].remove(elt)
    ...
    def remove_duplicates_edges(self) -> None:
        ...
        # Purging duplicate values
        self.lines = list(
            {f"{fedg.datas['start']}_{fedg.datas['end']}": fedg for fedg in self.lines}.values())
