...
def _get_template(wstl_actions: List[Dict[str, Any]], tvars: Dict[str, Any]):
    ...
    for wstl_action in wstl_actions:
        if _is_in_target('cj-template', wstl_action):
            ...
            l_section = list((i, x) for i, x in enumerate(cj_template['data']) if 'section' in x and 'index' in x)
            to_replace = zip((item[0] for item in l_section), sorted((item[1] for item in l_section), key=lambda x: (x['section'], x['index'])))
            for item in to_replace:
                cj_template['data'][item[0]] = item[1]
            break
    return cj_template
