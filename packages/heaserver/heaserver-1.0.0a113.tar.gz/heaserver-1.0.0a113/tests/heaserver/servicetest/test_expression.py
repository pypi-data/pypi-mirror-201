...
from heaobject.folder import AWSS3Folder, AWSS3FolderFileItem
...
# Object passed in is an AWSS3Folder; an AWSS3FolderFileItem is passed in as an extra name obj2.
TEST_EXPRESSIONS_WITH_EXTRA = [
        BoolExpressionTuple(expr='isheaobject(heaobject.folder.AWSS3FolderFileItem)', value=False),
...
def _my_test_generator(expr: str, expected: Any):
    def test_(self):
        self.assertEquals(expected, self.eval_(expr))
    return test_


for test_case in TEST_EXPRESSIONS:
    setattr(TestEvaluator, f'test_{test_case.expr}', _my_test_generator(test_case.expr, test_case.value))
