            ...
            try:
                try:
                    ...
                    if self.is_redirect:
                        ...
                        return self.html
                    if res.status != 200 and res.status != 304:
                ...
                index += 1
                self.log.critical('...' + str(e), out=1)
        self.log.critical('Index is over than %s times,crawl fail, URL;%s' % (self.maxtime, self.url), out=1)
        self.html = None


class Crawl2:

    def __init__(self, url, interval=1, headers={}, data=None, protocol=None, port=None, method=None):
        parser = urllib.parse.urlparse(url)
        self.hostname = parser.hostname
        self.url_path = parser.path
        self.protocol = parser.scheme
        ...
        if method:
            self.method = method
        if self.protocol == 'https':
            self.connection = http.client.HTTPSConnection(host=self.hostname, port=self.port)
        else:
            self.connection = http.client.HTTPConnection(host=self.hostname, port=self.port)
        self.connection.request(method=self.method, url=self.url_path, headers=headers)
