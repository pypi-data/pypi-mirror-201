"""
ItemRefererMiddleware: populates Request referer field, based on the Response which
originated it.
"""
import warnings
    ...
    def process_spider_output(self, response, result, spider):
        def _set_referer(r):
            if isinstance(r,scrapy.Item):
                referrer = self.policy(response,response.request ).referrer(response.url, response.request.url)
                headers = response.request.headers.copy()
                cookie = ''
                if response.headers.get('Set-Cookie'):
                    # 取上层cookie以及setcookie重组
                    cur_cookie = headers.get('cookie','').decode("utf-8")+ '; ' if headers.get('cookie','').decode("utf-8") else ''
                    cookie =  cur_cookie +';'.join(i.decode("utf-8") for i in response.headers.getlist('Set-Cookie'))
                    headers['Cookie'] = cookie
                if '_refer' not in r.__dict__:
                    r.__setattr__('_refer',referrer)
                if '_header' not in r.__dict__:
                    r.__setattr__('_header',headers)
                if '_set_cookies' not in r.__dict__ and cookie:
                    r.__setattr__('_set_cookies',cookie)
            return r
