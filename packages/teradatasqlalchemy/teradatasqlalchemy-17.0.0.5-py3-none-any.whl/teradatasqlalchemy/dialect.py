...
class TeradataDialect(default.DefaultDialect):
    ...
    def has_table(self, connection, table_name, schema=None):
        ...
        # which is faster.
        dbc_tables = self.__get_xviews_obj("dbc.tablesV")

        # Permanent tables (TableKind in 'O', 'Q', 'T')
        stmt = select([column('tablename')],
                      from_obj=[text(dbc_tables)]).where(
                        and_(text('DatabaseName (NOT CASESPECIFIC) = ' + ':schema' + ' (NOT CASESPECIFIC)'),
    ...
    def get_columns(self, connection, table_name, schema=None, **kw):
        ...
        is_view_c = "CASE WHEN tablekind = 'V' THEN 1 ELSE 0 END as is_view"
        is_art_c = "CASE WHEN TVMFlavor = 'A' THEN 1 ELSE 0 END as is_art_table"
        stmt = select([text(is_view_c), text(is_art_c)], from_obj=text(dbc_tables)).where(
        ...
        if isView or isVolatile:
            ...
            dbc_columns = self.__get_xviews_obj("dbc.ColumnsV")
            ...
            stmt = select(['*'], from_obj=text(dbc_columns)).where(
                and_(text('DatabaseName (NOT CASESPECIFIC) = ' + schema_name + ' (NOT CASESPECIFIC)'),
    ...
    def get_schema_names(self, connection, **kw):
        ...
        dbc_schemas = self.__get_xviews_obj("dbc.DatabasesV")
        
        stmt = select([column('DatabaseName')],
               from_obj=[text(dbc_schemas)],
    ...
    def get_view_definition(self, connection, view_name, schema=None, **kw):
        ...
        res = connection.execute('show view {}.{}'.format(schema, view_name)).scalar()
    ...
    def get_pk_constraint(self, connection, table_name, schema=None, **kw):
        ...
        dbc_indices = self.__get_xviews_obj("dbc.IndicesV")

        stmt = select([column('ColumnName'), column('IndexName')],
                      from_obj=[text(dbc_indices)]).where(
                          and_(text('DatabaseName (NOT CASESPECIFIC) = ' + ':schema' + ' (NOT CASESPECIFIC)'),
    ...
    def get_unique_constraints(self, connection, table_name, schema=None, **kw):
        ...
        stmt = select([column('ColumnName'), column('IndexName')], from_obj=[text(dbc_indices)]) \
    ...
    def get_foreign_keys(self, connection, table_name, schema=None, **kw):
        ...
        dbc_child_parent_table_info = self.__get_xviews_obj("DBC.All_RI_ChildrenV")
        ...
        stmt = select([column('IndexID'), column('IndexName'), column('ChildKeyColumn'), column('ParentDB'),
                      ...
                      from_obj=[text(dbc_child_parent_table_info)]) \
    ...
    def get_transaction_mode(self, connection, **kw):
        ...
        dbc_sessions = self.__get_xviews_obj("dbc.sessioninfoV")

        stmt = select([text('transaction_mode')],\
                from_obj=[text(dbc_sessions)]).\
