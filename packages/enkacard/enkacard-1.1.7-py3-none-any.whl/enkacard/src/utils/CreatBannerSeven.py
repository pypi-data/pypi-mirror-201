...
async def generationSeven(characters, assets, img, lvl, splash, signatureRes, player, typelang = 0):
    person = assets.character(characters.id)
    task = []
    if img:
        task.append(background(img, False, characters.element, signatureRes, characters, player, lvl,typelang))
    else:
        task.append(background(characters.image.banner.url, True, characters.element, signatureRes, characters, player, lvl,typelang) if splash else background(person.images.banner.url, True, characters.element, signatureRes, characters, player, lvl,typelang))
