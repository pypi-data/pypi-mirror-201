    ...
    def parse_data_line(self, line):
        ...
        for i in range(len(line)):
            if line[i] != '"':
                continue
            if not q:
                parse_list.extend(line[mark+1:i].split())
                mark = i
                q = True
                qpair = False
            else:
                if qpair:
                    ...
                    parse_list.append(line[mark:i+1])
                    mark = i
                    q = False
        if q:
            raise AuxParseException("Quotes not closed")
        if mark < len(line) - 1:
            parse_list.extend(line[mark+1:].split())
        return parse_list
        ...
        for data_line in db.data:
            ...
            for x, finstr in zip(data_line, field_info):
                ...
                if process_instr == "default":
                    ...
                    processed_obj_data[field_name] = NQ(x)
                elif process_instr == "string_main":
                    processed_obj_data[otype] = NQ(x)
            ...
            if otype == "gen":
                ...
                try:
                    ...
                    br = Branch(next(from_bus.nodes), next(to_bus.nodes), id)
