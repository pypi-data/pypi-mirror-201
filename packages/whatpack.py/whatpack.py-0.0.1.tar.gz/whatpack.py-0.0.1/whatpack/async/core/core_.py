...
async def find_link():
    ...
    locations = [locate_on_screen(f"{dir_path}\\data\\{loc}",
                                  grayscale=True, confidence=0.9, multiscale=True) for loc
                 in link_paths]
    location = None
    y = 0
    for pos_link in locations:
        if pos_link is not None and pos_link[1] > y:
            y = pos_link[1]
            location = pos_link
    print(location)
...
async def find_photo_or_video():
    dir_path = os.path.dirname(os.path.realpath(__file__))
    location = locate_on_screen(f"{dir_path}\\data\\photo_or_video.png", confidence=0.8, multiscale=True,
                                grayscale=True)
    print(location)
    moveTo(location[0] + location[2] / 2, location[1] + location[3] / 2)
    click()
