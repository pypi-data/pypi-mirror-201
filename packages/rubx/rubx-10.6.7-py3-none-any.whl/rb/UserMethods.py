...
class UserMethods:
    ...
    get_my_sticker_sets         =   lambda self='Client': GetData.api(version=self.api_version or '5', method='getMyStickerSets', auth=self.auth, data={}, mode=self.city, proxy=self.proxy, platform=self.platform or 'web')
    ...
    def upload_file(self, file: str,
        ...
        if not isinstance(file, bytes) and not compile(r'http.*\:\/\/\S+|www\.\S+').search(file):
            ...
            if isinstance(file, bytes):
                with open('uploading.txt', 'wb+') as f:
                    f.write(file)
                    file: str = 'uploading.txt'

            else:

                with open(file, 'wb+') as f:
                    f.write((bdata := get(file).content))
    ...
    def __get_message_id(self, message_id: str = None,
                         action: bool = False) -> typing.Union[str, typing.Tuple[str]]:
        '''
        if your param is a post link, this method has a the return of tuple [guid, message id]
        
        otherwise, it returns a string that is the message id. '1234...'
        
        `self.__get_message_id('rubika.ir/+')`
        '''
        
        if not message_id:
            return message_id

        if compile(r'rubika\.ir\/\S+').search(message_id.__str__()):
            result = Maker.check_link(post=message_id, client=self)
            return result if action else result[1]

        else:
            return message_id.__str__()


    def __bypass(self, chat_id: str = None,
                 starter: bool = False) -> None:
        '''
        this method is for bypass the rubika lock, and a action for methods
        '''
        
        if starter:
            for _ in range(2): self.get_base_info
            for _ in range(2): self.get_barcode_action('barcode')
            try: self.get_object('test')
            except Exception: ...

        from .rubino import RubinoClient
        with RubinoClient(__name__, self.auth) as client:
            client.get_profile_list()

        self.get_base_info
        self.get_tag_list()
        self.register_device(self.auth)


    def get_updates(self, *args, **kwargs) -> typing.Union[dict, str, object]:
        '''
    ...
    def send_message(
        ...
        button_id           :   (str)                   =   None,
        sticker             :   typing.Optional[bool]   =   (False),
        emoji_character     :   (str)                   =   ('😜'),
        ...
        parse_mode          :   (str)                   =   None,
        mention_user_ids    :   typing.Union[typing.List[str], typing.Tuple[str]] = None,
        bypass              :   (bool)                  =   False,
        *args,
        **kwargs
            ...
            if bypass:
                self.__bypass(chat_id)

            chat_id: str = self.__get_chat_id(chat_id)

            data: (dict) = {
                    'object_guid'           :   (chat_id),
                    'rnd'                   :   str(randint(100000,999999)),
                    'text'                  :   (text),
                    'reply_to_message_id'   :   (self.__get_message_id(reply_to_message_id))
                }
    ...
    def edit_message(
        ...
        **kwargs
        ...
        data: (dict) = {
            'message_id'    :   self.__get_message_id(message_id),
            'object_guid'   :   chat_id,
            'text'          :   new_text
        }
    ...
    def delete_messages(
        ...
        chat_id: str = self.__get_chat_id(chat_id)
        message_ids = [self.__get_message_id(ids) for ids in message_ids]
    ...
    def get_messages_by_id(
        ...
        bypass      :   (bool) = False
            ...
            chat_id, message_ids = self.__get_chat_id(chat_id), [self.__get_message_id(ids) for ids in message_ids]
    ...
    def forward_messages(
        ...
        bypass              :   (bool) = False
            ...
            from_object_guid, message_ids, to_object_guid = self.__get_chat_id(from_object_guid), [self.__get_message_id(ids) for ids in message_ids], self.__get_chat_id(to_object_guid)
    ...
    def seen_chats(
        self        :   ('Client'),
        seen_list   :   (typing.Union[typing.Dict[str, int], typing.List[typing.Union[dict, str, int]], str, int, typing.Tuple[typing.Union[str, int, dict]]])
        ) -> typing.Union[dict, str, object]:

        '''
        USE:
        
            self.seen_chats({'guid':'message_id'})
            # OR
        ...
        if isinstance(seen_list, (list, tuple)):

            if isinstance(seen_list.__iter__().__next__(), (str, int)):
                if not compile(r'[^0-9]').search(seen_list[0].__str__()):
                    seen_list = seen_list[::-1]

                seen_list = {self.__get_chat_id(seen_list[0]): self.__message_id(seen_list[1])}

            else:
                seen_list = [{self.__get_chat_id(seen.get(seen.keys()[0])): self.__message_id(seen.get(seen.keys()[1]))} if isinstance(seen, dict) else {self.__get_chat_id(seen[0]): self.__message_id(seen[1])} if compile(r'[^0-9]').search(seen[0].__str__()) else {self.__get_chat_id(seen[1]): self.__message_id(seen[0])} for seen in seen_list]

        else:
            seen_list = {self.__get_chat_id(seen_list.get(seen_list.keys()[0])): self.__message_id(seen_list.get(seen_list.keys()[1]))}
    ...
    def set_pin_message(
        ...
        chat_id, message_id = self.__get_chat_id(chat_id), self.__get_message_id(message_id)
    ...
    def send_photo(
        ...
        **kwargs,
            ...
            bdata = open(file, 'rb').read() if not isinstance(file, bytes) and not 'http' in file else file if isinstance(file, bytes) else get(file).content
            file = findall(r'(\w+\..+)', file)[0] if not isinstance(file, bytes) else 'uploading.txt'

            if (not thumbnail):
                thumbnail: str = str(UserMethods.get_thumb_inline(bdata))

            size: tuple = size or UserMethods.get_image_size(bdata)
            ...
            data: (dict) = {
                        ...
                        'object_guid'           :   chat_id,
                        'rnd'                   :   randint(100000,999999999).__str__(),
                        'reply_to_message_id'   :   self.__get_message_id(reply_to_message_id)
                    }
        ...
        **kwargs,
            ...
            the_data: (dict) = {
                        'file_inline'           :   {
                            ...
                            'file_name'         :   file,
                            'size'              :   bdata.__len__().__str__(),
                            'time'              :   time or (__import__('tinytag').TinyTag.get(file).duration).__round__(),
                            'mime'              :   file,
                            'access_hash_rec'   :   uresponse[1],
                        },
                        'object_guid'           :   chat_id,
    ...
    def send_gif(
        ...
        chat_id             :   str,
        file                :   str,
        width               :   typing.Union[str, int] = 250,
        height              :   typing.Union[str, int] = 250,
        thumbnail           :   str =   (None),
        caption             :   str =   (None),
        reply_to_message_id :   str =   (None),
        auto_play           : typing.Optional[bool] = False,
        is_mute             : typing.Optional[bool] = False,
        bypass              :   bool =  (False)
        ) -> typing.Union[dict, str]:
            
            '''
            self.send_gif('chat-id', 'path/file')
            
            if you paused to file a bytes:
              file = b'bytes+'
            or a link:
              file = 'https://+'
            '''
                ...
                GetData.api(
                    ...
                    data        =   {
                        ...
                        'file_inline'           :   {
                            'access_hash_rec'   :   uresponse[1],
                            'dc_id'             :   uresponse[0]['dc_id'],
                            'file_id'           :   uresponse[0]['id'].__str__(),
                            'auto_play'         :   auto_play,
                            'file_name'         :   file,
                            'width'             :   width,
                            'height'            :   height,
                            'mime'              :   file,
                            'size'              :   bdata.__len__().__str__(),
                            'thumb_inline'      :   thumbnail or Thumbnail(bdata, width, height).to_base64,
                            'type'              :   'Gif'
                        },
                        'text'                  :   caption,
                        'reply_to_message_id'   :   self.__get_message_id(reply_to_message_id)
        ...
        **kwargs
            ...
            if (action != 'rubika'):
                ...
                if search(r'rubika\.ir\/\w{4,25}\/\w+', action):
                    message: str = self.__get_message_id(action)
            ...
            if bypass:
                self.__bypass(kwargs.get('chat_id') or message.get('chat_id'))
    ...
    def set_setting(self: Client,
                    **kwargs: dict)-> typing.Union[dict, str, object]:
    ...
    def create_poll(
        ...
        __type: typing.Literal['Regular', None] = 'Regular',
        is_anonymous: typing.Optional[bool] = True,
        allows_multiple_answers: typing.Optional[bool] = False,
        bypass: bool = False,
        *args,
        **kwargs,
        )-> typing.Union[dict, str, object]:
            ...
            '''
            this is a method for send a question quiz on the a chat
            
            self.send_poll('chat-guid', ['im fine', 'im not good', 'so so'], 'how are u?')
            '''
            
            data: dict = kwargs or {
                'object_guid'               :   self.__get_chat_id(chat_id),
                'options'                   :   options,
                'rnd'                       :   randint(99999, 9999999).__str__(),
                'question'                  :   question,
                'type'                      :   __type,
            ...
            return (GetData.api(version=self.api_version or '5', method='createPoll', auth=self.auth, data=data, mode=self.city, proxy=self.proxy, platform=self.platform or 'web'))
            ...
            return (
                GetData.api(
                    ...
                    data        =   {
                        ...
                        'meesage_id'            :   self.__get_message_id(reply_to_message_id)
                        },
                    mode        =   self.city,
                    proxy       =   self.proxy,
                    platform    =   self.platform or 'android'
        ...
        state   :   typing.Union[int, str] =   None
            ...
            return (
                GetData.api(
                    ...
                    data        =   {
                        'object_guid'   :   chat_id,
                        'state'         :   state or ((datetime.today().timestamp()).__round__() - 250).__str__()
                      },
                    mode        =   self.city,
                    platform    =   self.platform or 'web',
                    proxy       =   self.proxy
    ...
    def mention_text(
        ...
        return (
                GetData.api(
                    ...
                    auth        =   self.auth,
                    method      =   'sendMessage',
                    data        =   {'object_guid': chat_id, 'rnd': str(randint(100000,999999999)), 'reply_to_message_id': self.__get_message_id(reply_to_message_id)}.update({'text': mention[1], 'metadata': {'meta_data_parts': mention[0]}}),
                    mode        =   self.city,
                    platform    =   self.platform or 'web',
                    proxy       =   self.proxy
                )
    ...
    def send_report(
        ...
        sleep(0.5)
        self.get_messages('b0Y0a2cafbaf668e282d2dc02a1fe2a7')
        get: list = loads(list(client.last_response)[::-1].__iter__().__next__())['data']['messages']
    ...
    def send_movie(
        ...
        data: dict = {
            'file_inline'           :   {
                ...
                'size'              :   bdata.__len__().__str__(),
                'thumb_inline'      :   thumbnail or Thumbnail(bdata, width, height).to_base64,
                'time'              :   (__import__('tinytag').TinyTag.get(file).duration * 1000).__round__(),
                'type'              :   'Video',
                'width'             :   width
                },
            'is_mute'               :   is_mute,
    ...
    def get_chat_last_message_id(self, chat_id: str) -> (str):
        ...
        if isinstance(result, dict):
            return (result.get('data') or data).get('chat').get('last_message_id')
        else:
            try:
                return result.data.chat.last_message_id
            except Exception:
                try:
                    return result.chat.last_message_id
                except Exception:
                    result = loads(list(self.last_response)[::-1].__iter__().__next__())
                    return (result.get('data') or data).get('chat').get('last_message_id')
    ...
    def get_chat_last_text(self, chat_id: str) -> (str):
        ...
        return (result.get('data') or data).get('chat').get('last_message').get('text')
    ...
    def get_chat_last_object(self, chat_id: str) -> (str):
        ...
        return (result.get('data') or data).get('chat').get('last_message').get('object_guid')
    ...
    def get_chat_first_message(self, chat_id: str) -> typing.Union[str, dict]:
        ...
        if isinstance(data, dict):
            data: list = (data.get('data') or data).get('messages')
        else:
            try:
                ...
                try:
                    ...
                    data = loads(list(self.last_response)[::-1].__iter__().__next__())
                    data = (data.get('data') or data).get('messages')
    ...
    def get_last_message(self, chat_id: str) -> typing.Union[str, dict]:
        ...
        return (result.get('data') or result).get('chat').get('last_message')
    ...
    def get_chat_last_author(self, chat_id: str) -> typing.Union[str, dict]:
        ...
        return (result.get('data') or result).get('chat').get('last_message').get('author_object_guid')
    ...
    def get_messages(self, chat_id: str,
        ...
        if action:
            ...
            if isinstance(messages, dict):
                ...
                return list(map(lambda event: Message(**event), (messages.get('data') or messages).get('messages')))
                ...
                if isinstance(messages, str):
                    ...
                    try:
                        ...
                        try:
                            ...
                            from .models import Message
                            result = loads(list(self.last_response)[::-1].__iter__().__next__())
                            return [Message(**event) for event in (result.get('data') or result).get('messages')]
        else:
        ...
        if isinstance(messages, dict):
            ...
            try:
                ...
                if isinstance(messages, str):
                    ...
                    result = loads(list(self.last_response)[::-1].__iter__().__next__())
                    return (result.get('data') or result).get('messages')[offset]
    ...
    @property
    def get_dcs(self) -> typing.Union[dict, str, object]:

        data = {'api_version': '4', 'method': 'getDCs', 'client': clients.web}
        return GetData.pwa(json=data, api='https://getdcmess.iranlms.ir/')        


    def request_delete_account(self, *args, **kwargs) -> typing.Union[dict, str, object]:
        '''
        delete your account with session (auth key)
        a SMS sended in your device system (on simcart)
        a message will be sent to your SIM card
        u must click on the link that was sent.
        
        note: the message will be sent to the number on your account seted.
        
        `print(self.request_delete_account())`
        '''

        return (GetData.api(version = self.api_version or '5', auth = self.auth,
                            method = 'requestDeleteAccount', data = {},
                            mode = self.city, platform = self.platform or 'rubx',
                            proxy = self.proxy))


    @property
    def delete_account(self) -> typing.Union[dict, str, object]:
        '''
        `print(self.delete_account)`
        '''
        
        return self.request_delete_account()


    def get_chat_ads(self, state: typing.Union[str, int] = None,
                     minus: int = 200) -> typing.Union[str, dict, object]:
        '''
        `self.get_chat_ads()`
        '''

        return (GetData.api(version = self.api_version or '5', auth = self.auth,
                            method = 'getChatAds', data = {'state': state or (datetime.today().timestamp().__round__() - minus)},
                            mode = self.city, platform = self.platform or 'web',
                            proxy = self.proxy))
