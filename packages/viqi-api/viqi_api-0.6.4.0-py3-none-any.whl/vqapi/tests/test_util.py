...
from vqapi import BQServer, BQSession
from vqapi.comm import BQApiError
from vqapi.util import *
from vqapi.util import fetch_dataset
...
TEST_PATH = "tests_%s" % urllib.parse.quote(
    datetime.now().strftime("%Y%m%d%H%M%S%f")
)  # set a test dir on the system so not too many repeats occur
...
def image_uri(session, stores):
    ...
    uploads an image
    """
    resource = etree.Element("resource", name=f"{TEST_PATH}/{stores.files[0].name}")
...
def setup_fetchimageplanes():
    ...
    """
    global image_uri
    resource = etree.Element("resource", name=f"{TEST_PATH}/{filename1}")
...
def test_saveblob_1(session, stores):
    ...
    try:
        ...
        assert False, "BQApiError: Status: %s" % e.http_code
    if result is None:
...
def test_fetchimageplanes_1():
    ...
    """
    try:
        result = fetch_image_planes(
            bqsession, image_uri, results_location, uselocalpath=False
        )
    except BQApiError as e:
        assert False, "BQApiError: Status: %s" % e.http_code
...
def test_fetchimageplanes_2():
    ...
    try:
        result = fetch_image_planes(
            bqsession, image_uri, results_location, uselocalpath=True
        )
    except BQApiError as e:
        assert False, "BQApiError: Status: %s" % e.http_code
...
def test_saveimagepixels():
    ...
    try:
        result = save_image_pixels(
            bqsession, stores.files[0].location, image_tags=bqimage
        )
    except BQApiError as e:
        assert False, "BQApiError: Status: %s" % e.http_code
