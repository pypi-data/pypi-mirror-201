...
@jit(nopython=True, parallel=True, fastmath=True, cache=True, nogil=True, inline='always')
...
def rgb_to_hsl(rgb_array: np.ndarray) -> np.ndarray:
    ...
    # Calculate the minimum, maximum, and difference of the RGB values
    cmin = np.min(rgb_array, axis=-1)
    cmax = np.max(rgb_array, axis=-1)
    delta = cmax - cmin
    ...
    h[mask_r] = np.divide(g[mask_r] - b[mask_r], delta[mask_r], out=np.zeros_like(delta[mask_r]), where=delta[mask_r] != 0) % 6
    h[mask_g] = np.divide(b[mask_g] - r[mask_g], delta[mask_g], out=np.zeros_like(delta[mask_g]), where=delta[mask_g] != 0) + 2
    h[mask_b] = np.divide(r[mask_b] - g[mask_b], delta[mask_b], out=np.zeros_like(delta[mask_b]), where=delta[mask_b] != 0) + 4
