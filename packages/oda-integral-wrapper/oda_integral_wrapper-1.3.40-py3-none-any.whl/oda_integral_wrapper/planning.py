...
cons_url =  "ftp://isdcarc.unige.ch/arc/rev_3"
            ...
            try:
                ...
                for cc in columns_to_remove:
                    if cc in ao.columns.names:
                        ...
                        ao=ao.columns.del_col(cc)
                print("found valid pad", pad)
                ao_list.append(ao)
                break
...
def get_pointings(revid, kind="historic", getdata=True, max_version=5):
    if getdata:
    ...
    if kind == 'historic':
        pt = cons_url + "/aux/adp/%s.001/pointing_definition_%s.fits.gz" % (revid, kind)
    elif kind == 'predicted':
        pt = pvphase_url + "/aux/adp/%s.000/pointing_definition_%s_" % (revid, kind)
    else:
    ...
    if len(pods) == 0:
        raise Exception("no pointing definition as " + pt)
...
def get_pod(revid):
    ...
    for num in range(1, 10):
        ...
        try:
            ...
            pod_test = cons_url + "/aux/org/%s/pod_%s_%04d.fits.gz" % (revid, revid, num)
            try:
                ff = fits.open(pod_test)
                pod_list.append(pod_test)
                ff.close()
