...
def main(args=None):
    ...
    with PDGM.open(args.input, 0) as pdgm:
        spatial_searcher = SpatialSearcher(list(zip(pdgm.birth_indices, pdgm.death_indices)), pdgm.births, pdgm.deaths)
...
class Graph(object):
    ...
    def birth_component(self):
        return depth_first_order(self.graph, self.index_to_vertex(self.birth_index), False, False)
...
def argument_parser():
    ...
    cp.add_argument("-e", "--epsilon", type=float, default=0.0, help="noise bandwidth (default: 0.0)")
