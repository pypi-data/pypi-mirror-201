...
def build_api(config: dict, verbose: bool, project_dir: str, module_build_dir: str):
    ...
    if config['api']['entrypoint'] == 'api.lambda.handler':
        ...
        if not os.path.exists(lambda_file):
            ...
            shutil.copyfile(
                os.path.join(os.path.dirname(os.path.abspath(__file__)), 'default_lambda.py'),
                lambda_file
            )
    ...
    subprocess.run(export_command, cwd=project_dir, check=True)
    ...
    subprocess.run(install_command, check=True, stdout=subprocess.DEVNULL if not verbose else None)
