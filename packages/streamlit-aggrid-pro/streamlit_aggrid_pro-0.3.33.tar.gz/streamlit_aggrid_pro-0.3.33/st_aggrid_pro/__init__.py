...
from st_aggrid_pro.shared import GridUpdateMode, DataReturnMode, JsCode, walk_gridOptions, ColumnsAutoSizeMode, \
    AgGridTheme, ExcelExportMode
...
if not _RELEASE:
    warnings.warn("WARNING: ST_AGGRID_PRO is in development mode.")
    ...
    _component_func = components.declare_component("agGridpro", path=build_dir)
    ...
    if component_value:
        ...
        if not frame.empty:
            ...
            if try_to_convert_back_to_original_types:
                ...
                if numeric_columns:
                    frame.loc[:, numeric_columns] = frame.loc[:, numeric_columns].apply(pd.to_numeric,
                ...
                if date_columns:
                    frame.loc[:, date_columns] = frame.loc[:, date_columns].apply(pd.to_datetime,
                                                                                  errors=conversion_errors)
