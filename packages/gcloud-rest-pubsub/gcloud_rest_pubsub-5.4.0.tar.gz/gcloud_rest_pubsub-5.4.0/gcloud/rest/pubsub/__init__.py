...
```console
$ pip install --upgrade gcloud-rest-pubsub
```
...
`gcloud-rest-pubsub` provides `SubscriberClient` as an interface to call
pubsub's HTTP API:
...
# pull messages
messages: List[SubscriberMessage] = client.pull(
    'projects/<project_name>/subscriptions/<subscription_name>',
    max_messages=10)
```

There's also `gcloud.rest.pubsub.subscribe` helper function you can use to setup
a pubsub processing pipeline. It is built with `asyncio` and thus only
available in the `gcloud-rest-pubsub` package. The usage is fairly simple:
...
- `handler`: An function that will be called for each message. It should
...
```python
subscribe_task = asyncio.create_task(gcloud.rest.pubsub.subscribe(...))
...
Any `gcloud-rest-pubsub` Publisher requests made with that environment variable
...
class SubscriberClientWithBackoff(SubscriberClient):
    ...
    def pull(self, *args: Any, **kwargs: Any):
