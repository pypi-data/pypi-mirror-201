...
class applyMp(BaseCli):
    ...
    def __init__(self, f:Callable[[T], T], prefetch:int=None, timeout:float=8, utilization:float=0.8, bs:int=1, newPoolEvery:int=0, quiet=True, **kwargs):
...
:param quiet: if True, suppresses some boilerplate log messages (recommended), but still displays normal error logs. """
        ...
        self.newPoolEvery = newPoolEvery; self.quiet = quiet; self.ps = []
    def __ror__(self, it:Iterator[T]) -> Iterator[T]:
        ...
        def gen(it):
            try:
                ...
                for p in self.ps: p.terminate();
        if self.quiet:
            with k1lib.captureStdout(False, True): return gen(it)
        else: return gen(it)
