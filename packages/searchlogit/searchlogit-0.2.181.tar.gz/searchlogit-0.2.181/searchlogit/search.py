...
class Solution(dict):  # TODO: subclass UserDict
    ...
    def add_objective(self, bic, multi_val=-10000000.0, loglik=-10000000.0):
        self['bic'] = bic
...
class Search():
    ...
    def __init__(self, df, varnames, df_test=None, dist=None, code_name="search",
        ...
        if (multi_objective_variable != 'BIC' and
            ...
            multi_objective_variable != 'LL'):
            raise ValueError('Must select BIC or MAE for multi-objective')
    ...
    def fit_mnl(self, sol):
        ...
        return (bic, ll, multi_val, as_vars, is_vars, rand_vars, bc_vars, corvars,
    ...
    def fit_mxl(self, sol):
        ...
        try:
            ...
            # logger.info(f"Model convergence: {model.convergence}")
            sys.stdout = old_stdout
            log_file.close()
            if self.multi_objective:
                # Validation
        ...
        logger.debug("model convergence: {}".format(conv))
        logger.debug("dof: {}".format(len(coef_names)))
        return (bic, ll, multi_val, as_vars, is_vars, rand_vars, bcvars, corvars, conv,
                pvals, coef_names)
    ...
    def fit_lccm(self, sol):
        ...
        if self.multi_objective:
            ...
            model.fit(X_test,
                      ...
                      avail=self.test_av,
                      maxiter=0,
                      init_class_betas=init_class_betas,  # TODO? weird saving?
        ...
        return (bic, ll, multi_val, as_vars, is_vars, rand_vars, transvars, corvars,
    ...
    def fit_lccmm(self, sol):
        ...
        for ii, class_betas in enumerate(class_params_spec):
            ...
            init_class_betas.append(model.coeff_[count:count+class_len])
            count += class_len
        # init_class_thetas = np.array(np.repeat('tmp', self.num_classes-1),
        #                                 dtype='object')
        # count = 0
        # for ii, member_class in enumerate(member_params_spec):
        #     member_len = len(model._get_member_X_idx(ii))
        #     if '_inter' in member_class:
        #         member_len += 1
        #     init_class_thetas[ii] = model.class_x[count:count+member_len]
        #     count += member_len
    ...
    def increase_sol_by_one_class(self, sol):
        if sol['class_params_spec'] is None:
            ...
            # num_classes = len(class_params_spec) + 1
    ...
    def remove_member_paramfeature(self, solution):
        ...
        if rem_member_param not in np.concatenate(member_params_spec):
            solution['isvars'] = [var for var in solution['isvars'] if var != rem_member_param]
    ...
    def local_search_new(self, improved_harmony, itr, PAR_itr):
        ...
        step_funcs = [self.add_new_asfeature, self.remove_asfeature,
                      self.add_new_isfeature, self.remove_isfeature,
                      self.add_new_bcfeature,
                      self.remove_bcfeature,
                      self.add_new_corfeature, self.remove_corfeature,
                      self.add_new_class_paramfeature,
                      self.remove_class_paramfeature,
                      self.add_new_member_paramfeature,
                      self.remove_member_paramfeature]

        num_steps = 1
        for step in range(num_steps):
            step_fn = self.random_state.choice(step_funcs)
            sol = step_fn(sol)
    ...
    def local_search(self, improved_harmony, itr, PAR_itr):
        ...
        if current_sol['class_params_spec'] is not None:
            if self.random_state.rand() < 0.5:
                ...
                solution = self.remove_new_class_paramfeature(current_sol)
    ...
    def run_search(self, HMS=10, HMCR_min=0.6, HMCR_max=0.9, PAR_max=0.85,
        ...
        # TODO: here generate new solutions
        existing_HMS = []
        if existing_sols is not None:
            for sol in existing_sols:
                # TODO: LOGIC HERE TO UPDATE SOLUTION
    ...
    def run_search_latent(self, HMS=10, min_classes=1, max_classes=10,
        ...
        for i in range(num_classes_considered):
            ...
            if self.multi_objective_variable == 'MAE':
                # pareto_value = np.log(pareto_val)
                all_val = np.log(all_val)
                # init_val = np.log(init_val)
        ...
        for _, all_val in enumerate(all_val_classes):
            for ii, val in enumerate(all_val):
                if val == 1000000:  # Some values default to 1 for MAE causing issues
                    val = -1e+30
                if self.multi_objective:
                    if val < min_val:
                        min_val = val
