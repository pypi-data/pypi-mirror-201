...
def has_file(
    ...
    headers = build_hf_headers(
        use_auth_token=use_auth_token, user_agent=http_user_agent()
    )
...
class PushToHubMixin:
    ...
    def _create_repo(
        ...
        url = create_repo(
            repo_id=repo_id, token=use_auth_token, private=private, exist_ok=True
        )
...
def get_checkpoint_shard_files(
    ...
    for shard_filename in tqdm(
        shard_filenames, desc="Downloading shards", disable=not show_progress_bar
    ):
        try:
            # Load from URL
