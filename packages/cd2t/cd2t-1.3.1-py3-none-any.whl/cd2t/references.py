...
class References():
    ...
    def _link_to_producers(self, consumer :ReferenceElement) -> list[ReferenceElement]:
        ...
        producer_search_list = self.producer
        if isinstance(consumer, ConsumerElement) and consumer.provider_namespace:
            if consumer.provider_namespace not in self.namespaces.keys():
                raise ReferenceError('namespace %s not found' % str(consumer.provider_namespace))
            producer_search_list = self.namespaces[consumer.provider_namespace]['producer']
        for provider in producer_search_list:
