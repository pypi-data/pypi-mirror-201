...
class FileSystemIterableSource(IterableSource, ABC):
    ...
    Args:
        ...
        file_extension: File extension of the data files. If None, all files will be
            searched.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.selected_file_names: List[str] = []
    ...
    def file_names(self) -> List[str]:
        ...
        if self.file_extension:
            files = [x for x in files if x.suffix == self.file_extension]
    ...
    def _get_data(
        ...
        This method is called under the hood by `get_data` and `yield_data`. This method
        must return a dataframe with the column `_original_filename` containing the
        original file name of each row.
