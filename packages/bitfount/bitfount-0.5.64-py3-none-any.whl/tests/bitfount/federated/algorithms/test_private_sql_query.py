...
class TestPrivateSqlQuery:
    ...
    def test_bad_sql_no_table(
        ...
        worker.run()
        assert "The default table for single table datasource is the pod" in caplog.text
    ...
    def test_join_sql_query_statement(
        ...
        assert "JOIN clauses are not supported" in caplog.text
    ...
    def test_sql_algorithm_db_connection_wrong_schema_name(
        ...
        worker.pod_identifier = "test-pod"
        worker.run()
        assert "Schema 'wrong' not found in database." in caplog.text
