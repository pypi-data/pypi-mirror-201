...
from ..components.torch.synctwin_models import (
    LinearDecoder,
    RegularDecoder,
    RegularEncoder,
    SyncTwinModule,
...
class SyncTwinRegressor(
    ...
    def _extract_pre_treatment(
        ...
        time_series_samples = time_index_utils.time_series_samples.take_all_before_start(
    ...
    def _extract_post_treatment(
        ...
        time_series_samples = time_index_utils.time_series_samples.take_all_from_start(
            time_series_samples, event_time_indexes
        )
        if not time_series_samples.all_samples_same_n_timesteps:
            raise RuntimeError(
    ...
    def _prep_data_for_fit(self, data: Dataset, **kwargs):
        synctwin_tensors, _ = self._convert_data_to_synctwin_format(data)
    ...
    def get_possible_prediction_horizon(self, sample_index: T_SamplesIndexDtype, data: Dataset):
        ...
        sample_event_time = data.event_treatments.df.loc[[sample_index], :, :].index.get_level_values(1)[0]
