...
class TestObsUnitaire(unittest.TestCase):
    ...
    def test_TimeSlot(self):
        ...
        s = TimeSlot(
            '[["2001-02-03T00:00:00","2001-02-01T00:00:00"], ["2001-02-05T00:00:00","2001-02-06T00:00:00"]]')
        self.assertTrue(s.stype == 'slot')
        self.assertTrue(
            TimeSlot([matin, aprem]).Centroid.bounds[1] == TimeSlot(matin).bounds[1])
        self.assertTrue(TimeSlot([matin, aprem]).bounds ==
                        TimeSlot([matin, midi, aprem]).bounds)

    def test_union(self):
        ts1 = TimeSlot([_inte(19, 22), _inte(1, 4), _inte(1, 5), _inte(
            2, 6), _inte(7, 8), _inte(6, 8), _inte(10, 15), _inte(20, 25)])
        ts2 = TimeSlot([_inte(19, 22), _inte(1, 4), _inte(1, 5), _inte(2, 6)])
        ts3 = TimeSlot([_inte(7, 8), _inte(6, 8),
                       _inte(10, 15), _inte(20, 25)])
        ...
        self.assertTrue(ts1 == ts2+ts3 == ts2+ts3+ts4 == ts2 +
                        ts3+ts4+ts5 == ts5+ts2+ts3+ts4 == ts1+ts1)
    ...
    def test_link(self):
        ...
        self.assertEqual(TimeSlot(_date(5)).link(
            TimeSlot(_date(5))), ('equals', True))
