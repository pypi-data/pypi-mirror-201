    ...
    def fetch_identity_token(self):
        try:            
            if self.execute_get_identity_token():
                ...
                self._identity_token = requests.request("POST", self._auth_token_url, data=_payload)
            ...
            raise Exception (f'Failed to Authenticate the User: {e} self._identity_token" {self._identity_token} self._identity_token.content" {self._identity_token.content}')
