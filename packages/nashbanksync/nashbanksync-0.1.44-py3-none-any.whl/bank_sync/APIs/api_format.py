...
class API(object):
    ...
    def fetch_identity_token(self):
        try:            
            if self.execute_get_identity_token():
                ...
                self._identity_token = requests.request("POST", self._auth_token_url, data=_payload,verify=False)
                # self._identity_token = requests.post(self._auth_token_url, data=_payload, verify=False)
                print(f"self._identity_token: {self._identity_token.request}")
                if self._identity_token.status_code == 200:
                    ...
                    raise Exception(f'Identity Post Request Error: {self._identity_token}')
        except Exception as e:
            raise Exception (f'Failed to Authenticate the User: {e} self._identity_token {self._identity_token} self._identity_token.content received {self._identity_token.content} self._identity_token.request.url sent to {self._identity_token.request.url}')
    ...
    def api_request(self, payload, method, verify=False, files=None):
        ...
        # except Exception as e:
        #     self._response = { 'error':f'Bank Sync Error: {e}' }
        # finally:
