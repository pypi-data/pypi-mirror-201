        ...
        try:            
            if self.execute_get_identity_token():
                ...
                if self._identity_token.status_code == 200:
                    ...
                    print(f'fetch_identity_token: {self._identity_token.json()}')
                    raise Exception(self._identity_token.text)
        except Exception as e:
            raise Exception (f'Failed to Authenticate the User: {e} self._identity_token.json()" {self._identity_token.json()}')
