...
class BeaconAPI(object):
    ...
    def do(self, action, name, args):
        ...
        if r.headers['content-type'] in ('application/json', 'application/vnd.api+json'):
            return r.json()
        return r.content
    ...
    def iter_paged(self, _name, max_items=six.MAXSIZE, **args):
        cur_items = 0

        while 1:
            r = self.do('get', _name, args)
            try:
                data, links = r['data'], r['links']
            except (KeyError, TypeError):
                raise BeaconAPIException(code=400, message="Endpoint did not return JSON:API structure", body=r)
            for x in data:
                yield x
                cur_items += 1
            ...
            if links['next'] is not None:
                args.update(links['next']['meta'])  # meta will contain page[offset] -> offset
            else:
                break
