...
/** @file custatevec.h
 *  @details custatevec API Version 1.3.0.20
 */
...
#define CUSTATEVEC_VER_MINOR 3 //!< cuStateVec minor version.
#define CUSTATEVEC_VER_PATCH 0 //!< cuStateVec patch version.
#define CUSTATEVEC_VERSION 1300 //!< cuStateVec Version.
 ...
 *
 * If all the destination sub state vectors are specified by using custatevecSVSwapWorkerSetSubSVsP2P(),
 * the \p communicator argument can be null. In this case, the internal CUDA calls are not serialized on the
 * stream specified by the \p stream argument. It's the user's responsibility to call \p cudaStreamSynchronize()
 * and global barrier such as \p MPI_Barrier() in this order to complete all internal CUDA calls. This limitation
 ...
 * by the call to custatevecSVSwapWorkerCreate().
 */
...
/**
 * \brief Set sub state vector pointers accessible via GPUDirect P2P
 *
 * \param[in] handle the handle to cuStateVec library
 * \param[in] svSwapWorker state vector swap worker
 ...
 * worker instances.
 *
 * If sub state vectors are allocated in different processes, the sub state vector pointers and the events should
 * be retrieved by using CUDA IPC.
 */
