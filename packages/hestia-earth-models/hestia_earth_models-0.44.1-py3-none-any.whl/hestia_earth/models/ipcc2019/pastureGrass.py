...
def _calculate_NEa(cycle: dict, animal: dict, system: dict, NEm: float):
    ...
    return _NEa_BY_GROUPING[grouping](cycle, animal, system, NEm)
...
def _calculate_NEl(cycle: dict, animal: dict):
    grouping = _get_grouping(animal)
    return _NEl_BY_GROUPING[grouping](cycle, animal)
...
def _calculate_NEg(cycle: dict, animal: dict):
    grouping = _get_grouping(animal)
    return _NEg_BY_GROUPING[grouping](cycle, animal)
...
def _calculate_GE(cycle: dict, meanDE: float, system: dict):
    ...
    NEm = sum([NEm for (_animal, NEm) in animals])
    NEa = sum([_calculate_NEa(cycle, animal, system, NEm) for (animal, NEm) in animals])
    NEl = sum([_calculate_NEl(cycle, animal) for (animal, _NEm) in animals])
    NEwork = sum([_calculate_NEwork(cycle, animal, NEm) for (animal, NEm) in animals])
    NEp = sum([_calculate_NEp(cycle, animal, NEm) for (animal, NEm) in animals])
    NEg = sum([_calculate_NEg(cycle, animal) for (animal, _NEm) in animals])
    ...
    return ((NEm + NEa + NEl + NEwork + NEp - NEfeed_m)/REM + (NEg + NEwool - NEfeed_g)/REG) / (meanDE/100) if all([
        REM, REG
    ]) else 0


def _calculate_meanECHHV(cycle: dict, practices: list):
...
def _run_practice(cycle: dict, GE: float, meanECHHV: float):
    def run(practice: dict):
        ...
        logRequirements(cycle, model=MODEL_LOG, term_id=input_term_id,
                        GE=GE,
                        meanECHHV=meanECHHV,
                        key_id=key_id)

        logShouldRun(cycle, MODEL_LOG, input_term_id, True)
...
def _should_run_practice(cycle: dict):
    def should_run(practice: dict):
        ...
        logRequirements(cycle, model=MODEL_LOG, term_id=term_id,
                        value=list_sum(value),
                        key_term_type=key_term_type)
