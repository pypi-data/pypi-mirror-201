...
import re
from htmlparsingbs4based.utils.helpers import get_crawling_results
from htmlparsingbs4based.utils.elastic import get_single_page_from_elastic
from htmlparsingbs4based.utils.visualization import visual 
...
parser.add_argument("-f", "--file_path", help="File path to the crawled html", required=True)
...
def parse_single_page():
    ...
    if get_pages_from == 'local':
        ...
        try:
            ff = get_single_page_from_elastic(ELASTIC_PRODUCT_INDEX, input_url)
        except:
            ff = []
        if len(ff) == 0:
            print('ERROR:cannot get html ', input_url)
