...
class HTML2Text(html.parser.HTMLParser):
    def __init__(
        ...
        self.show_format = config.SHOW_FORMAT # covered in cli
    ...
    def handle_tag(
        ...
        if tag in ["p", "div"]:
            if self.google_doc:
                ...
                self.p()
                if len(self.outtextlist)>0 and self.show_format:
                    if self.outtextlist[-1] not in ['\n', '<txt> ', None] and attrs != {} and start:
                        #print('p text: ', self.outtextlist[-1])
                        self.o('<txt>: ')
        ...
        if tag == "a":
            another_ignore = 1
            if len(self.outtextlist)>0 and not start:
                #print('a text: ', self.outtextlist[-1])
                #print(self.outtextlist[-1] == '\n')
                if self.outtextlist[-1] == '\n':
                    another_ignore = 0

        if tag == "a" and not self.ignore_links and another_ignore:
            if start:
                if (
                    "href" in attrs
        ...
        if tag == "img" and start and not self.ignore_images:
            if "src" in attrs:
                ...
                if self.images_to_alt:
                    #self.o(escape_md(alt))
                    text = escape_md(alt) if escape_md(alt).replace(' ', '') != '' else ''
                    if text != '':
                        if self.show_format:
                            self.o("<img>: " + escape_md(alt))
                        else:
                            self.o(escape_md(alt))
                else:
                    self.o("![" + escape_md(alt) + "]")
    ...
    def handle_data(self, data: str, entity_char: bool = False) -> None:
        ...
        if self.maybe_automatic_link is not None:
            ...
            if (
                ...
                #if data.replace(' ', '') != '':
                if self.show_format:
                    if len(self.outtextlist)==0:
                        #self.o("[")
                        self.o("<a>: ")
                    elif self.outtextlist[-1] != '[':
                        #self.o("[")
                        self.o("<a>: ")
                else:
                    pass
                #print(self.outtextlist[-1] == '\n')
                #print(self.maybe_automatic_link, ': ', data)
                self.maybe_automatic_link = None
                self.empty_link = False
