...
from hvac.api.system_backend import Init
...
from pawnlib.typing.converter import append_suffix, append_prefix, hex_to_number, FlatDict, FlatterDict, flatten
...
class NetworkInfo:
        ...
        self._platform_info = {
            "icon":{
                ...
                "network_info": {
                    ...
                    "cdnet": {
                        "network_api": "http://20.20.1.242:9000",
                        "nid": "0x53"
                    },
                }
            },
    ...
    def _initialize(self, network_name="", platform="havah"):
        pawn.console.log("Initializing")
...
class IconRpcHelper:
    ...
    def _convert_valid_payload_format(self, payload=None, method=None, params=None):
        if payload:
            self.request_payload = self._convert_payload_to_dict(payload)
        else:
            self.request_payload = json_rpc(method=method, params=params)
    ...
    def rpc_call(self, url=None, method=None, params: dict = {}, payload: dict = {}, print_error=False) -> dict:
        ...
        pawn.console.log(f"--- {_url}, {self.url}")
        self._convert_valid_url_format(url=url)
        self._convert_valid_payload_format(payload=payload, method=method, params=params)
        ...
        if print_error and self.response.get('status_code') != 200:
            if self.response.get('json') and self.response['json'].get('error'):
                ...
                pawn.console.log(f"[ERROR] payload={self.request_payload}")
            else:
    ...
    def sign_tx(self, wallet=None, payload=None):
        ...
        payload['params']['from'] = address
        # pawn.console.log(payload)
        # res = singer.sign_tx(json.loads(payload))
        res = singer.sign_tx(payload)
