...
# def check_arguments_namespace(args):
# FILL_REQUIRED_ARGUMENTS
...
class WalletCli:
    def __init__(self, args=None):
        ...
        # pawn.console.log(f"args in WalletCli => {self._args}")
    ...
    def load(self):
        ...
        if _keystore:
            ...
            if is_file(keystore) and is_json(keystore):
                pawn.console.debug(f"Found Keystore file - {keystore}")
                try:
    ...
    def print_wallet(self):
        if self._wallet:
            ...
            pawn.console.debug(f" 🔑 private_key={self._wallet.get('private_key')}")
            pawn.console.print("")
...
def load_wallet_key(file_or_object=None, password=None, _compressed=compressed):
...
class IcxSigner(object):
    ...
    def get_address(self, _compressed=compressed) -> bytes:
...
class IcxSignVerifier(object):
    ...
    def get_address(self, _compressed=compressed):
        """Create an address with pubkey.
        address is made from pubkey.
        ...
        Returns:
            str: address represented in hexadecimal string starting with '0x'
        """
        pubkey_bytes = self.__pubkey.serialize(compressed=_compressed)
        return get_address(pubkey_bytes)
