...
class ReturnCode(Enum):
    ...
    AUTHENTICODE_SIGNATURE_MISMATCH_OR_INCORRECT_IMAGEBASE = (
    6, 'Certificate\'s hash mismatch calculated hash, or incorrect ImageBase during reconstruction')
...
class SigValidator:
    ...
    def verify_signature(self, cert):
        ...
        process = subprocess.Popen(['openssl', 'asn1parse', '-inform', 'DER', '-in', self.file_signature],
        ...
        if where:
            ...
            process = subprocess.Popen(['openssl', 'smime', '-verify', '-inform', 'DER', '-in', self.file_signature,
                                        ...
                                        '/etc/ssl/certs/', '-out', '/tmp/dummy.txt'], stdout=subprocess.PIPE,
    ...
    def extract_cert(self, pe):
        ...
        if self.has_cert(pe):
            ...
            return pe.__data__[
                   security_directory.VirtualAddress:security_directory.VirtualAddress + security_directory.Size]
    ...
    def calculate_pe_digest(self, algorithm, raw_data):
        ...
        if (certificate_virtual_addr and certificate_size) != 0x0:
            data += raw_data[certificate_table_addr + 0x08:certificate_virtual_addr] + raw_data[
                                                                                       certificate_virtual_addr + certificate_size:]
