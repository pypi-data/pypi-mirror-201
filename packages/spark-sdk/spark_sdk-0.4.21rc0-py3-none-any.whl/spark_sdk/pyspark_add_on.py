...
class PySpark:
    def __init__(self, driver_memory='1G', num_executors='1', executor_memory='1G', port='', yarn=False, **spark_configs):
        ...
        if self.yarn == 'yarn':
            ...
            if os.path.exists('./tmp_sdk/'):
                import shutil
                # try:
                #     shutil.rmtree('./tmp_sdk/', ignore_errors=False)
                #     os.mkdir('./tmp_sdk/')
                # except:
                #     pass
            else:
                os.mkdir('./tmp_sdk/')
            conf.set('spark.local.dir', './tmp_sdk/')
        ...
        conf.set("spark.databricks.delta.optimize.maxFileSize", 134217728) # 128MB
