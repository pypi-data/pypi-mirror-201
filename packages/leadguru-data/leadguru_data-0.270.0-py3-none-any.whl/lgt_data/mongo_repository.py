...
from .model import LeadModel, BaseModel, UserModel, UserBotCredentialsModel, UserLeadStatusModel, \
    UserResetPasswordModel, BotModel, BoardModel, BoardedStatus, Contact, DedicatedBotModel, SlackMemberInformation, \
    UserTemplateModel, LinkedinContact, ExtendedUserLeadModel, UserLeadModel
...
class UserLeadMongoRepository(BaseMongoRepository):
    ...
    def get_leads(self, user_id, skip: int, limit: int, **kwargs) -> List[ExtendedUserLeadModel]:
        ...
        return [ExtendedUserLeadModel.from_dic(x) for x in docs]
...
class SlackContactUserRepository(BaseMongoRepository):
    ...
    def find(self, text: Optional[str] = None, skip: int = 0, limit: int = 1000, **kwargs):
        ...
        presence_updated_at = kwargs.pop("presence_updated_at", None)
        if text:
            pipeline['$text'] = {'$search': text}
        ...
        if presence_updated_at:
            pipeline['presence_updated_at'] = {'$gte': presence_updated_at}
