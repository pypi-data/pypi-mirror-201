...
def inverse(a,avoid_np_linalg_inv=False):
	...
	- a: array of shape (vdim,vdim, n1,...,nk),
	 where vdim is the ambient vector space dimension.
	- avoid_np_linalg_inv: for unknown reasons, the np.linalg.inv routine can be very slow 
	  when applied to large arrays of many small matrices. (numpy 1.23.3, mac OS, M1 processor)
	  Use an explicit inversion instead, via Cramer's formula, implemented for shape (2,2) and (3,3)
	"""
	a=ad.asarray(a)
