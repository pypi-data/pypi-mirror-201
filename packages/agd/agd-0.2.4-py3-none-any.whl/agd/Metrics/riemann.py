...
class Riemann(Base):
	...
	def gradient(self,v):
		v,m = fd.common_field((v,self.m),(1,2))
		mv = lp.dot_AV(m,v)
		return mv/np.sqrt(lp.dot_VV(v,mv))
	...
	def make_proj_dual(self,**kwargs):
		...
		"""
		λ,v = np.linalg.eigh(np.moveaxis(self.m,(0,1),(-2,-1)))
		λ = np.moveaxis(λ,-1,0) 
		v_ = np.moveaxis(v,(-2,-1),(0,1))
		proj_diag = Diagonal(λ).make_proj_dual(**kwargs)
		def proj(x):
			v,x = fd.common_field((v_,x),depths=(2,1))
			return lp.dot_AV(v,proj_diag(lp.dot_VA(x,v)))
		return proj
