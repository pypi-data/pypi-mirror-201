...
def SetKernel(self):
	...
	if self.hasTips:
		...
		geodesic.source += '#include "Kernel_GeodesicODE.h"\n'+self.cuda_date_modified
	...
	self.exportActiveNeighs = self.GetValue('exportActiveNeighs',default=False,
		help="Export which finite differences are active, and the min/max index.")
	flow_weightpos_needed = self.exportActiveNeighs or (self.hasTips and self.nstates>1)
	self.flow_needed = (self.forwardAD or self.reverseAD or self.exportGeodesicFlow 
		or flow_weightpos_needed or not online_flow)
	geodesic_outofline = None if online_flow else geodesic

	if self.flow_needed:
		flow.traits = {
			**nofront_traits,
			'flow_weightpos_macro':flow_weightpos_needed,
		}
	...
	if self.isCurvature or self.model_=='DubinsState' or self.model_ == 'Forward':
	...
	if self.model_ == "DubinsState":
		state_transition_costs = self.GetValue('state_transition_costs',
			array_float=(self.hfmIn.shape[-1],)*2,
			help="Cost of the transition between each state of the vehicle")
		SetCst('state_transition_costs_m2',state_transition_costs**-2,float_t)
