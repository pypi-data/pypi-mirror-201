...
#if nstates_macro>1 // Handling of the state (coded in the last dimension), see DubinsState_.h
	...
	if(newCell){ // Load cell corners data (flow and dist)
		for(Int icorner=0; icorner< ncorners; ++icorner){
			...
			if(!Grid::InRange_per(yq,shape_tot) 
				// For DubinsState, only include points from the same vehicle state.
				STATE(|| yq[ndim-1]!=xq[ndim-1]) 
				){
...
__global__ void GeodesicODE(
	...
	STATE(const Int * __restrict__ flow_weightpos_t,)
	...
	for(len = 1; len<max_len; ++len){
		...
		STATE( // Jump to adequate new step if needed
		if(flow[ndim-1]!=0){ // There may be a jump
			...
			const int nact = (ndim*(ndim-1))/2, nmix = ncontrols_macro+1; 
			if( (flow_weightpos>>nact) == nmix-1 ){//Effectively jumping to another state
