...
class Interface(object):
	...
	def __init__(self,hfmIn):
		...
		self.convex = self.model.startswith("Convex")
		if self.convex: self.model=self.model[6:]
	...
	def GetValue(self,key,default="_None",verbosity=2,array_float=False,
		...
		if key in self.hfmIn:
			...
			if not ad.is_ad(value): value = self.caster(value)
			# Check shape
...
# --- Specific model properties ---
	...
	@property
	def nstates(self):
		return self.kernel_data['eikonal'].traits.get('nstates_macro',1)
...
# ---- Metrics ---
	...
	def metric(self):
		...
		if self._metric is None: 
			if self._dualMetric is None: return None
