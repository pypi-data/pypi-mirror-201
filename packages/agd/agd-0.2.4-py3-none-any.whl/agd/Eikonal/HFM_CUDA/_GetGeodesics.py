...
#	SetCst('size_i', self.size_i, self.int_t)
	...
	if self.nstates>1: 
		argnames.append(('flow_weightpos','flow'))
		tip_states = np.round(tips[:,-1])
		assert np.allclose(tips[:,-1],tip_states,atol=1e-5)
		assert np.all(tip_states>=0)
		assert np.all(tip_states<self.nstates)
		tips[:,-1]=tip_states
	...
	while len(corresp)>0:
		if geoIt>=geoMaxIt: 
			self.Warn("Geodesic solver failed to converge, or geodesic has too many points"
				" (in latter case, try setting 'geodesic_max_length':np.inf)")
			break
		geoIt+=1
		nGeo = len(corresp)
