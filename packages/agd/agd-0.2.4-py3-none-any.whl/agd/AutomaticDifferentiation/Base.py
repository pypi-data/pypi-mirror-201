...
class baseAD:
	...
	def cupy_based(self): return not isinstance(self.value,np.ndarray)
...
def is_ad(data,iterables=tuple()): 
	...
	return any(isinstance(x,baseAD) for x in functional.rec_iter(data,iterables))
...
def from_cupy(x): 
	...
	return isinstance(x,_cp_ndarray) or (isinstance(x,baseAD) and x.cupy_based())
