...
class OperationType(ModelSimple):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Args:
            args[0] (str): Type of operation:   * `HARVEST` - Harvesting crops   * `STORE` - Culture storage   * `DRY` - Drying culture   * `RELOCATE` - Transportation between production facilities   * `CONSUMPTION` - Plant crop consumption   * `LOAD` - Loading a culture into a storage location   * `UNLOAD` - Unload culture from storage location   * `SELL` - Selling culture   * `BUY` - Buying culture ., must be one of ["HARVEST", "STORE", "DRY", "RELOCATE", "CONSUMPTION", "LOAD", "UNLOAD", "SELL", "BUY", ]  
