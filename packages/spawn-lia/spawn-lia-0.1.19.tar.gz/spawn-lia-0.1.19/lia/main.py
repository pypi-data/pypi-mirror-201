...
def spells() -> None:
    """
    Collection for Lia's spells. For more info ask for help
    on each specific spell.
    """
...
def heal(packagename: str, t: str) -> None:  # pragma: no cover
    """
    One of Lias most basic spells. It lints, and typechecks
    the code specified in the path. To test use the -t option.
    Lia only supports pytest.
...
def deploy(t: str) -> None:
    ...
    if t == "y":
        ...
        assert "FAILED" not in str(out), str(out)
