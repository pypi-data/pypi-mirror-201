...
from airflow_commons.internal.bigquery.core import (
...
from airflow_commons.internal.bigquery.query import (
...
class BigqueryOperator(object):
    def __init__(self, service_account_file: str, enable_logging=False):
        """
    ...
    def deduplicate(
        ...
        source_partition_field = get_time_partition_field(
            self.client, source_dataset, source_table
        )
        ...
        partition_pruning_params["target_partition_field"] = get_time_partition_field(
            self.client, target_dataset, target_table
        )
        if allow_partition_pruning:
            partition_pruning_params[
                "available_target_partitions"
            ] = single_value_select(
                client=self.client,
                sql=read_sql(
    ...
    def write_query_results_to_destination(
        ...
        job_config.destination = get_table_ref(
            self.client, destination_dataset, destination_table
        )
    ...
    def archive(
        ...
        if mode not in self.ARCHIVE_MODES:
            raise ValueError(
                ...
                + (COMMA + WHITE_SPACE).join(i for i in self.ARCHIVE_MODES)
            )

        if primary_keys is None or primary_keys == []:
            primary_keys = get_primitive_column_list(
                self.client, project_id, source_dataset, source_table
            )
        source_statement_params = dict()
        ...
        delete_where_statement_params[
            "source_partition_field"
        ] = source_statement_params["source_partition_field"]
        self.delete(
            project_id=project_id,
            dataset_id=source_dataset,
            table_id=source_table,
    ...
    def insert_to_table_from_dataframe(
        self, dataset_id: str, table_name: str, data: pd.DataFrame()
    ):
        """
    ...
    def load_to_table_from_json(
        ...
        """
        Loads json to the specified table. Returns list of errors for each row
