...
class CommonTestCase(TestCase):
    ...
    def validate_field_in_bad_request(self,
        ...
        for invalid_param in bad_data:
            if invalid_param in valid_data:
                continue
            if required_data:
                data.update(required_data)
            data[field_name] = invalid_param

            json_response = self._send_request(params=data, expected_status_code=expected_status_code)
            self.assertIn('errors', json_response)
    ...
    def edit_success(self, edit_obj, edit_field: str, new_value: Union[str, list, int], check_new_value=True):
        ...
        edit_obj.reload()
        if check_new_value:
            self._check_new_values(edit_obj, expected_values={edit_field: new_value})
    ...
    def _counter_data(cls, key_object, raw_data):
        ...
        for field, value in deepcopy(raw_data).items():
            if isinstance(value, str) and '{i}' in value:
                ...
                value = f(value, i=cls.counter_map[key_object][field])
    ...
    def _check_new_values(self, document, expected_values):
        ...
        def convert_value_to_str(value):
            if isinstance(value.__class__, TopLevelDocumentMetaclass):
                return str(value.id)
        ...
        def recuresive_check(obj, expected_values):
            for field, exp_value in expected_values.items():
                cur_value = getattr(obj, field)
                if isinstance(exp_value, dict):
                    recuresive_check(cur_value, exp_value)
                elif isinstance(cur_value, list) and isinstance(exp_value, list):
                    ...
                    self.assertEqual(obj.check_password(exp_value), True)
                else:
                    self.assertEqual(exp_value, convert_value_to_str(cur_value))

        document.reload()
        recuresive_check(document, expected_values)
