...
from pptx.chart.series import CategoryPoints, _BaseCategorySeries, _BaseSeries
...
def add_data_labels_to_each_series(
    ...
    *,
    ...
    if data_label_spec.position == DataLabelPosition.ALL_POINTS:
        add_data_labels_to_every_point_of_each_series(chart, pivoted_df=pivoted_df)
    elif data_label_spec.position == DataLabelPosition.FIRST_AND_LAST_POINTS:
        add_data_labels_to_first_and_last_points_of_each_series(chart, pivoted_df=pivoted_df)
...
def add_data_labels_to_every_point_of_each_series(chart: Chart, *, pivoted_df: DataFrame) -> None:
    ...
    for chart_series in cast(Iterable[_BaseSeries], chart.series):
        for point, period in zip(cast(CategoryPoints, chart_series.points), pivoted_df.index):
            add_data_label_to_point(point, text=period, font=value_axis_font)


def add_data_label_to_point(point: Point, *, text: str, font: Font | None = None) -> None:
    data_label = cast(DataLabel, point.data_label)
    data_label.has_text_frame = True
    data_label.text_frame.text = text
    data_label.position = XL_DATA_LABEL_POSITION.ABOVE
    if font is not None:
        for paragraph in data_label.text_frame.paragraphs:
            copy_font_properties(font, paragraph.font)


def add_data_labels_to_first_and_last_points_of_each_series(chart: Chart, *, pivoted_df: DataFrame) -> None:
    ...
    for chart_series in cast(Iterable[_BaseSeries], chart.series):
        ...
        add_data_label_to_point(points[0], text=pivoted_df.index[0], font=value_axis_font)
        add_data_label_to_point(points[len(points) - 1], text=pivoted_df.index[-1], font=value_axis_font)
