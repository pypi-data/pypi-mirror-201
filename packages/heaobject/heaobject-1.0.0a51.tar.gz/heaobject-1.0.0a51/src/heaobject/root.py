...
class HEAObject(abc.ABC):
    ...
    HEA objects have built-in facilities for extracting the object's non-callable attributes into a dictionary or a
    JSON string (the to_dict() and to_json() methods, respectively). These methods are used by other parts of HEA to
    build REST API responses. Dynamically created attributes are not guaranteed to appear in the return values of these
    methods, nor may they appear in the get_attributes() and get_all_attributes() methods. A future version of this
    library may break the current behavior of dynamically created attributes, thus they should be avoided for now.
    ...
    Properties of a HEAObject must never raise an error because another property is unset. This restriction may be
    lifted in a future version of heaobject.
...
class AbstractHEAObject(HEAObject, abc.ABC):
    ...
    def get_attributes(self) -> Iterator[str]:
        # TODO: consider switching to inspect.getmembers_static in python 3.11.
...
class Version(AbstractMemberObject):
    """
    Represents version metadata for an object. Microservices may use subclasses of Version if their versions have extra
    attributes or their attributes have custom validation logic.
    """
    def __init__(self):
        super().__init__()
        self.__id: str | None = None
        self.__current = False
        self.__modified: date | None = None
    ...
    def current(self, current: bool):
        self.__current = bool(current) if current is not None else False
    ...
    def modified(self, modified: date | None):
        if modified is None or isinstance(modified, date):
            self.__modified = modified
        else:
            self.__modified = dateutil.parser.isoparse(modified)
...
class VersionedDesktopObject(DesktopObject, abc.ABC):
    ...
    def versions(self) -> list[Version]:
        """
        All available versions of this object, in order from earliest version to latest version. The default value is
        the empty list, and passing None into this property will set its value to the empty list. If the provided list
        is out of order, the internal copy of the list will be sorted. If no version has its current flag set to True,
        the last version in the list, after sorting, will be flagged as the current version.
        """
        pass

    @versions.setter
    @abc.abstractmethod
    def versions(self, versions: list[Version]) -> None:
        pass
...
class AbstractDesktopObject(AbstractHEAObject, DesktopObject, abc.ABC):
    ...
    def derived_from(self, derived_from: List[str]) -> None:
        ...
        elif not isinstance(derived_from, Iterable | Iterator):
            self.__derived_from = [str(derived_from)]
        else:
...
class AbstractVersionedDesktopObject(AbstractDesktopObject, VersionedDesktopObject, abc.ABC):
    ...
    def __init__(self):
        ...
        self.__versions: list[Version] = []

    @property
    def version(self) -> Optional[str]:
        return next((version.id for version in self.__versions if version.current), None)
    ...
    def versions(self, versions: list[Version]) -> None:
        if versions is None:
            ...
            _versions = [versions]
        elif isinstance(versions, Iterable | Iterator) and all(
            i is not None and isinstance(i, Version) for i in versions):
            _versions = deepcopy(versions)
            _versions.sort(key=lambda version: version.modified or datetime.min)
            if _versions:
                if any(version.id is None for version in _versions):
                    raise ValueError(f'Version with None id attribute: {versions}')
                count = 0
                for version in _versions:
                    if version.current:
                        count += 1
                if count == 0:
                    _versions[-1].current = True
                elif count > 1:
                    raise ValueError(f'More than one version is current: {versions}')
        else:
            raise ValueError(f'Invalid versions {versions}')
        self.__versions = _versions
