...
4. solve the ode system by by applying the 'LSODA' method from scipy package
...
def init_start(params):
    ...
    if RUN_TYPE == 2:
        if target_type == 'd13c' and cinpflag == 0:
            return html.P('\n When surface d13c is the target for inversion, the carbon emission scenario needs to be predefined, i.e., cinpflag is not 0. \n Inversion stops.'), 'Fail'
...
def model_run(set_progress):
    ...
    if RUN_TYPE == 2:
        ...
        if tracer_type == 'pCO2':
            ...
            global fcinp
            fcinp = interpolate.interp1d(t_target, tracer_eval, bounds_error=False, fill_value=0)
            ...
            tt = t_target
            emi_c = fcinp(tt)
            ts = t_target[1:]-t_target[0:-1]
            emi_interval = np.concatenate([[0], (emi_c[0:-1]+emi_c[1:])/2 * ts])
            ...
            ysol = solve_ivp(derivs, (t_target[0]-1000, t_target[-1]), ystart, args = [0, hpls], method = 'LSODA')
            ...
            c_ems = fcinp(t_target)
            # index = np.abs(c_ems)>0
            # t_target = t_target[index]
            ...
            info = html.Div([
                            html.P(f'{elapse: .2f}s used.'),
                            html.P('The calculated emission scenario has been saved to inverse_emission_d13c_results.csv'),
            ...
            c_ems = fcinp(t_target_d13c)
            index = np.abs(c_ems)>=0
            t_target_d13c = t_target_d13c[index]
            d13c_target = d13c_target[index]
            ...
            ysol = solve_ivp(derivs, (t_start-1000, t_end), ystart, args = [0, hpls], method = 'LSODA')
...
def cost_function(ems_new, tracer_type, hpls, tems, ems):
    if tracer_type == 'pCO2':
        ...
        errors = np.abs(tracer_sol[-1]-f_target(tems[-1]))
    ...
    if tracer_type == 'd13c':
        ...
        errors = np.abs(d13c_surf-f_target(tems[-1]))/np.abs(f_target(tems[-1]))
    ...
    if tracer_type == 'GSpH':
        ...
        errors = np.abs(surface_ph - f_target(tems[-1]))/np.abs(f_target(tems[-1]))
