...
df_param_inv = pd.DataFrame(OrderedDict([
   ...
   ('Unit', ['yr', 'yr', 'uatm', 'uatm',
    ...
    ('Comment', [ 'start time (yr) for the experiment', 'end time (yr) for the experiment', 'pCO2 reference', 'steady-state pCO2',
        ...
        'CaCO3 weathering flux (initial)', 'CaSiO3 weathering flux (initial)', 'd13c carbonate weathering', 'd13c volcanic degassing',
...
df_file_inv = pd.DataFrame(OrderedDict([
    ('File name', ['./preind_steady.dat', './1000_0500.dat']),
...
parameters_carbon_inv = {'cinpflag': 0, 'cinp': 1000, 'dccinp': -55, 'tcin0':0, 'tcspan': 6000}
    ...
    if data:
        ...
        if dict1['target'] == 'pCO2' or dict1['target'] == 'GSpH' or dict1['target']=='d13c':
            if dict1['target'] == 'pCO2':
                ...
                data2 = pd.read_csv(dict1['target_file'])
                data3 = pd.read_csv('inverse_emission_d13c_results.csv')
                ...
                fig.add_trace(go.Scatter(x = data3.Age/1000, y = data3.d13C_of_carbon_emission,
                                    mode = 'lines',
                                    showlegend=False,
                                    line = dict(color = '#00b4d8', width = 2)),
                ...
                fig.update_yaxes(title_text = 'emission d13C', row =2, col=1,
                                 showgrid = True, zeroline = True, showline = True,
                                    ticks = 'outside', linecolor = 'rgb(204,204,204)',
                                    linewidth = 2)
                fig.update_layout(
