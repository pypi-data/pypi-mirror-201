...
class DocumentThumbnail(APIObject):
    ...
    def list(
        ...
        but just ``DocumentPageFile`` object constructs that when accessed later on will fetch
...
class DocumentTextExtractionSampleDocument(APIObject):
    ...
    Holds data about feature and model prediction values, as well as the thumbnail of the document.

    Attributes
    ----------
    document_index: int
        Index of document page sample
    feature_name: str
        Name of feature that document text extraction sample is related to
    thumbnail_id: str
        Document page ID
    thumbnail_width: int
    ...
    thumbnail_link: str
        Thumbnail image download link
    document_task: str
        Document blueprint task this document belongs to
...
class DocumentTextExtractionSamplePage(APIObject):
    ...
    text_lines: List[Dict[str, Union[int, str]]]
        List of text lines and their coordinates
    document_task: str
        Document Blueprint task this page belongs to
    actual_target_value: Optional[Union[str, int, List[str]]
        Actual target value
    prediction: Optional[PredictionType]
        Prediction values and labels
    ...
    def list(
        ...
        """
        Returns list of document text extraction sample pages.

        Parameters
        ----------
        model_id: str
            Model ID to retrieve document text extraction page samples.
        feature_name: str
            Feature name to retrieve document text extraction page samples.
        document_index: Optional[int]
            Default None. Specific document index to retrieve.
        document_task: Optional[str]
            Document Blueprint task
    ...
    def get_document_page_with_text_locations(
        ...
        ----------
        line_color: str
            Default blue. Color to draw bounding box on image page
        line_width: int
            Default 3. Line width of the bounding boxes that will be drawn
        padding: int
            Default 3. Additional space left between the text and the bounding box in pixels

        Returns
        -------
        Image
            Returns PIL.Image with drawn text bounding boxes
        """
        try:
...
class DocumentTextExtractionSample(APIObject):
    ...
    1) Compute text extraction samples for specific model, and fetch all existing document text
    extraction samples for specific project.
    ...
    2) Fetch document text extraction samples for specific `model_id` and `feature_name` and
    ...
    def compute(
        ...
        await_completion: bool
            Should method wait for completion before exiting.
        max_wait: int (default=600)
            Maximum number of seconds to wait for request to finish processing before raising
            AsyncTimeoutError.
        ...
        ClientError
            ...
            Most likely cause is bad `model_id`.
        AsyncFailureError
    ...
    def list_features_with_samples(cls, project_id: str) -> List[FeaturesWithSamples]:
        """
        Returns a list of feature  - model_id pairs with computed document text extraction samples.

        Parameters
        ----------
        project_id: str
            Project ID to retrieve list of computed samples for
    ...
    def list_pages(
        ...
        model_id: str
            Model ID
        feature_name: str
            Specific feature name to retrieve.
        document_index: Optional[int]
            Default None. Specific document index to retrieve.
        document_task: Optional[str]
            Document Blueprint task
    ...
    def list_documents(
        ...
        """
        Returns list of documents used for text extraction.
