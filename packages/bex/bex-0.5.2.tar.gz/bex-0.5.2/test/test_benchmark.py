...
class DummyExplainer(ExplainerBase):
    ...
    def explain_batch(self, latents: torch.Tensor, logits: torch.Tensor,
        ...
        z = latents[:, None, :].repeat(1, self.num_explanations, 1).to(device)
        z_perturbed = z + torch.rand_like(z)

        return z_perturbed.view(b, self.num_explanations, c)


def test_benchmark_explainers(device):

    bn = Benchmark(n_samples=2, logger=None)
    bn.runs({"explainer": exp, "num_explanations": 2, "device": device} for exp in ["IS", "dice", "dive", "gs", "stylex", "lcf", "xgem", DummyExplainer])
