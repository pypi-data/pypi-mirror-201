...
def raw_submit(discovery_path, spark_home, spark_args, spyt_version=None,
    ...
    if local_source:
        if len(spark_args) >= 1:
            source_file = spark_args[0]
            _, file_extension = os.path.splitext(source_file)
            yt_temp_path = "//tmp/{}{}".format(uuid.uuid4(), file_extension)
            logger.info("%s will be uploaded to YT as %s and used later", source_file, yt_temp_path)
            with open(source_file, 'rb') as source_stream:
                write_file(yt_temp_path, source_stream)
            spark_args = "yt:/{}".format(yt_temp_path) + spark_args[1:]
        else:
            logger.warn("Using local source is enabled, but a file is not provided")
