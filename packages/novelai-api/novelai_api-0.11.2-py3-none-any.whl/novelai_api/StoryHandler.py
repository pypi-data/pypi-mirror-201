...
class NovelAIStory:
    ...
    # prefix of the model, without version
    models_names = {model.value: model for model in Model}
    models_prefix = {"-".join(name.split("-")[:-1]) if "-" else name: model for name, model in models_names.items()}

    def _try_infer_model(self, model: str) -> Model:
        if model in self.models_names:
            return self.models_names[model]

        prefix = "-".join(model.split("-")[:-1]) if "-" in model else model

        if prefix in self.models_prefix:
            return self.models_prefix[prefix]
    ...
    def _handle_preset(self, data: Dict[str, Any]):
        ...
        self.model = self._try_infer_model(settings["model"])
    ...
    def _build_context_from_entries(self, entries: List[Dict[str, Any]]):
        memory_struct = {
            "text": "",
            "contextConfig": {
                "prefix": "",
    ...
    def build_context(self) -> List[int]:
        ...
        memory, an = self.storycontent["data"]["context"]
        entries = [memory, an]

        for entry in self.storycontent["data"]["lorebook"]["entries"]:
...
class NovelAIStoryStorage:
    ...
    async def load_from_remote(self) -> List[NovelAIStory]:
        ...
        print(len(stories), len(storycontents))
