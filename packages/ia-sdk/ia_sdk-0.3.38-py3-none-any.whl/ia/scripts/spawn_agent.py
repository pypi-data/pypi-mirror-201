...
def single_container_start(genome : Genome, 
    ...
    port_connects={}
    # port_offset = random.randrange(0,500)
    port_offset=0
    port_base = 8000
    port_limit = 8200
    if(BIND_PORTS):
        ...
        while port_base + port_offset < port_limit:
            try:
                ...
                if(avail != 0):
                    ...
                    # print(f"avail = {avail}")
                    if(avail != 0):
                        print(f"port {port_base+port_offset} not used. using {port_offset} as offset")
                        break
                port_offset = port_offset + 1
                tmpSocket.close()
                if(port_base + port_offset >= port_limit):
                    print(f"ran out of available ports in range. first available port > {port_limit}")
                    sys.exit(-1)
            except ConnectionRefusedError:
                print("Connection refused when searching for ports :(")
                break
            finally:
                tmpSocket.close()
    ...
    print(f"{genome.primitive_map.keys()}")
    ...
    # print(f"{genome.primitive_map.items()=}")
    print(f"{primitive_genes=}")
    print(f"pre {targets_map=}")
    ...
    print(f"{genome.manipulatives.keys()=}")
    ...
    # print(f"{manipulatives_genes=}")

    manipulatives_genes = str(manipulatives_genes)
    print(f"pre processing {manipulatives_genes=}")
    ...
    # print(f"{manipulatives_genes=}")
    print(f"{manip_ids=}")
    print(f"{manip_targets=}")
    print(f"{manip_sources=}")

    str_genome = genome.topology.__str__()

    print(f"{genome.topology.__str__()=}")

    ASSETS['cp-containers'][_id] = docker_client.containers.run(name=genome.topology["agent"],
        image=f"{REGISTRY}connectome:{VERSION}",
        network=net_name,
        detach=True,
        init=True,
        hostname=options.BOTTLE,
        ports=port_connects,
        ipc_mode="shareable",
        privileged=True,
