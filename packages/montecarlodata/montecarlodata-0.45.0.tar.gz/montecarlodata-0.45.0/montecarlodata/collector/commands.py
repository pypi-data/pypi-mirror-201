...
DEPLOYMENT_OPTIONS = [
    click.option(
        "--aws-profile", required=False, help=f"AWS profile. {PROFILE_VERBIAGE}."
    ),
    click.option(
        "--params",
        required=False,
...
def list_collectors(ctx, active_only):
    CollectorManagementService(config=ctx["config"], aws_wrapper=None).echo_collectors(
...
def get_template(ctx, aws_region, **kwargs):
    ...
    CollectorManagementService(
        config=ctx["config"], aws_region_override=aws_region
