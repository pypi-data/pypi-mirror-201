...
def CMD():
    try:
        ...
        if result==0:
            if args["name"]:
                result= os.system("sudo sed -i 's/{project_name}/'"+args['name']+"'/g' "+ args['path']+"/manage.py")
                result= os.system("sudo sed -i 's/{project_name}/'"+args['name']+"'/g' "+ args['path']+"/app_name/asgi.py")
                result= os.system("sudo sed -i 's/{project_name}/'"+args['name']+"'/g' "+ args['path']+"/app_name/wsgi.py")
