...
from osta.__utils_enrich import __add_org_data, __add_account_data,\
    ...
    __get_driver, __secret_switch, __search_fin_from_stats
...
def getComp(
    ...
    search_website = __secret_switch(**args)
    # Initialize driver
    if search_website:
        driver = __get_driver()
    else:
        ...
        if not pd.isna(bid):
            try:
                res = __fetch_company_based_on_bid(
                    ...
                    search_website=search_website, **args)
...
def getMuniFin(
    ...
    df_org = pd.DataFrame([bid, year], index=["bid", "year"])
