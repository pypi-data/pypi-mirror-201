...
@retry(stop=stop_after_attempt(max_attempt_number=retry_count), wait=wait_fixed(5), reraise=True)
def request_generate_config(fileName, filePath, user, env, token, requestUrl, request_data):
    # 构建其他参数
...
@retry(stop=stop_after_attempt(max_attempt_number=retry_count), wait=wait_fixed(5), reraise=True)
def request_syn_apollo(user, env, publish, token, requestUrl, configTemplates, request_data):
    # 构建其他参数
    request_data['apolloUser'] = user
    request_data['apolloEnv'] = env
...
def generate_config(user, env, token, publish, synApollo,  generateConfigUrl, apolloUrl, configTemplates, request_data):
    ...
    if synApollo:
        request_syn_apollo(user, env, publish, token,
