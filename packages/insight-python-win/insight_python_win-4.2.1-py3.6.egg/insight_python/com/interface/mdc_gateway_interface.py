    ...
    def queryfininfosynchronous(self, querytype, params, securityIDSourceAndTypes=[], securityIdList=[]):
        if self.get_login_flag():
            ...
            if retdatalen == self.ineffective_size:
                ...
                retdatastr = mdc_gateway_client.cdata(rettempdata, self.min_synchronous_data_len + self.error_code_len + self.error_msg_data_len)
                errorcode = retdatastr[self.min_synchronous_data_len:self.min_synchronous_data_len + self.error_code_len]
                errormsglenstr = retdatastr[self.min_synchronous_data_len + self.error_code_len:self.min_synchronous_data_len + self.error_code_len + self.error_msg_data_len]
                ...
                if errormsglen > 0:
                    ...
                    errormsg = errormsgstr[self.min_synchronous_data_len + self.error_code_len + self.error_msg_data_len :errortoatlsize]
