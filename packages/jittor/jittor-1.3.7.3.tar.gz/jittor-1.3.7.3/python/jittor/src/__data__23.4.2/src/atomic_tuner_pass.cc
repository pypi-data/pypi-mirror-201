    ...
    for (uint i=0; i<ir->children.size(); i++) {
        ...
        for (auto j : order) {
            ...
            for (k=count; k<loops.size(); k++)
                if (loops[k]->check_attr("lvalue", idx_name[j]))
                    break;
            if (k<loops.size()) {
        ...
        for (uint j=0;j<atomics.size();j++) {
            ...
            auto check = [&](const string& t, const vector<string>& args, const string& cpu, const string& cuda, const string& acpu, const string& acuda) -> bool {
                ...
                if (var->dtype() != op->input(0)->dtype()) {
                    code = OpCompiler::precompile(defs, 
                        replace(t, "@b", 
                            var->dtype().to_cstring()+string("(@b)"))) + ";";
                } else
...
void AtomicTunerPass::run() {
    ...
    for (uint j=0;j<sfunc.size();j++)
        for (uint i=0; i<ir->children.size(); i++) {
            ...
            func_call->rebuild_scope();
            if (para_opt_level <= 3) {
                // reduce last dim by 4
                // for example, origin source is:
                //      int tn1 = xxx;
                //      int tn0 = xxx;
                //      int tn3 = xxx;
                //      int tn2 = xxx;
                // change tn3 -> min(xxx, max(xxx/4,32))

                int max_dim = 0;
                int max_dim_id = 0;
                for (int i=0; i<sorder[j].size(); i++)
                    if (sorder[j][i] > max_dim) {
                        max_dim = sorder[j][i];
                        max_dim_id = i;
                    }
                if (max_dim_id > 0 && sorder[j][max_dim_id-1] < max_dim) {
                    vector<string> prev_tn;
                    for (uint p=0; p<func_call->children.size(); p++) {
                        auto& x = func_call->children[p];
                        if (x->has_attr("lvalue") && startswith(x->attrs["lvalue"], "tn"+S(max_dim))) {
                            auto& rvalue = x->attrs["rvalue"];
                            ASSERT(startswith(rvalue, "get_thread_range_log"));
                            LOGvvvv << "change rvalue from" << rvalue;
                            auto part = split(rvalue, ",").at(1);
                            part = split(part, ")").at(0);
                            rvalue = "get_thread_range_log(thread_num_left, std::min<int64>("+
                                part + ", std::max<int64>(" + part + "/4,32)))";
                            LOGvvvv << "change rvalue to" << rvalue;
                            if (para_opt_level>=2) {
                                string mytn = "tn"+S(max_dim);
                                for (auto& ptn : prev_tn) {
                                    func_call->insert(p+1, "if ("+mytn+"<5) {int _ = std::min(5-"+mytn+","+ptn+"); "+mytn+"+=_; "+ptn+"-=_;}");
                                }
                            }
                            break;
                        }
                        if (x->has_attr("lvalue") && startswith(x->attrs["lvalue"], "tn")) {
                            prev_tn.push_back(x->attrs["lvalue"]);
                        }
                    }
                    if (para_opt_level>=3) {
                        string range_s;
                        for (auto& ptn : prev_tn) {
                            auto* def = func_call->find_define(ptn);
                            ASSERT(def);
                            auto& rvalue = def->attrs["rvalue"];
                            // LOGir << ptn << rvalue;
                            // LOGir << func_call->to_string();
                            auto part = split(rvalue, ",").at(1);
                            part = split(part, ")").at(0);
                            if (range_s.size())
                                range_s += '*';
                        ...
                        if (range_s.size()) {
                            auto* def = func_call->find_define("thread_num");
                            ASSERT(def);
                            auto& rvalue = def->attrs["rvalue"];
                            int reduce_op_num = 0;
                            for (auto o : op->ops)
                            ...
                            if (op->ops.size() <= reduce_op_num*3)
                                rvalue = "(("+range_s+")<=256)?std::max("+rvalue+"/2,65536):"+rvalue;
                        }
                    }
                }
            }
