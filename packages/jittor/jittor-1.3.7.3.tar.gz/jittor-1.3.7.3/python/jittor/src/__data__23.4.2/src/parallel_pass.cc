...
const char* __get_thread_range_log = 
    "inline static int get_thread_range_log(int& thread_num, int64 range) {"
    "int nbits = NanoVector::get_nbits(std::min((int64)thread_num, range)) - 2;"
    "thread_num >>= nbits;"
    "return nbits;}";
...
void ParallelPass::run() {
    ...
    for (auto sm : cuda_archs) {
        if (sm >= 80)
            default_block_num = 2048;
    }
    int cuda_block_num = to_pow(op->get_loop_option("cuda_block_num", default_block_num));
    ...
    ir->push_back(__get_thread_range_log, &ir->before, true);
