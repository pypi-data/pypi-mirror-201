import pickle
from zlib import compress as cmp
from zlib import decompress as dcmp
from redis import Redis as BaseRedis
from pyredpack.functions import *
...
class Redis(BaseRedis):
    ...
    def get(self, key, pickle_first=False,auto_remove=False):
        encoded = super().get(key)
        if auto_remove:
            super().delete(key)
        return convert_get_type(encoded, pickle_first)

    # TODO  add compress mode msgpack , pickle
    # TODO add compress level
    def cset(self, key, value, ex=None, px=None, nx=False, xx=False)->bool: 
        return super().set(key,cmp( pickle.dumps(convert_set_type(value))))

    def cget(self, key, pickle_first=False,auto_remove=False)->Any:
        encoded = pickle.loads(dcmp(super().get(key)))
        if auto_remove:
            super().delete(key)
        return convert_get_type(encoded, pickle_first)
