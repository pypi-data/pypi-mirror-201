...
class ServiceFactory():
    ...
    def __init__(self, configFilePath):
        ...
        self.system_config = get_dict_from_json_file(configFilePath, startup_text_pre_processor)

        self.services = {}
        for service_name in self.system_config:
            if not SVC_LOAD_IT_KEY in self.system_config[service_name] or self.system_config[service_name][SVC_LOAD_IT_KEY]:
                logger.info("Loading service %s" % service_name)
                service_object = self.init_service_object(service_name)
    ...
    @func_logger
    def init_service_object(self, name):
        if not name in self.system_config:
            raise FactoryServiceException("Invalid service name  was requested name:[%s]" % name)

        service_type_config = self.system_config[name]
...
def get_service_factory(configFilePath=None):
    if not configFilePath:
        configFilePath = bootstrap.get_app_home() + "/config/utah_service_config.pp_json"
