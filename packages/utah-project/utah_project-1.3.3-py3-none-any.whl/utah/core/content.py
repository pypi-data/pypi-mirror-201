...
class AuxContentRef():
    ...
    def __eq__(self, other):
        ret_value = False
        if self.aux_content_type == other.aux_content_type and \
            self.uri == other.uri:
            ret_value = True
...
class Doc():
    ...
    def __eq__(self, other):
        ret_value = False
        if self.resource_header == other.resource_header and \
...
class EHTMLDoc(TextDoc):
    ...
    def __eq__(self, other):
        ret_value = False
        if TextDoc.__eq__(self, other) and \
...
class BaseContentService():
    ...
    def parse_ehtml_from_text(resource_header:ResourceHeader, text:str) -> EHTMLDoc:
    ...
    def write_file(self, doc_uri, file) -> None:
        ...
        if resource_header.render_type == RENDER_TYPE_BINARY:
            ...
            if resource_header.render_type == RENDER_TYPE_TEXT:
                doc = TextDoc(resource_header, text)
