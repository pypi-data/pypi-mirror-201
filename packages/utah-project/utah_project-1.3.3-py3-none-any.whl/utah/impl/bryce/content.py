...
def write_ehtml_doc_to_zip(zip, doc:EHTMLDoc):
    zi = zipfile.ZipInfo(doc.resource_header.uri[1:], zi_tuple(doc.last_modified_date))
...
class ContentService(BaseContentService):
    def __init__(self, mongo_url:str, temp_dir:str, main_microsite: str, default_doc_name: str = "default.html", cache_timeout_secs: int=60, deployment_model:int=0):
...
class PromotionService(BasePromotionService):
    def __init__(self, mongo_url:str, repo_list:list):
