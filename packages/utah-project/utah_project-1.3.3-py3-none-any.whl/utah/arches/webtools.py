...
class BaseFlaskConfig():
    ...
    def locale_from_headers(self):
        ...
        processed_locale = ""
        if raw_locale:
            processed_locale = first_locale(raw_locale)
        else:
...
class FlaskConfigFS(BaseFlaskConfig):
    def __init__(self, 
        ...
        super().__init__(blueprint_module_list, temp_download_path, default_locale, default_timezone, translation_directories, message_domain, host_to_locale_mappings)
...
@lru_cache(500, False)
def first_locale(raw_locale:str):
    return raw_locale.split(",")[0].replace("-", "_")
