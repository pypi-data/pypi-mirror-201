...
def register_form():
    return render_template('registration/register_form.html', field_errors={}, message=gettext("Please enter your registration information"), admin_register=False)
...
def admin_register_form():
    return render_template('registration/register_form.html', field_errors={}, message=gettext("Please enter new user registration information"), admin_register=True)
...
def generic_register(admin_register=False):
    ...
    if len(field_errors) > 0:
        rendering = render_template('registration/register_form.html', 
        ...
        if ai:
            ...
            if authentication_service.is_account_verification_required() and not admin_register:
                ...
                rendering = render_template('registration/verify.html', message=gettext("Verification code was sent to your email address. Please enter for verification."), error=False, user_id=user_id, field_errors={})
            else:
...
def verify_account():
    ...
    if authentication_service.verify(user_id, verification_code):
        rendering = render_template('authentication/login_form.html', go_back="/", user_id=user_id, message=gettext("Your account is now verified. You may log in."), error=False)
    else:
        field_errors = {"verification_code" : gettext("An incorrect verification code was entred") }
        rendering = render_template('registration/verify.html', message=gettext("The verification code entered was incorrect"), error=True, user_id=user_id, field_errors=field_errors)
...
def resend_verification_code():
    ...
    return render_template('registration/verify.html', message=gettext("A new verification code was sent to your email address. Please enter for verification."), error=False, user_id=user_id, field_errors={})


@app.route("/password_change/form", methods=["GET"])
@uri_authorized(URIUSE_SESSION_ONLY)
def change_password_form():
    return render_template("registration/password_change_form.html")
...
def change_password():
    ...
    if len(field_errors) > 0:
        rendering = render_template("/registration/password_change_form.html", message=gettext("Change password failure"), error=True, field_errors=field_errors)
    else:
        rendering = render_template("/common/message.html", message=gettext("Password change was sucessful"))
...
def adding_new_token():
    ...
    return render_template('/registration/token_list.html', authentication_tokens=ai.authentication_tokens, key="_new", description=description, expire_date=expire_date)
...
def add_new_token():
    ...
    if len(field_errors) == 0:
        ...
        return render_template('/registration/token_list.html', authentication_tokens=ai.authentication_tokens)
    else:
        return render_template('/registration/token_list.html', authentication_tokens=ai.authentication_tokens, key="_new", description=description, expire_date=str_expire_date, field_errors=field_errors)
...
def updating_token(inkey):
    ...
    return render_template('/registration/token_list.html', authentication_tokens=ai.authentication_tokens, key=key, description=description, expire_date=expire_date)
...
def update_token(key):
    ...
    if len(field_errors) == 0:
        ...
        return render_template('/registration/token_list.html', authentication_tokens=ai.authentication_tokens, key=key, description=description, expire_date=expire_date, field_errors=field_errors)
...
def get_delete_token(inkey):
    ...
    return render_template('/registration/token_list.html', authentication_tokens=ai.authentication_tokens, key=key, delete=delete)
