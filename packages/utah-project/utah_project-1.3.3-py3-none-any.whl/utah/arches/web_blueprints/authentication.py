...
def login_form():
    ...
    return render_template('authentication/login_form.html', go_back=go_back)
...
def login():
    ...
    if authentication_service.authenticate(user_id, password):
        ...
        if not ai.verification_code:
            ...
            rendering = render_template('registration/verify.html', message=gettext("This account is unverified. Please enter the verification code that was send to your registered email address."), error=True, user_id=user_id, field_errors={})

    else:
        rendering = render_template('authentication/login_form.html', go_back=go_back, user_id=user_id, message=gettext("Login Failure"), error=True)
