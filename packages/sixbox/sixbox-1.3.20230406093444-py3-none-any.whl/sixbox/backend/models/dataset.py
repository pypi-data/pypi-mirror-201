...
from sixbox.backend.models.link import Link
from sixbox.backend.models.member import Member
from sixbox.backend.decorators import inplace_reload
from sixbox.backend.meta.resource import Resource
from sixbox.backend.meta.collection import Collection
from sixbox.backend.meta.transformer import Transform
from sixbox.backend.meta.fields import HrefField, StringField
...
class Dataset(Resource):
    ...
    access_key_id= StringField(read_only=True)
    access_key_secret= StringField(read_only=True)
    security_token = StringField(read_only=True)
    Bucket = StringField(read_only=True)
    Region = StringField(read_only=True)
    ossdir = StringField(read_only=True)
    callbackUrl = StringField(read_only=True)
    ...
    @classmethod
    def oss_token(cls, api=None):

        api = api if api else cls._API

        # logger.info('Get pipe', extra=extra)
        data = api.get(url=cls._URL['oss_token'].format(
            )).json()["data"]

        return Dataset(api=api, **data)
