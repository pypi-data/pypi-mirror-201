...
from hbshare.quant.CChen.cons import sql_write_path_hb, HSJY_VIRTUAL_CONTRACT
...
def nav_lines(
        ...
        cal_path=sql_write_path_hb['work'],
        all_selected=True,
        print_info=True
):
    if axis_cross:
...
def market_win(start_date, end_date, freq='D', table='index_win', sql_path=sql_write_path_hb['daily']):
...
def gzb_xs_position(data, width=800, height=600):
    ...
    web = Pie(
        ...
        title_opts=opts.TitleOpts(title=name + ' ' + date.strftime('%Y/%m/%d') + " 持仓分布"),
        legend_opts=opts.LegendOpts(
            is_show=False,
            pos_left="left",
            orient="vertical"
