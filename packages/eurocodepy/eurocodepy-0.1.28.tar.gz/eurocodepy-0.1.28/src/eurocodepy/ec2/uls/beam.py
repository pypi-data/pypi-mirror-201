...
class RCBeam:
    ...
    def calcBending(self, med: float, delta: float = 1.0, iprint: bool=False) -> Tuple[float, float, float, float, float]:
        ...
        epsyd = self.fyd/ec.ReinforcementClasses[self.reinf]['Es']
        epssc = (alpha-self.ac/self.d)/alpha*ec.ConcreteClasses[self.conc]['epscu2']
        return aslt, aslc, alpha, epsst, epssc

if __name__ == "__main__":
    # test RCBeam class
    print("\nTest RCBeam:\n")
    beam = RCBeam(0.3, 0.5, at=0.05, ac=0.05, conc="C30/37", reinf="A500NR")
