...
def getHubAndRound(client_nr):
    try: 
        ...
        sql = """
                ...
                BEGIN
                    SELECT CustomerIdPost, Hub,RouteName,Name FROM SFPickupLocations WHERE CustomerIdPost = @CustomerIdPost
                END
                ELSE
                ...
                BEGIN
                    SELECT CustomerIdPost, Hub,RouteName,Name FROM SFPickupLocations WHERE SFId = (SELECT SFId FROM SFDepartments WHERE CustomerIdPost = @CustomerIdPost)
                END
                ELSE
                ...
                BEGIN
                    SELECT CustomerIdPost, Hub, RouteName, Name FROM SFAccounts WHERE CustomerIdPost = @CustomerIdPost
                END
                ELSE
                ...
                BEGIN
                    SELECT CustomerId,HubName as Hub, RoundName as RouteName, c.Name  FROM EasyPostLogistiek.dbo.CustomerHub ch LEFT JOIN EasyPost.dbo.Companies c on ch.CustomerId=c.Id WHERE CustomerId = @CustomerIdPost
                END
                ELSE
