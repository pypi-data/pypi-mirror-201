...
def getHubAndRound(client_nr):
    try: 
        db_conn = pyodbc.connect(driver = '{SQL Server Native Client 11.0}',
                                       ...
                                       user = 'sa',
                                       password = 'sql')
        sql = """SELECT inr.CustomerIdPost,inr.CustomerParentIdPost,inr.Hub as Hub, outr.Hub as ParentHub, inr.RouteName as RouteName, outr.RouteName as ParentRouteName, inr.Name
  ...
  WHERE inr.CustomerIdPost = {}""".format(client_nr)
        results = []
        with db_conn.cursor() as cursor:
            cursor.execute(sql)
            columns = [column[0] for column in cursor.description]
