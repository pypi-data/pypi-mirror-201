...
async def open_map(filename: str, program: str = 'navigator', ext: Optional[str] = 'json'):
    ...
    if model_file.is_file():
        try:
            ...
            raise ComponentError(
                f"Task: Open Map Error: {err}"
            ) from err
    else:
        return False

async def open_model(filename: str, program: str = 'navigator', ext: Optional[str] = 'json'):
