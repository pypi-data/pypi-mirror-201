...
MISTRESSHUB_URL = os.getenv('MISTRESSHUB_URL', 'https://hub.mistressgpt.top')
SEPCHAR = '👠'
HIDE_MY_KEY = False
MODELS = [
    "gpt-3.5-turbo",
...
def share_prompt(systemPromptTxt):
    ri = RoleInfo(**yaml.safe_load(systemPromptTxt))
    try:
        ri_dict = ri.dict()
        ri_dict['mistress_tasks'] = SEPCHAR.join(ri_dict['mistress_tasks'])
        mistresshub_url = MISTRESSHUB_URL
        logger.debug(f'向 {mistresshub_url} 分享模板: {ri_dict}')
        response = requests.post(
            url=f"{mistresshub_url}/mistresses",
            headers={
                "Content-Type": "application/json; charset=utf-8",
            },
            data=json.dumps(ri_dict, ensure_ascii=False).encode('utf-8')
        )
        if response.status_code != 200:
            logger.error(f"分享失败! {response.json()}")
            raise gr.Error(f"分享失败! {response.json()}")

        logger.info(f'分享结果: {response.json()}')
    except Exception as e:
        logger.error(f"分享结果失败: {e}")
        raise gr.Error(f"分享失败! {e}")
...
def get_templates(role_cards, plain=False):
    ...
    if type(role_cards) is not dict:
        role_cards = role_cards.value
    if mistresshub_url is not None:
        logger.debug(f'从 {mistresshub_url} 获取模板列表')
        # 从网上获取模板列表
        try:
            external_role_cards = {}
            r = requests.get(f'{mistresshub_url}/mistresses')
            if r.status_code == 200:
                ext_templates = r.json()
                for _template in ext_templates:
                    _template['mistress_tasks'] = _template['mistress_tasks'].split(
                        SEPCHAR)
                    mistress_name = _template['mistress_name']
                    # offset = 1
                    # while mistress_name in role_cards.keys():
                    #     mistress_name = _template['mistress_name'] + f'({offset})'
                    #     offset += 1
                    # _template['mistress_name'] = mistress_name

                    external_role_cards[mistress_name] = RoleInfo(
                        **_template)
                logger.debug(f'外部模板:{external_role_cards}')
                role_cards = {**role_cards, **external_role_cards}

            status_display = '从网络获取模板成功!'
        except Exception as e:
            logger.error(f"从网络获取模板失败! {e}")
            status_display = '从网络获取模板失败!'
    # roles = {}
    ...
    if plain is True:
        return templates, role_cards
    else:
        # 选择最新的那个prompt
        return gr.Dropdown.update(choices=templates, value=templates[-1]), role_cards, gr.Markdown.update(value=status_display)
...
#     """
#     logger.info(f"从用户 {user_name} 中获取历史记录文件名列表")
#     return ['s', 'k']
...
def predict(
    ...
    logger.debug(f'本次消息发送的APIKEY为{openai_api_key}')
    ...
    logger.debug(f'预测返回结果:{parsed_response}')
    ...
    with gr.Blocks(css=customCSS, theme=small_and_beautiful_theme, title=page_title) as demo:
        ...
        with gr.Row().style(equal_height=True):
            ...
            with gr.Column(scale=1, min_width=400):
                with gr.Column(min_width=400):
                    role_names, role_cards.value = get_templates(
                        role_cards, plain=True)
        ...
        get_all_templates_args = dict(
            fn=get_templates, inputs=[role_cards], outputs=[
                templateSelectDropdown, role_cards, status_display]
        )
        ...
        sharePromptBtn.click(
            share_prompt, [systemPromptTxt], [status_display]).then(
            **get_all_templates_args
        )
