...
class CrossPyArray(numpy.lib.mixins.NDArrayOperatorsMixin):
    ...
    def __init__(self, obj, dim: Optional[int] = None, shape = None, **kwargs) -> None:
        ...
        def _get_shape(block):
            if hasattr(block, 'shape'):
                return getattr(block, 'shape')
            elif initial_shapes and id(block) in initial_shapes:
                return initial_shapes[id(block)]
            else:
                raise Exception("Unknown shape for block %d" % block)  # TODO len()?
        ...
        for i, block in enumerate(list_obj):
            ...
            if self._concat_dim is not None:
                ...
                self._device_to_indices[repr(device)].append(block)
            self._device_to_blocks[repr(device)].append(block)
    ...
    def _init_shape(self, shapes: Sequence[ShapeType]) -> ShapeType:
        ...
        if self._concat_dim is None:
            final_shape: ShapeType = (len(shapes), *shapes[0])
        else:
            # merge shapes
    ...
    def data_map(self):
        ...
        if self._concat_dim == 0:
            ...
            bounds = numpy.cumsum([0] + [x.shape[self._concat_dim] for x in self._original_data])
            return {(*((0, s) for s in subshape[:self._concat_dim]), (bounds[i], bounds[i+1]), *((0, s) for s in subshape[self._concat_dim+1:])): self._original_data[i] for i in range(len(self._original_data))}
    ...
    def _global_index_to_block_id(self, i):
        ...
        """
        return numpy.sum(numpy.expand_dims(i, axis=-1) >= self._bounds, axis=-1, keepdims=False)
    ...
    def _indexing_check(self, index: IndexType):
        ...
        assert isinstance(index, (int, slice)) or isinstance(index, list) and all(isinstance(i, (int, slice)) for i in index), NotImplementedError("Only support 1-D indexing")
    ...
    def __getitem__(self, index: IndexType):  # -> Union[Array, List[Array]]
        ...
        if isinstance(index, (*self._device_types, str)):
            ...
            return CrossPyArray([self.block_at(slice_.start) for slice_ in slices], dim=0)
        ...
        if self._concat_dim == 0 and len(index) == 1:
            if isinstance(index[0], slice):
                ...
                if len(relevant_blocks) == 1:
                    singleton = relevant_blocks[0][is_partial_head and head_start or None: is_partial_tail and tail_stop or None]
                    return CrossPyArray(singleton, dim = None)
            ...
            if isinstance(index[0], list) and all(isinstance(i, int) for i in index[0]):
                list_ = index[0]
                block_ids = self._global_index_to_block_id(list_)
                assert len(block_ids) == len(list_)
        ...
        if self._concat_dim == 0 and len(index) == 1:
            if isinstance(index[0], slice):
                ...
                for source_global_stop in bounds:
                    ...
                    source_obj = value._original_data[source_block_id]
    ...
    def _locate(self, global_index):
        ...
        local_int = global_int - l2g_offset(block_id)
