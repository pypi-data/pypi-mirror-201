    ...
    *,
    ...
    if is_array(obj):  # numpy, cupy, crosspy
        if partition is not None:
            ...
            if len(obj) == 1:
                arr = CrossPyArray(obj[0], dim=None, initial_devices=wrapper.initial_devices, initial_shapes=wrapper.initial_shapes)
            else:
                dim = 0  # TODO warning when dim is not 0
                arr = CrossPyArray(obj, dim=dim, initial_devices=wrapper.initial_devices, initial_shapes=wrapper.initial_shapes)
        else:
        ...
        try:
            ...
            def _recursive_parse(list_of_obj, d, wrapper):
                if all(is_array(a) for a in list_of_obj):
                    if wrapper:
                        wrapper = PerObjWrapper(wrapper, perserve_device=True, perserve_shape=True)
                        return CrossPyArray([wrapper(obj)for obj in list_of_obj], dim=d, initial_devices=wrapper.initial_devices, initial_shapes=wrapper.initial_shapes)
                    return CrossPyArray(list_of_obj, dim=d, initial_devices=wrapper.initial_devices, initial_shapes=wrapper.initial_shapes)
