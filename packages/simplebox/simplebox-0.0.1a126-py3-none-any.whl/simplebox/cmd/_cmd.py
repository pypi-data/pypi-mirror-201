...
class Command(metaclass=StaticClass):
    ...
    @staticmethod
    def executed(cmd, cwd: str or Path = None, timeout: int = None, encoding: str = None, **kwargs) -> '_Executor':
        """
    ...
    @staticmethod
    def runned(*popenargs, cwd: str or Path = None, input_=None, capture_output=False, timeout=None, check=False,
               encoding: str = None, **kwargs) -> '_Runner':
        """
        Quickly execute the subprocess run and return the result.
        """
        return Command.run(*popenargs, cwd=cwd, input=input_, capture_output=capture_output, timeout=timeout,
...
class _Executor(_AbstractCommand):
    ...
    def finish(self) -> '_Executor':
        ...
        while self.__process.poll() is None:
            run_time = time() - t1
            if self.__timeout:
                if run_time > self.__timeout:
                    self.__process.kill()
                    raise_exception(TimeoutExpiredException("exec commandline timeout!!!"))
            else:
                curr_time = time()
                if int(run_time % 60) == 0 and curr_time - last_time > 60:
                    last_time = curr_time
                    _logger.warning(
                        f"command '{self.__cwd} => {self.__cmd}' run time is more than {run_time:.0f} seconds..."
                    )

        self.build_log_meta(self.__cmd, self.__cwd, self.__process.returncode,
