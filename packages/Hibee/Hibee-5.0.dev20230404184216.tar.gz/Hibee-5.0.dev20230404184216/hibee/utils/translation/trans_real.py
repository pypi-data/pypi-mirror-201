...
from hibee.conf.locale import LANG_INFO
from hibee.core.exceptions import AppRegistryNotReady
...
class TranslationCatalog:
    """
    Simulate a dict for HibeeTranslation._catalog so as multiple catalogs
...
class HibeeTranslation(gettext_module.GNUTranslations):
    """
    ...
    def __init__(self, language, domain=None, localedirs=None):
        ...
        if self.domain == "hibee":
            if localedirs is not None:
                # A module-level cache is used for caching 'hibee' translations
                warnings.warn(
                    "localedirs is ignored when domain is 'hibee'.", RuntimeWarning
                )
                localedirs = None
    ...
    def __repr__(self):
        return "<HibeeTranslation lang:%s>" % self.__language
    ...
    def _init_translation_catalog(self):
        """Create a base catalog using global hibee translations."""
    ...
    def _add_fallback(self, localedirs=None):
        ...
        if self.domain == "hibee":
            ...
            default_translation = HibeeTranslation(
    ...
    def merge(self, other):
        ...
        if self._catalog is None:
            # Take plural and _info from first catalog found (generally Hibee's).
...
def translation(language):
    """
    Return a translation object in the default 'hibee' domain.
    """
    global _translations
    if language not in _translations:
        _translations[language] = HibeeTranslation(language)
...
def check_for_language(lang_code):
    ...
    <https://www.hibeeproject.com/weblog/2007/oct/26/security-fix/>.
    """
    # First, a quick check to make sure lang_code is well-formed (#21458)
    if lang_code is None or not language_code_re.search(lang_code):
        return False
    return any(
        gettext_module.find("hibee", path, [to_locale(lang_code)]) is not None
