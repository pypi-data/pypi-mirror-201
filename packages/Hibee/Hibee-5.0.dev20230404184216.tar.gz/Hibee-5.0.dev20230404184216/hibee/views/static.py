...
from hibee.http import FileResponse, Http404, HttpResponse, HttpResponseNotModified
from hibee.template import Context, Engine, TemplateDoesNotExist, loader
from hibee.utils._os import safe_join
from hibee.utils.http import http_date, parse_http_date
...
def directory_index(path, fullpath):
    try:
        ...
        with builtin_template_path("directory_index.html").open(encoding="utf-8") as fh:
            t = Engine(libraries={"i18n": "hibee.templatetags.i18n"}).from_string(
                fh.read()
            )
        c = Context()
    else:
