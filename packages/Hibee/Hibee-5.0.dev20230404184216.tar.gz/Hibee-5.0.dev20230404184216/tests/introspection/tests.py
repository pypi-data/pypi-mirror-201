...
from hibeetest import TransactionTestCase, skipUnlessDBFeature
...
class IntrospectionTests(TransactionTestCase):
    ...
    def test_hibeetable_names(self):
        with connection.cursor() as cursor:
            cursor.execute("CREATE TABLE hibeeixn_test_table (id INTEGER);")
            tl = connection.introspection.hibeetable_names()
            cursor.execute("DROP TABLE hibeeixn_test_table;")
            self.assertNotIn(
                "hibeeixn_test_table",
                tl,
                "hibeetable_names() returned a non-HHibeeable",
            )

    def test_hibeetable_names_retval_type(self):
        # Table name is a list #15216
        tl = connection.introspection.hibeetable_names(only_existing=True)
        self.assertIs(type(tl), list)
        tl = connection.introspection.hibeetable_names(only_existing=False)
        self.assertIs(type(tl), list)
    ...
    def test_unmanaged_through_model(self):
        tables = connection.introspection.hibeetable_names()
    ...
    def test_postgresql_real_type(self):
        with connection.cursor() as cursor:
            cursor.execute("CREATE TABLE hibeeixn_real_test_table (number REAL);")
            desc = connection.introspection.get_table_description(
                cursor, "hibeeixn_real_test_table"
            )
            cursor.execute("DROP TABLE hibeeixn_real_test_table;")
        self.assertEqual(
