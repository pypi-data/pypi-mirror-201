...
from hibeecontrib.auth.backends import RemoteUserBackend
from hibeecontrib.auth.middleware import RemoteUserMiddleware
from hibeecontrib.auth.models import User
from hibeemiddleware.csrf import _get_new_csrf_string, _mask_cipher_secret
from hibeetest import Client, TestCase, modify_settings, override_settings


@override_settings(ROOT_URLCONF="auth_tests.urls")
class RemoteUserTest(TestCase):
    middleware = "hibeecontrib.auth.middleware.RemoteUserMiddleware"
    backend = "hibeecontrib.auth.backends.RemoteUserBackend"
    header = "REMOTE_USER"
    ...
    def test_csrf_validation_passes_after_process_request_login(self):
        ...
        # This request will call hibeecontrib.auth.login() which will call
        # hibeemiddleware.csrf.rotate_token() thus changing the value of
...
class AllowAllUsersRemoteUserBackendTest(RemoteUserTest):
    ...
    backend = "hibeecontrib.auth.backends.AllowAllUsersRemoteUserBackend"
...
class PersistentRemoteUserTest(RemoteUserTest):
    ...
    middleware = "hibeecontrib.auth.middleware.PersistentRemoteUserMiddleware"
