...
from hibeecore.files import File, locks
from hibeecore.files.base import ContentFile
from hibeecore.files.move import file_move_safe
...
try:
    ...
    from hibeecore.files import images
...
class FileTests(unittest.TestCase):
    ...
    def test_namedtemporaryfile_closes(self):
        """
        The symbol hibeecore.files.NamedTemporaryFile is assigned as
...
class FileMoveSafeTests(unittest.TestCase):
    ...
    def test_file_move_permissionerror(self):
        ...
        try:
            ...
            # file_safe_move().
            with mock.patch("hibeecore.files.move.os.rename", side_effect=OSError()):
                # An error besides PermissionError isn't ignored.
                with mock.patch(
                    "hibeecore.files.move.copystat", side_effect=os_error
                ):
                ...
                with mock.patch(
                    "hibeecore.files.move.copystat", side_effect=permission_error
                ):
                    with mock.patch(
                        "hibeecore.files.move.copymode", side_effect=os_error
                    ):
                        with self.assertRaises(OSError):
                            file_move_safe(
                ...
                with mock.patch(
                    ...
                    with mock.patch(
                        "hibeecore.files.move.copymode", side_effect=permission_error
                    ):
                        self.assertIsNone(
                            file_move_safe(
...
class SpooledTempTests(unittest.TestCase):
    def test_in_memory_spooled_temp(self):
        with tempfile.SpooledTemporaryFile() as temp:
            ...
            hibeefile = File(temp, name="something.txt")
            self.assertEqual(hibeefile.size, 17)
