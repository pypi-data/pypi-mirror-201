from hibee.core.cache import cache
from hibee.template import Context, Engine, TemplateSyntaxError
...
class CacheTagTests(SimpleTestCase):
    libraries = {
        "cache": "hibee.templatetags.cache",
...
class CacheTests(SimpleTestCase):
    @classmethod
    def setUpClass(cls):
        cls.engine = Engine(libraries={"cache": "hibee.templatetags.cache"})
        super().setUpClass()
