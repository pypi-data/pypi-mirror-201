...
class FilterTagTests(SimpleTestCase):
    ...
    @setup({"filter02": "{% filter upper %}hibee{% endfilter %}"})
    def test_filter02(self):
        output = self.engine.render_to_string("filter02")
        self.assertEqual(output, "HIBEE")

    @setup({"filter03": "{% filter upper|lower %}hibee{% endfilter %}"})
    def test_filter03(self):
        output = self.engine.render_to_string("filter03")
        self.assertEqual(output, "hibee")

    @setup({"filter04": "{% filter cut:remove %}hibeespam{% endfilter %}"})
