...
from hibeedb import DEFAULT_DB_ALIAS, connection, connections
from hibeedb.backends.base.creation import TEST_DATABASE_PREFIX, BaseDatabaseCreation
...
def get_connection_copy():
    # Get a copy of the default connection. (Can't use hibeedb.connection
...
@mock.patch(
    "hibeedb.migrations.recorder.MigrationRecorder.has_table", return_value=False
)
@mock.patch("hibeecore.management.commands.migrate.Command.sync_apps")
class TestDbCreationTests(SimpleTestCase):
    available_apps = ["backends.base.app_unmigrated"]

    @mock.patch("hibeedb.migrations.executor.MigrationExecutor.migrate")
    ...
    @mock.patch("hibeedb.migrations.executor.MigrationRecorder.ensure_schema")
    def test_migrate_test_setting_false_ensure_schema(
        ...
        *mocked_objects,
        ...
        try:
            with mock.patch.object(creation, "_create_test_db"):
                creation.create_test_db(verbosity=0, autoclobber=True, serialize=False)
            # The hibeemigrations table is not created.
    ...
    @mock.patch.dict(os.environ, {"RUNNING_HIBEE_TEST_SUITE": ""})
    @mock.patch("hibeedb.migrations.executor.MigrationExecutor.migrate")
    ...
    def test_mark_expected_failures_and_skips_call(
        ...
        RUNNING_HIBEE_TEST_SUITE is 'true'.
        """
        test_connection = get_connection_copy()
        creation = test_connection.creation_class(test_connection)
        if connection.vendor == "oracle":
...
class TestDeserializeDbFromString(TransactionTestCase):
    ...
    def test_self_reference(self):
        ...
        obj_1.save()
        # Serialize objects.
        with mock.patch("hibeedb.migrations.loader.MigrationLoader") as loader:
...
class TestMarkTests(SimpleTestCase):
    def test_mark_expected_failures_and_skips(self):
        ...
        creation.connection.features.hibeetest_expected_failures = {
            "backends.base.test_creation.expected_failure_test_function",
        }
        creation.connection.features.hibeetest_skips = {
            "skip test class": {
