...
from hibeedb import DEFAULT_DB_ALIAS, connection, connections, transaction
...
from hibeetest.utils import CaptureQueriesContext, override_settings
...
class DatabaseWrapperLoggingTests(TransactionTestCase):
    ...
    def test_commit_debug_log(self):
        ...
        with CaptureQueriesContext(conn):
            with self.assertLogs("hibeedb.backends", "DEBUG") as cm:
                ...
                self.assertRegex(
                    cm.output[0],
                    r"DEBUG:hibeedb.backends:\(\d+.\d{3}\) "
    ...
    def test_no_logs_without_debug(self):
        with self.assertNoLogs("hibeedb.backends", "DEBUG"):
            with self.assertRaises(Exception), transaction.atomic():
                Person.objects.create(first_name="first", last_name="last")
                raise Exception("Force rollback")
