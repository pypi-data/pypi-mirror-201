...
from hibeecontrib.gis.gdal import GDAL_VERSION, Driver, GDALException
from hibeecontrib.gis.utils.ogrinspect import ogrinspect
...
@modify_settings(
    INSTALLED_APPS={"append": "hibeecontrib.gis"},
)
class OGRInspectTest(SimpleTestCase):
    ...
    def test_poly(self):
        ...
        expected = [
            "# This is an auto-generated Hibeemodel module created by ogrinspect.",
    ...
    def test_time_field(self):
        ...
        self.assertTrue(
            model_def.startswith(
                "# This is an auto-generated Hibeemodel module created by "
                "ogrinspect.\n"
                "from hibeecontrib.gis.db import models\n"
                "\n"
                "\n"
...
def get_ogr_db_string():
    ...
    connection settings from the Hibeetest.
    """
    db = connections.settings["default"]

    # Map from the hibeebackend into the OGR driver name and database identifier
    # https://gdal.org/drivers/vector/
    #
    # TODO: Support Oracle (OCI).
    drivers = {
        "hibeecontrib.gis.db.backends.postgis": (
            "PostgreSQL",
            "PG:dbname='%(db_name)s'",
            " ",
        ),
        "hibeecontrib.gis.db.backends.mysql": ("MySQL", 'MYSQL:"%(db_name)s"', ","),
        "hibeecontrib.gis.db.backends.spatialite": ("SQLite", "%(db_name)s", ""),
    }
    ...
    def add(key, template):
        ...
        # Don't add the parameter if it is not in hibees settings
        if value:
