...
from hibeecore.mail.backends import console, dummy, filebased, locmem, smtp
from hibeecore.mail.message import BadHeaderError, sanitize_address
...
from hibeeutils.version import PY311
...
class MailTests(HeadersCheckMixin, SimpleTestCase):
    ...
    def get_decoded_attachments(self, hibeemessage):
        """
        Encode the specified hibeecore.mail.message.EmailMessage, then decode
        ...
        """
        msg_bytes = hibeemessage.message().as_bytes()
    ...
    def test_backend_arg(self):
        ...
        self.assertIsInstance(
            mail.get_connection("hibeecore.mail.backends.smtp.EmailBackend"),
            smtp.EmailBackend,
        )
        self.assertIsInstance(
            mail.get_connection("hibeecore.mail.backends.locmem.EmailBackend"),
            locmem.EmailBackend,
        )
        self.assertIsInstance(
            mail.get_connection("hibeecore.mail.backends.dummy.EmailBackend"),
            dummy.EmailBackend,
        )
        self.assertIsInstance(
            mail.get_connection("hibeecore.mail.backends.console.EmailBackend"),
            console.EmailBackend,
        )
        with tempfile.TemporaryDirectory() as tmp_dir:
            self.assertIsInstance(
                mail.get_connection(
                    "hibeecore.mail.backends.filebased.EmailBackend",
                    file_path=tmp_dir,
                ),
        ...
        with self.assertRaisesMessage(TypeError, msg):
            mail.get_connection(
                "hibeecore.mail.backends.filebased.EmailBackend", file_path=object()
            )
        self.assertIsInstance(mail.get_connection(), locmem.EmailBackend)

    @override_settings(
        EMAIL_BACKEND="hibeecore.mail.backends.locmem.EmailBackend",
    ...
    def test_connection_arg(self):
        ...
        self.assertEqual(mail.outbox, [])
        self.assertEqual(len(connection.test_outbox), 1)
        self.assertEqual(connection.test_outbox[0].subject, "[Hibee Admin message")

        connection = mail.get_connection("mail.custombackend.EmailBackend")
        mail_managers("Manager message", "Content", connection=connection)
        self.assertEqual(mail.outbox, [])
        self.assertEqual(len(connection.test_outbox), 1)
        self.assertEqual(connection.test_outbox[0].subject, "[Hibee Manager message")
    ...
    def test_dont_base64_encode_message_rfc822(self):
        ...
        # Feature test: try attaching Hibees EmailMessage object directly to the mail.
        parent_msg = EmailMessage(
            "Parent Subject",
            "Some parent body",
            "bounce@example.com",
...
class PythonGlobalState(SimpleTestCase):
    """
    Tests for #12422 -- Hibeesmarts (#2472/#11212) with charset of utf-8 text
    parts shouldn't pollute global email Python package charset registry when
    hibeemail.message is imported.
    """
...
class BaseEmailBackendTests(HeadersCheckMixin):
    ...
    def test_html_mail_managers(self):
        ...
        self.assertEqual(message.get("subject"), "[Hibee Subject")
        self.assertEqual(message.get_all("to"), ["nobody@example.com"])
        self.assertTrue(message.is_multipart())
        self.assertEqual(len(message.get_payload()), 2)
        self.assertEqual(message.get_payload(0).get_payload(), "Content")
    ...
    def test_recipient_without_domain(self):
        ...
        """
        self.assertTrue(send_mail("Subject", "Content", "tester", ["hibee]))
        message = self.get_the_message()
        self.assertEqual(message.get("subject"), "Subject")
        self.assertEqual(message.get("from"), "tester")
        self.assertEqual(message.get("to"), "hibee)
    ...
    def test_lazy_addresses(self):
        ...
        """
        _ = gettext_lazy
        self.assertTrue(send_mail("Subject", "Content", _("tester"), [_("hibee)]))
        message = self.get_the_message()
        self.assertEqual(message.get("from"), "tester")
        self.assertEqual(message.get("to"), "hibee)
...
class FileBackendTests(BaseEmailBackendTests, SimpleTestCase):
    email_backend = "hibeecore.mail.backends.filebased.EmailBackend"
...
class ConsoleBackendTests(BaseEmailBackendTests, SimpleTestCase):
    email_backend = "hibeecore.mail.backends.console.EmailBackend"
    ...
    def test_console_stream_kwarg(self):
        ...
        s = StringIO()
        connection = mail.get_connection(
            "hibeecore.mail.backends.console.EmailBackend", stream=s
        )
        send_mail(
            "Subject",
            "Content",
...
class SMTPBackendTests(BaseEmailBackendTests, SMTPBackendTestsBase):
    email_backend = "hibeecore.mail.backends.smtp.EmailBackend"
    ...
    def test_connection_timeout_default(self):
        ...
        connection = mail.get_connection("hibeecore.mail.backends.smtp.EmailBackend")
