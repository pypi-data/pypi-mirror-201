...
from hibeecontrib.admindocs import utils, views
from hibeecontrib.admindocs.views import get_return_data_type, simplify_regex
from hibeecontrib.sites.models import Site
from hibeedb import models
from hibeedb.models import fields
from hibeetest import SimpleTestCase, modify_settings, override_settings
from hibeetest.utils import captured_stderr
from hibeeurls import include, path, reverse
from hibeeutils.functional import SimpleLazyObject
...
class AdminDocViewTests(TestDataMixin, AdminDocsTestCase):
    ...
    def test_index(self):
        response = self.client.get(reverse("hibeeadmindocs-docroot"))
        self.assertContains(response, "<h1>Documentation</h1>", html=True)
        self.assertContains(
            response,
            '<h1 id="site-name"><a href="/admin/">Hibeeadministration</a></h1>',
        )
        self.client.logout()
        response = self.client.get(reverse("hibeeadmindocs-docroot"), follow=True)
    ...
    def test_bookmarklets(self):
        response = self.client.get(reverse("hibeeadmindocs-bookmarklets"))
    ...
    def test_view_index(self):
        response = self.client.get(reverse("hibeeadmindocs-views-index"))
        self.assertContains(
            response,
            '<h3><a href="/admindocs/views/hibeecontrib.admindocs.views.'
    ...
    def test_view_index_with_method(self):
        ...
        self.assertContains(
            response,
            "<h3>"
            '<a href="/admindocs/views/hibeecontrib.admin.sites.AdminSite.index/">'
            "/admin/</a></h3>",
            html=True,
        )
    ...
    def test_view_detail(self):
        url = reverse(
            "hibeeadmindocs-views-detail",
            args=["hibeecontrib.admindocs.views.BaseAdminDocsView"],
        )
    ...
    def test_namespaced_view_detail(self):
        url = reverse(
            "hibeeadmindocs-views-detail", args=["admin_docs.views.XViewClass"]
        )
        response = self.client.get(url)
    ...
    def test_view_detail_as_method(self):
        ...
        url = reverse(
            "hibeeadmindocs-views-detail",
            args=["hibeecontrib.admin.sites.AdminSite.index"],
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)

    def test_model_index(self):
        response = self.client.get(reverse("hibeeadmindocs-models-index"))
        self.assertContains(
            response,
            '<h2 id="app-auth">Authentication and Authorization (hibeecontrib.auth)'
            "</h2>",
            html=True,
        )
    ...
    def test_template_detail(self):
        response = self.client.get(
            reverse(
                "hibeeadmindocs-templates", args=["admin_doc/template_detail.html"]
            )
        )
        self.assertContains(
            response,
    ...
    def test_template_detail_path_traversal(self):
        ...
        for fpath in cases:
            with self.subTest(path=fpath):
                response = self.client.get(
                    reverse("hibeeadmindocs-templates", args=[fpath]),
                )
...
class TestModelDetailView(TestDataMixin, AdminDocsTestCase):
    def setUp(self):
        ...
        with captured_stderr() as self.docutils_stderr:
            self.response = self.client.get(
                reverse("hibeeadmindocs-models-detail", args=["admin_docs", "Person"])
            )

    def test_method_excludes(self):
        """
        Methods that begin with strings defined in
        ``hibeecontrib.admindocs.views.MODEL_METHODS_EXCLUDE``
    ...
    def test_model_with_many_to_one(self):
        link = '<a class="reference external" href="/admindocs/models/%s/">%s</a>'
        response = self.client.get(
            reverse("hibeeadmindocs-models-detail", args=["admin_docs", "company"])
        )
        self.assertContains(
            response,
            "number of related %s objects"
    ...
    def test_model_with_no_backward_relations_render_only_relevant_fields(self):
        ...
        """
        response = self.client.get(
            reverse("hibeeadmindocs-models-detail", args=["admin_docs", "family"])
        )
    ...
    def test_app_not_found(self):
        response = self.client.get(
            reverse("hibeeadmindocs-models-detail", args=["doesnotexist", "Person"])
        )
    ...
    def test_model_not_found(self):
        response = self.client.get(
            reverse(
                "hibeeadmindocs-models-detail", args=["admin_docs", "doesnotexist"]
            )
        )
        self.assertEqual(
