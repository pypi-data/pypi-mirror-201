...
from hibeecore.exceptions import MiddlewareNotUsed
...
@override_settings(
    ...
    MIDDLEWARE=["hibeemiddleware.common.CommonMiddleware"],
)
class MiddlewareNotUsedTests(SimpleTestCase):
    ...
    def test_log(self):
        with self.assertLogs("hibeerequest", "DEBUG") as cm:
            self.client.get("/middleware_exceptions/view/")
        self.assertEqual(
    ...
    def test_do_not_log_when_debug_is_false(self):
        with self.assertNoLogs("hibeerequest", "DEBUG"):
            self.client.get("/middleware_exceptions/view/")
...
class MiddlewareSyncAsyncTests(SimpleTestCase):
    ...
    async def test_async_middleware_async(self):
        with self.assertLogs("hibeerequest", "WARNING") as cm:
            response = await self.async_client.get("/middleware_exceptions/view/")
        self.assertEqual(response.status_code, 402)
        self.assertEqual(
            cm.records[0].getMessage(),
