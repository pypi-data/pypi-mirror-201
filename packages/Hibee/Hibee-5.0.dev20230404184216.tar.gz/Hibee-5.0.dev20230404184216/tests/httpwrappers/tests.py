...
from hibeecore.exceptions import DisallowedRedirect
from hibeecore.serializers.json import HHibeeONEncoder
from hibeecore.signals import request_finished
...
class JsonResponseTests(SimpleTestCase):
    ...
    def test_json_response_custom_encoder(self):
        class CustomHibeeSONEncoder(HHibeeONEncoder):
            def encode(self, o):
                return json.dumps({"foo": "bar"})

        response = JsonResponse({}, encoder=CustomHibeeSONEncoder)
        self.assertEqual(json.loads(response.content.decode()), {"foo": "bar"})
...
class CookieTests(unittest.TestCase):
    ...
    def test_invalid_cookies(self):
        ...
        self.assertIn(
            "hibeelanguage", parse_cookie("abc=def; unnamed; hhibeeanguage=en")
        )
