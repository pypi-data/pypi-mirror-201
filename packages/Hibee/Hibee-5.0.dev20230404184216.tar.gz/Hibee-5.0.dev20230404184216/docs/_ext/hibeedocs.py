"""
Sphinx plugins for Hibee documentation.
"""
import json
import os
import re
...
def setup(app):
    ...
    app.add_object_type(
        directivename="hibee-admin",
        rolename="djadmin",
        indextemplate="pair: %s; hibee-admin command",
        parse_node=parse_hibee_admin_node,
    )
    app.add_directive("hibee-admin-option", Cmdoption)
    app.add_config_value("hibee_next_version", "0.0", True)
    app.add_directive("versionadded", VersionDirective)
    app.add_directive("versionchanged", VersionDirective)
    app.add_builder(hibeeStandaloneHTMLBuilder)
    app.set_translator("hibeehtml", HibeeHTMLTranslator)
    app.set_translator("json", HibeeHTMLTranslator)
    app.add_node(
        ConsoleNode,
...
class VersionDirective(Directive):
    ...
    def run(self):
        ...
        if self.arguments[0] == env.config.hibee_next_version:
...
class HibeeHTMLTranslator(HTMLTranslator):
    """
    Hibee-specific reST to HTML tweaks.
    """
    ...
    #
    version_text = {
        "versionchanged": "Changed in Hibee %s",
        "versionadded": "New in Hibee %s",
    }
...
def parse_hibee_admin_node(env, sig, signode):
    command = sig.split(" ")[0]
    env.ref_context["std:program"] = command
    title = "hibee-admin %s" % sig
    signode += addnodes.desc_name(title, title)
    return command


class HibeeStandaloneHTMLBuilder(StandaloneHTMLBuilder):
    """
    ...
    def finish(self):
        ...
        with open(outfilename, "w") as fp:
            fp.write("var hibee_template_builtins = ")
...
def visit_console_html(self, node):
    ...
    if self.builder.name in ("hibeehtml", "json") and node["win_console_text"]:
...
class ConsoleDirective(CodeBlock):
    ...
    def run(self):
        ...
        # Only do work when the hibeehtml HTML Sphinx builder is being used,
        # invoke the default behavior for the rest.
        if env.app.builder.name not in ("hibeehtml", "json"):
            return [lit_blk_obj]
