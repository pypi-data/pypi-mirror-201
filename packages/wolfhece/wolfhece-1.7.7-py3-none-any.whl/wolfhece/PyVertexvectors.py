...
class vector:
    ...
    def insert_nearest_vert(self,x,y):
        ...
        indexmp= np.argwhere(mynp==nearmp[0])[0,0] #index du vertex
    ...
    def get_values_linked_polygon(self,linked_arrays) -> dict:
        """
        ...
        for curarray in linked_arrays:
            if curarray.plotted:
                vals[curarray.idx] = curarray.get_values_insidepoly(self)
            else:
                vals[curarray.idx] = None

        return vals

    def get_all_values_linked_polygon(self,linked_arrays) -> dict:
        """
        ...
        for curarray in linked_arrays:
            if curarray.plotted:
                vals[curarray.idx] = curarray.get_all_values_insidepoly(self)
            else:
                vals[curarray.idx] = None

        return vals

    def get_all_values_linked_polyline(self,linked_arrays) -> dict:
        """
        ...
        for curarray in linked_arrays:
            if curarray.plotted:
                vals[curarray.idx], xy = curarray.get_all_values_underpoly(self, getxy=True)
            else:
                vals[curarray.idx] = None
...
class zone:
    ...
    def add_vector(self,addedvect,index=-99999):
        """
    ...
    def createmultibin(self,nb=None,nb2=0):
        """
    ...
    def get_all_values_linked_polygon(self, linked_arrays, stats=True, key_idx_names:Literal['idx', 'name']='idx'):
        """
        Récupération des valeurs contenues dans tous les polygones de la zone
        ...
        *****
        ...
        if key_idx_names=='idx':
            vals= {idx: {'values' : curpol.get_all_values_linked_polygon(linked_arrays)} for idx, curpol in enumerate(self.myvectors)}
        else:
            vals= {curpol.myname: {'values' : curpol.get_all_values_linked_polygon(linked_arrays)} for idx, curpol in enumerate(self.myvectors)}
        ...
        ***
        ...
        for curlabel, curarray in linked_arrays.items():
            if curarray.plotted:
                ...
                if withtopography and curarray==linked_arrays[linked_arrays.keys()[0]]:
...
class Zones(wx.Frame, Element_To_Draw):
    ...
    def add_zone(self, addedzone:zone):
        """
