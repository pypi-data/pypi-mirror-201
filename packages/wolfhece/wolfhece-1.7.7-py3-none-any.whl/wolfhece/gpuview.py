...
#extension GL_EXT_geometry_shader4 : enable
    ...
    // Parmetric color generation is said to be faster
    ...
    void main() {
        ...
        for(i = 0; i < gl_VerticesIn; i++)
        {
            ...
            // This has been normed in the texture, so value is between zero an done
            ...
            if (value >= 0) {
                ...
                //vec3 color = plasma_quintic(value);
...
class VectorField(Element_To_Draw):
    ...
    """
    def __init__(self, x_extent, y_extent, bounds, dx, dy, idx: str = '', plotted: bool = True, mapviewer=None, need_for_wx: bool = False) -> None:
    ...
    def _set_data(self, x_extent, y_extent):
    ...
    def plot(self, sx, sy,xmin,ymin,xmax,ymax):
        ...
        if inter is not None:
            ...
            # parent_projection_matrix = (GLfloat*16)()
            # glGetFloatv(GL_PROJECTION_MATRIX, parent_projection_matrix)
            ...
            # So we want arrows every 10 pixels
            # The step is expressed in world coordinates
            step_x = view_bounds.width / (vp_width / self.arrow_pixel_size*3)
            step_y = view_bounds.height  / (vp_height / self.arrow_pixel_size*3)

            # Size of the arrow on screen (will be multiplied
            # by the norm of the vector which is in [0,1])
            # in normalized coordinates (0,1 -> 0,screen size)
            # FIXME This is dependant on the screen resolution !
            ...
            arrow_scale_x *= self.zoom_factor
            arrow_scale_y *= self.zoom_factor
            ...
            glMatrixMode(GL_PROJECTION)
            glPopMatrix()
            #glLoadMatrixf(parent_projection_matrix)
