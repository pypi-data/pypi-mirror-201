...
class PlotCSAll(ProfilePanel):
    ...
    def init_UI(self):
        ...
        #Other options
        self.Butsimul = wx.Button(self, label = _('Simulations'))
        self.Butprofvert =  wx.Button(self, label = _('Modify profile'))
        self.Butupdateprof = wx.Button(self, label = _('Update profile'))
        ...
        self.Butprofvert.Bind(wx.EVT_BUTTON, self.modify_vertex)
        self.Butupdateprof.Bind(wx.EVT_BUTTON, self.update_profile)
        ...
        self.Butprofvert.SetToolTip(_('Display a table containing the profile vertices (only available in modification mode -> Options).'))
    ...
    def modify_vertex(self, event: wx.Event):
        '''
    ...
    def modify_vertex_sz(self, event: wx.Event):
        '''
        '''
        if self.compare is not None:
            if self.grid_sz is None:
                self.grid_sz = windowsgrid(parent=None)
                self.grid_sz.set_grid_sz(my_profile =self.mycs,
                                            ref = self.compare,
                                            title= 'C.S.- %s'%(self.mycs.myname),
                                            callback= self._update_profile_callback)
    ...
    def update_profile(self, event:wx.Event):
        ...
        if self.compare is not None and self.grid_vertices is not None:
            ...
            #self.project_zones_on_trace(self.zones) # FIXME (new line bellow)
    ...
    def harmonize_profiles(self):
        ...
        if self.compare:
            ...
            mycs = np.array([sz1[0], sz1[1]])
            reference = np.array([sz2[0], sz2[1]])
            merged_sz = np.concatenate((mycs,reference), axis =1)
            new_sz = np.unique(merged_sz, axis = 1)

            # Finding all differences between sz data
            dif_sz1 = np.setdiff1d(new_sz[0], sz1[0], assume_unique=True)
            dif_sz2 = np.setdiff1d(new_sz[0], sz2[0], assume_unique=True)

            # Harmonizing the modified profile
            if dif_sz1.any() != False:
                length_cs = self.mycs.asshapely_ls()
                cs_vertices = []
                cs_nb = 0
                for i in new_sz[0]:
                    coords  = length_cs.interpolate(i)
                    curvert = wolfvertex(coords.x, coords.y, coords.z)
                    cs_vertices.append(curvert)
                    cs_nb += 1
                self.mycs.myvertices= cs_vertices
                self.mycs.nbvertices = cs_nb

            # Harmonizing the profile of reference
            if dif_sz2.any() != False:
                length_compare = self.compare.asshapely_ls()
                compare_vertices = []
                compare_nb = 0
                for i in new_sz[0]:
                    coords  = length_compare.interpolate(i)
                    curvert = wolfvertex(coords.x, coords.y, coords.z)
                    compare_vertices.append(curvert)
                    compare_nb += 1
                self.compare.myvertices= compare_vertices
                self.compare.nbvertices = compare_nb

            self.grid_vertices.update_grid_vert(my_profile =self.mycs,
                                            ref = self.compare,
                                            title= 'C.S.- %s'%(self.mycs.myname),
                                            callback= self._update_profile_callback)
...
class windowsgrid(wx.Frame):
    ...
    def __init__(self, *args, callback=None, **kwargs):
        ...
        self.Bind(wx.EVT_CLOSE, self.Quit) #FIXME Nothing happens
...
class ProfileNotebook(wx.Frame):
    ...
    def modify_cs( self,event):
        ...
        newpage = self.add("Modified C.S. {}".format(n))
        ...
        newpage.vec = page_src.vec # To avoid redrawing the vector (structures)
