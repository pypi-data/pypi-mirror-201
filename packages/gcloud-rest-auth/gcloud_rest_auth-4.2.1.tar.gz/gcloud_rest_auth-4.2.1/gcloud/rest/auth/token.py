...
class Token:
    ...
    def _refresh_authorized_user(self, timeout: int) -> Response:
        payload = urlencode({
        ...
        resp: Response = self.session.post(  # type: ignore[assignment]
            url=self.token_uri, data=payload, headers=REFRESH_HEADERS,
            timeout=timeout,
        )
        return resp

    def _refresh_gce_metadata(self, timeout: int) -> Response:
        resp: Response = self.session.get(  # type: ignore[assignment]
            url=self.token_uri, headers=GCE_METADATA_HEADERS, timeout=timeout,
        )
        return resp

    def _refresh_service_account(self, timeout: int) -> Response:
        now = int(time.time())
        assertion_payload = {
            'aud': self.token_uri,
