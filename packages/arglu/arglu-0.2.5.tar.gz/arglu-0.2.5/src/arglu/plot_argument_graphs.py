...
def preprocess_node_texts(nx_graph):
    for n in nx_graph.nodes:
        keys = list(nx_graph.nodes[n].keys())
        for k in keys:
            node_string = str(nx_graph.nodes[n][k]).replace(":","")
            node_string = node_string[:2500]
            nx_graph.nodes[n][k] = node_string
    return nx_graph
...
class GraphGenerator():
    ...
    def __init__(self, graph, colour_map =None, title=None):
        self.graph = preprocess_node_texts(graph)                
        self.colour_map = colour_map
        if colour_map:
            self.colour_map = {str(k): v for k, v in colour_map.items()}
...
#       xzklcmzcx
    ...
    def update_annot(self, ind):
        ...
        comment_text = re.sub("[^\s\w!?.,-]+", "", comment_text)
