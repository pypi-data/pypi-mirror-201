...
def get_daily_selic_from_bacen(init_date: datetime=datetime(year=1986, month=6, day=4), final_date: datetime=None):
    '''
...
def get_monthly_selic_from_bacen(init_date: datetime=datetime(year=1986, month=6, day=1), final_date: datetime=None):
    '''
    Get SELIC data from the BACEN API.
    '''
    ...
    response = requests.get(URL + INIT_DATE_PARAMETER.format(init_date=init_date.strftime('%d/%m/%Y')) + FINAL_DATE_PARAMETER.format(final_date=final_date.strftime('%d/%m/%Y')))
    if response.ok:
        return {datetime.strptime(selic_data['data'], '%d/%m/%Y'): Decimal(selic_data['valor']) / Decimal('100') for selic_data in json.loads(response.text)}
        ...
        for table in tables:
            ...
            for row in data:
                if row[0] == 'Mês/Ano':
                    ...
                    for i in range(len(row)):
                        if '%' in row[i]:
                            dict_selic[datetime.strptime(replace_month_name_with_number(row[0]) + '/' + dict_years[i], '%m/%Y') ] = Decimal(row[i].replace(',', '.').replace('%', '')) / Decimal('100')
...
def get_monthly_selic_hard_data():
    ...
    with open('dados_selic_mensal.csv') as file:
        ...
        for line in reader:
            selic_monthly_data.update({datetime.strptime(line[0], '%b/%Y'): Decimal(line[1].replace(',', '.'))})
...
def get_monthly_acc_selic_in_period(init_date: datetime, final_date: datetime):
    ...
    selic_data = get_daily_selic_from_bacen(init_date.replace(day=1), final_date.replace(day=1) + relativedelta(months=+1) + relativedelta(days=-1)) # Como a taxa é acumulada mensalmente, o dia da data final tem que ser o último dia do mês e o primeiro da data inicial.
    return {month: calc_selic_month(selic_data, month) - Decimal('1') for month in months}
